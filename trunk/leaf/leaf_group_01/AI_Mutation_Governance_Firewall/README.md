...
<h1 align="center">🛡️ AI Mutation Governance Firewall</h1>
...
A runtime AI mutation gate for Kubernetes-grade admission control for AI evolution. Deterministic shadow simulation and quorum validation ensure every model change proposal is constitutionally sound.
...
cd AI Mutation Governance Firewall
```
...
```bash
# Build image
docker build -t AI Mutation Governance Firewall:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  AI Mutation Governance Firewall:latest
```
...
```bash
kubectl rollout status deployment/AI Mutation Governance Firewall
```

1. **View logs**

```bash
kubectl logs -f deployment/AI Mutation Governance Firewall
```
...
- `service_ai_mutation_governance_firewall_requests_total` - Total requests
- `service_ai_mutation_governance_firewall_request_duration_seconds` - Request latency
- `service_ai_mutation_governance_firewall_requests_inflight` - In-flight requests
- `service_ai_mutation_governance_firewall_db_query_duration_seconds` - Database query latency
