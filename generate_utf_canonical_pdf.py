...
MD_FILE = BASE / "Thirsty-Lang_UTF_Reference_CANONICAL.md"
PDF_FILE = Path("./Thirsty-Lang_UTF_Reference_CANONICAL.pdf")
CSV_FILE = Path("./Thirsty-Lang_UTF_Source_Map_CANONICAL.csv")
...
UTF_STATS = {
    ...
    "modules": {
        "Thirsty-Lang (Tier 1)": {
            "files": 17,
            "lines": 4335,
            "desc": "Lexer, parser, interpreter, CLI",
        },
        "T.A.R.L. (Tier 3)": {
            "files": 6,
            "lines": 753,
            "desc": "Policy runtime and bytecode VM",
        },
        "Shadow Thirst (Tier 4)": {
            "files": 3,
            "lines": 262,
            "desc": "Mutation simulation layer",
        },
        "TSCG (Tier 5)": {
            "files": 3,
            "lines": 139,
            "desc": "Symbolic constitutional grammar",
        },
...
UTF_EXAMPLES = [
  ...
  return 0;
    ...
    (
        "gods.thirstofgods",
        "Advanced: Classes, Async, Error Handling",
...
glass main() -> Int {
  ...
  when actor.role != "builder" => DENY;
    ...
    (
        "promote.shadowthirst",
        "Shadow Thirst Mutation",
...
UTF_FEATURE_MATRIX = [
    ...
    [
        ...
        "Thirsty-Lang",
        "Source syntax",
        "Water metaphor keywords, type system",
        "✓ Implemented",
    ],
    [
        "2",
        "Thirst of Gods",
        "Advanced dialect",
        "Extended stdlib, async/await patterns",
        "✓ Implemented",
    ],
    [
        "3",
        "T.A.R.L.",
        "Policy/Runtime",
        "Governance policies, bytecode VM",
        "✓ Implemented",
    ],
    [
        "4",
        "Shadow Thirst",
        "Mutation sim",
        "Shadow/invariant/canonical execution",
        "✓ Implemented",
    ],
    [
        "5",
        "TSCG",
        "Symbolic grammar",
        "9 symbols (COG,DNT,SHD,INV,CAP,QRM,COM,ANC,RFX)",
        "✓ Implemented",
    ],
    [
        "6",
        "TSCG-B",
        "Binary protocol",
        "Frame-based, CRC32+SHA256 integrity",
        "✓ Implemented",
    ],
]
    ...
    while i < len(lines):
        ...
        if nm:
            story.append(
                Paragraph(
                    fmt_line(nm.group(2)),
                    ParagraphStyle("Num", parent=S["Body"], leftIndent=20),
                )
            )
            i += 1
            continue
...
def title_page():
    ...
    story.append(
        HRFlowable(
            width="60%", thickness=2, color=BLUE_MID, hAlign="CENTER", spaceAfter=16
        )
    )
    story.append(
        Paragraph(
    ...
    for k, v in meta:
        story.append(
            Paragraph(
                f'<b><font color="#0D2B55">{k}:</font></b>&nbsp;&nbsp;{v}', S["DocMeta"]
            )
        )
    ...
    story.append(
        HRFlowable(
            width="80%", thickness=0.5, color=GREY_RULE, hAlign="CENTER", spaceAfter=20
        )
    )
...
SOURCE_MAP = [
    (
        ...
        "Title and abstract of all three documents",
        "HIGH",
    ...
    (
        ...
        "src/utf/README.md",
        "Full document; 'What is executable now' section",
        "HIGH",
        "Primary source",
    ),
    (
    ...
    (
        ...
        "Complete Python class; Lexer.lex()",
        "HIGH",
        "",
    ),
    (
        ...
        "analyze() function returns 6 AnalysisResult per mutation",
        "HIGH",
        "",
    ),
    (
        ...
        "src/utf/shadow_thirst/core.py",
        "promote() function returns PROMOTE or REJECT",
        "HIGH",
        "",
    ),
    (
    ...
    ("TSCG-B magic = TSGB", "src/utf/tscg_b/core.py", "MAGIC = b'TSGB'", "HIGH", ""),
    (
        "TSCG-B CRC32 + SHA-256 integrity",
        "src/utf/tscg_b/core.py",
        "pack_text() and unpack_frame()",
        "HIGH",
        "",
    ),
    (
        ...
        "evaluate() returns 'DENY' when no rule matches (line 151)",
        "HIGH",
    ...
    (
        ...
        "tarl/runtime.py",
        "TarlRuntime class with _decision_cache and ThreadPoolExecutor",
        "HIGH",
        "",
    ),
    (
        ...
        "Layer 0-7 documented in both docs",
        "HIGH",
        "",
    ),
    (
    ...
    (
        ...
        "Two documents with different version numbers",
        "DOCUMENTED CONFLICT",
    ...
    (
        ...
        "initConstitution() pushes 5 rule objects with action fields",
        "HIGH",
        "",
    ),
    (
        ...
        "!enforcementActive branch returns allowed:false 'Gateway offline'",
        "HIGH",
        "",
    ),
    (
        ...
        "_execute_stage() with role_sig, policy_binding, sha256 hash",
        "HIGH",
        "",
    ),
    (
        ...
        "build.tarl",
        "Full file: drink/glass/pour/shield/thirsty keywords",
        "HIGH",
        "",
    ),
    (
        ...
        "PRIORITY_REALTIME=0 through PRIORITY_IDLE=7",
        "HIGH",
        "",
    ),
    (
        ...
        "None",
        "Inferred from src/psia/ directory name and Shadow Thirst spec",
        "LOW",
    ...
    (
        ...
        "policy PromotionGate { ... } with 3 rules",
        "HIGH",
        "",
    ),
    (
        ...
        "mutation validated_canonical set_counter(value: Int) { shadow invariant canonical }",
        "HIGH",
        "",
    ),
]
...
def implementation_statistics_section():
    ...
    for name, stats in UTF_STATS["modules"].items():
        module_data.append(
            [name, str(stats["files"]), f'{stats["lines"]:,}', stats.get("desc", "")]
        )
    module_table = Table(
        module_data, colWidths=[2.2 * inch, 0.8 * inch, 1 * inch, 2 * inch]
    )
...
def comprehensive_examples_section():
    ...
    for filename, title, code in UTF_EXAMPLES:
        ...
        # Use XPreformatted for better performance and exact code layout
        story.append(XPreformatted(code, S["Code"]))
...
def build_pdf():
    ...
    doc = DocTemplate(
        str(PDF_FILE),
        pagesize=letter,
        doc_title="Thirsty-Lang UTF Canonical Reference v1.3",
    ...
    print(
        f"  Comprehensive coverage: Statistics, Features, Architecture, Examples + Full Reference"
    )
