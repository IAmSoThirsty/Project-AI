---
type: protocol-doc
tags: [specialized-systems, h323, network-design, architecture-patterns, telecommunications]
created: 2026-01-15
last_verified: 2026-04-20
status: current
related_systems: [h323-zone-standard, network-architecture, dmz-design]
stakeholders: [network-team, architecture-team, voice-engineering]
...
1. Configure firewall ACLs
1. Deploy primary Gatekeeper
1. Deploy standby Gatekeeper
1. Configure GK clustering
1. Deploy gateways in DMZ
1. Configure gateway trunks
1. Issue PKI certificates
1. Deploy endpoints
1. Validate secure call flows
...
1. Establish WAN connectivity (MPLS/VPN)
1. Configure GK-to-GK TLS/IPsec
1. Configure LRQ/LCF routing
1. Test inter-site calls
1. Test local survivability (WAN failure simulation)
1. Document dial plan and routing rules
...
1. GK fails
1. Endpoints failover to cloud-based registration service
1. Calls route via cloud SIP trunks
1. Internal calls use cloud MCU as bridge

**Cloud Failure**:

1. Cloud SIP trunk fails
1. Calls failover to on-prem PSTN gateway
1. On-prem MCU handles conferences

### 5.9 Deployment Steps

1. Deploy Campus or Multi-Site pattern on-prem
1. Provision cloud SIP trunk account
1. Configure SBC/Gateway for cloud connectivity
1. Establish TLS/SRTP to cloud
1. Configure PKI (federated or centralized)
1. Test cloud call flows (inbound/outbound)
1. Test failover scenarios (on-prem ↔ cloud)
1. Configure cloud MCU (if used)
1. Integrate cloud logs with on-prem SIEM
...
1. Issue PKI certificates
1. Enable H.235 in phases (zone-by-zone)
1. Enforce SRTP
1. Decommission insecure endpoints

### 8.2 SIP → H.323 (Hybrid)

1. Deploy H.323 GK/GW alongside SIP infrastructure
1. Configure SIP ↔ H.323 gateway
1. Migrate endpoints in phases
1. Maintain dual-stack during transition

### 8.3 On-Prem → Hybrid Cloud

1. Deploy SBC for cloud connectivity
1. Provision cloud SIP trunk
1. Route selected calls via cloud
1. Migrate endpoints to cloud registration (optional)
1. Decommission on-prem gateways (if desired)
