...
2. 🐍 **Environment** - Hash-locked dependency install
3. 🔍 **Static Analysis** - Ruff linting
4. 🧪 **Unit Tests** - Pytest with coverage
5. ⚡ **TARL** - Runtime enforcement tests
6. 🎯 **Canonical** - 5 invariants (MUST PASS)
7. 🛡️ **Adversarial** - Red team evaluation
8. 📋 **SBOM** - Syft SPDX generation
9. 🐳 **Docker** - Multi-stage build + push
10. 🔏 **Attestation** - SLSA 3 provenance
11. 📦 **Artifacts** - Upload trace, SBOM, reports
12. 📊 **Summary** - Trust boundary report
...
2. Action Supply Chain → SHA pinning
3. Registry Impersonation → Hardcoded GHCR
4. Digest Mutation → Attestation signature
5. Replay Attack → Temporal metadata
6. Privilege Escalation → Minimal perms
7. Artifact Tampering → Immutable storage
8. Silent Failure → Explicit exits
9. Floating Tag Poison → Digest pinning
10. Build Drift → Canonical replay
11. Base Image Compromise → Digest pinning
12. Tool Injection → Official sources
13. Context Leakage → Secret masking
