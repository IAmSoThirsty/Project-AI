...
<!-- # COMPLIANCE: Sovereign Substrate / CODEX_DEUS_QUICK_REF.md # -->
<!-- # ============================================================================ # -->
<div align="right">
...
**File:** `.github/workflows/codex-deus-ultimate.yml` **Lines:** 2,507 **Jobs:** 55 **Phases:** 15
...
1. All tests pass
1. Security scans pass
1. Patch or minor version update
...
1. `package-release` - Build for Linux/Windows/macOS
1. `sign-artifacts` - Cosign signing
1. `create-github-release` - Create GitHub Release
1. `publish-pypi` - Publish to PyPI
1. `publish-docker` - Push to ghcr.io
...
1. Verify files changed not in `paths-ignore`
1. Check concurrency group (may have cancelled)

### Job Was Skipped

1. Check `if:` condition
1. Verify `needs:` dependencies succeeded
1. Review initialization outputs

### Tests Failed

1. Check test logs in job output
1. Download `python-test-reports-*` artifact
1. Review JUnit XML for details

### Security Scan Failed

1. Check SARIF uploads in Security tab
1. Download security report artifacts
1. Review specific tool output (Bandit/Trivy/etc)

### Release Didn't Publish

1. Verify tag format: `v1.2.3`
1. Check `prepare-release` job for version detection
1. Ensure all dependency jobs succeeded
1. Verify PyPI/Docker credentials configured
...
1. Edit `env.PYTHON_VERSION_MATRIX: '["3.11", "3.12"]'`
1. Update composite action default if needed

### Change Coverage Threshold

1. Edit `env.COVERAGE_THRESHOLD_PYTHON: 80`
1. Or pass via workflow_dispatch input

### Add New Job

1. Add job definition under appropriate phase
1. Set `needs:` for dependencies
1. Add conditional `if:` based on initialization
1. Set `timeout-minutes`
1. Update Phase 15 reporting to include new job
...
1. Wait for auto-approval if all gates pass
1. Review auto-fix commits if linting fails
1. Check coverage report in PR comments

### For Maintainers

1. Monitor workflow health weekly
1. Review stale issue/PR cleanup
1. Check security alerts from automated scans
1. Update dependencies from auto-fix PRs
1. Review release artifacts before publishing

### For Security

1. Check Security tab for SARIF uploads
1. Review dependency audit results
1. Monitor scheduled security scans
1. Triage auto-created security issues
