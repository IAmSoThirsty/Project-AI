...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate a Thirsty's Standard manifest or conformance report.")
    ...
    elif args.document.name.endswith(".manifest.yaml") or args.document.name.endswith(".manifest.json"):
    ...
    if args.document.name.endswith(".manifest.yaml") or args.document.name.endswith(".manifest.json"):
    ...
    if "rules" in document:
        ...
        expression_count = len({control["applies_when"] for rule in document["rules"] for control in rule["controls"]})
        print(f"VERIFIED: {len(document['rules'])} rules, {control_count} controls, {len(document['test_catalog'])} tests, {expression_count} CEL expressions")
    return 0
