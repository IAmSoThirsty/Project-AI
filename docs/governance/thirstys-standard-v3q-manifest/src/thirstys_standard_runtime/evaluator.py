    ...
    for rule in manifest["rules"]:
        for control in rule["controls"]:
            ...
            try:
                ...
                continue
            if not applies:
                results.append({"control_id": control_id, "result": "not_applicable", "evidence_ids": [], "notes": "CEL condition evaluated false."})
                continue
            assertion = actor_results.get(control_id)
            if assertion is None:
                result = "blocked" if control["severity"] == "critical" else "fail"
                results.append({"control_id": control_id, "result": result, "evidence_ids": [], "notes": "Actor supplied no control assertion."})
            ...
            results.append({
                ...
                "notes": "; ".join(notes) or assertion.get("notes", "Independent evaluator accepted the assertion and evidence envelope."),
            })
