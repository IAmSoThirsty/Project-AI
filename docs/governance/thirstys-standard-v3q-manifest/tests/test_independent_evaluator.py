...
from thirstys_standard_runtime.authority import generate_keypair, sign_document, verify_signed_document, write_private_key


def test_independent_evaluator_runs_in_separate_process_and_signs_report(root: Path, tmp_path: Path) -> None:
    actor_private, actor_public = generate_keypair("actor-key", "actor-agent", ["execution_record"])
    evaluator_private, evaluator_public = generate_keypair("evaluator-key", "independent-evaluator", ["evaluation_report"])
    ...
    manifest = {
        ...
        "evidence_model": {
            "admissibility_required_fields": [
                "evidence_id", "type", "source", "captured_at", "environment", "revision",
                "method", "outcome", "integrity", "freshness", "claim_ids"
            ]
        },
        "rules": [
            {
    ...
    record = {
        ...
        "recorded_at": now,
        "evaluation_scope": {"control_ids": ["Q-001-A"], "reason": "isolated evaluator conformance test"},
        "control_results": [{"control_id": "Q-001-A", "result": "pass", "evidence_ids": ["ev-test"], "notes": "Actor assertion"}],
        "claims": [],
        "evidence": [evidence],
        "residual_risks": [],
    }
    ...
    evaluator_identity_path.write_text(json.dumps({"id": "independent-evaluator"}), encoding="utf-8")
    write_private_key(evaluator_private_path, evaluator_private)

    env = os.environ.copy()
    env["PYTHONPATH"] = os.pathsep.join([str(root / "src"), str(root / "vendor"), env.get("PYTHONPATH", "")])
    completed = subprocess.run(
        [
            ...
            "--evaluator-identity", str(evaluator_identity_path),
            "--evaluator-private-key", str(evaluator_private_path),
...
def test_independent_evaluator_rejects_false_not_applicable_assertion(tmp_path: Path) -> None:
    ...
    actor_private, actor_public = generate_keypair("actor-key-2", "actor-agent-2", ["execution_record"])
    evaluator_private, evaluator_public = generate_keypair("evaluator-key-2", "independent-evaluator-2", ["evaluation_report"])
    registry = {"keys": [actor_public, evaluator_public]}
    manifest = {
        "manifest_id": "urn:test:manifest:applicable",
        "manifest_version": "test-1",
        "evidence_model": {"admissibility_required_fields": ["evidence_id", "type", "source", "captured_at", "environment", "revision", "method", "outcome", "integrity", "freshness", "claim_ids"]},
        "rules": [{"id": "Q-001", "controls": [{"id": "Q-001-A", "applies_when": "true", "severity": "critical", "required_evidence": []}]}],
    }
    now = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    record = {
        "record_id": "record-2",
        "task": {"task_id": "task-2", "request": "test", "mode": "governance_system", "risk_level": "high", "authority_context": {}, "workspace_context": {}},
        "actor": {"id": "actor-agent-2", "key_id": "actor-key-2", "process_id": 999999},
        "recorded_at": now,
        "evaluation_scope": {"control_ids": ["Q-001-A"]},
        "control_results": [{"control_id": "Q-001-A", "result": "not_applicable", "evidence_ids": [], "notes": "attempted bypass"}],
        "claims": [],
        "evidence": [],
        "residual_risks": [],
    }
...
def test_independent_evaluator_rejects_actor_key_id_mismatch() -> None:
    ...
    actor_private, actor_public = generate_keypair("actor-key-3", "actor-agent-3", ["execution_record"])
    evaluator_private, evaluator_public = generate_keypair("evaluator-key-3", "independent-evaluator-3", ["evaluation_report"])
    registry = {"keys": [actor_public, evaluator_public]}
    manifest = {
    ...
    record = {
        "record_id": "record-3",
        "task": {"task_id": "task-3", "request": "test", "mode": "governance_system", "risk_level": "high", "authority_context": {}, "workspace_context": {}},
        "actor": {"id": "actor-agent-3", "key_id": "wrong-key-id", "process_id": 999998},
        "recorded_at": now,
        "evaluation_scope": {"control_ids": ["Q-001-A"]},
        "control_results": [{"control_id": "Q-001-A", "result": "pass", "evidence_ids": [], "notes": "test"}],
        "claims": [],
        "evidence": [],
        "residual_risks": [],
    }
