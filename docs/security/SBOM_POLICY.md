...
1. **Component Name** - Package name
1. **Version** - Semantic version or commit hash
1. **Other Unique Identifiers** - Package URLs (purl), CPE
1. **Dependency Relationships** - Component hierarchy
...
1. **Sigstore Fulcio** - Issues short-lived signing certificates
1. **Sigstore Rekor** - Records signatures in transparency log
1. **Certificate Verification** - Validates GitHub identity claim
...
1. **Investigate:** Review Rekor log for unauthorized entries
1. **Regenerate:** Create new SBOM with updated signatures
1. **Notify:** Inform users via security advisory
1. **Review:** Audit GitHub Actions workflow permissions
...
1. **Prioritize:** Assign severity (Critical/High/Medium/Low)
1. **Patch:** Update vulnerable dependency
1. **Regenerate:** Create new SBOM with patched dependency
