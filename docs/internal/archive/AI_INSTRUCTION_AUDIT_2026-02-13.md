...
**Issue:** Repetitive bash command execution causing malfunction
**Status:** ✅ RESOLVED
...
2. **Line 29 (Original):** "After every response - verify you ran `codacy_cli_analyze` tool"
3. **Line 51 (Original):** "You MUST run the `codacy_cli_analyze` tool" (for dependencies)
4. **Line 68 (Original):** "You MUST NOT wait for the user to ask for analysis"
...
2. **Edit a markdown file** - Tool should NOT run (non-code file)
3. **Tool unavailable scenario** - AI should skip analysis and continue task
4. **Multiple file edits** - Each file analyzed at most once
5. **Dependency changes** - Optional security check, not blocking
...
2. **Implement circuit breakers** to prevent loops
3. **Track state** to avoid redundant operations
4. **Prioritize workflows** over perfect execution
5. **Fail gracefully** when tools unavailable
6. **Limit scope** with explicit exclusions
...
2. **Add circuit breakers** to any tool-calling instructions
3. **Use state tracking** to prevent redundant operations
4. **Test instructions** with simulated tool failures
5. **Monitor for repetitive behavior** in AI assistant logs
6. **Balance quality vs. efficiency** - don't let perfect be the enemy of good
...
**Date:** 2026-02-13
**Status:** Changes implemented and documented
**Next Review:** When similar issues reported or during quarterly instruction audit
