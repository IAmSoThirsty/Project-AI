...
**Status:** The Control Center surfaces required by the v0.0.3 single-user P1
Offline-First Local release are implemented and released. Remaining items in
this plan are post-release enhancements or future optional delivery surfaces.
**Date:** 2026-07-15; evidence updated 2026-07-22
...
7. **Protected audit access accepts a human session with field-level visibility.** Machine bearer access remains available for non-browser reads. Human search returns fixed summaries; exact raw-identifier filters require `audit.raw_view`; record detail is normalized and permission-filtered; and human exports are server-generated, redacted, capped, rate-limited, digested, audit-recorded, and subject to the same filter-visibility boundary.
8. **Coverage is still incomplete across clients.** The operator console now has route-wide automated accessibility checks, responsive/reflow and zoom acceptance, durable cross-platform visual regression, and targeted real-browser keyboard-flow checks. Broader role-matrix, module-workflow, native desktop screen-reader, and Android TalkBack coverage remains absent.
...
**Post-release enhancement work not ready:** managed PostgreSQL TLS/credential rotation and live-cluster restore drills, durable notification history, global record search, additional deep module workflows, complete desktop/mobile integration, and broader NVDA/TalkBack acceptance remain incomplete. These are not requirements of the released single-user P1 web Control Center. The current audit explorer verifies the complete chain, uses stable hash-cursor navigation that is not displaced by later appends, and supports query, event, actor, account, operation, resource, verdict, severity, and timezone-bounded time filters. Existing machine/proof clients retain offset compatibility while the human console uses cursor history for newer/older navigation and submits identifiers in a same-origin POST body instead of placing them in request URLs or default access logs. Human search responses contain fixed summaries rather than arbitrary relay fields. Exact actor, account, operation, and resource filters require `audit.raw_view`; lower-privilege free-text searches evaluate only the visible summary projection, and export applies the same restriction so result counts cannot reveal hidden values. Selecting a record re-verifies the complete chain and returns normalized integrity metadata through server-enforced field visibility: Owner, Administrator, and Auditor roles can inspect sanitized raw JSON under `audit.raw_view`, while Reviewer, Operator, and Viewer roles receive allowlisted fields, hashed identifiers, and no raw record. Credential-bearing fields remain redacted for every role, React renders all evidence as escaped text, and any verification failure clears cached results before presenting an integrity-lockdown state. Authorized owner, administrator, reviewer, and auditor sessions can request a server-generated export of up to 500 matching records; the API verifies the audit chain before release, removes arbitrary fields, hashes identifying values, binds the result to a SHA-256 digest, applies a durable rate limit, and appends an export receipt without copying raw query or record content into the receipt. Machine credentials and lower-privilege human roles cannot use the export endpoint. The shell provides keyboard screen search, live submitted-request notifications, and explicitly browser-local density/reduced-motion preferences. Axe-core automation now covers all 16 implemented authenticated routes plus consequential auth and workflow states. The command dialog contains and restores focus, route changes focus the main content landmark, the environment label comes from the server, and Help is a real documentation link rather than a dead control. The narrow navigation is removed from keyboard/accessibility traversal while closed; while open it is modal, contains focus, closes on Escape, and restores focus to its trigger. The work-queue horizontal scroller is a named, keyboard-focusable region with a visible narrow-screen continuation cue. A local rendered-browser pass on 2026-07-21 found zero axe violations, including rendered color contrast, across all 16 authenticated routes and zero document-level overflow across 48 route/viewport checks at 320, 768, and 1440 CSS pixels. A separate native-browser 200% zoom pass verified all 16 routes at a measured zoom factor of 2 with visible main content and no document-level horizontal overflow. Durable Playwright screenshot regression now covers the desktop and mobile Command Center, open mobile navigation, collapsed/expanded Audit Explorer, desktop privileged record detail, and mobile permission-filtered detail against visually reviewed Windows and Linux baselines; a dedicated CI job compares the production build and uploads failure artifacts. The negative control proves a material CSS change fails the gate. Real-browser functional checks also prove skip-link bypass, mobile navigation focus containment/restoration, record-detail focus movement/restoration and reflow, and the audit filter control sequence on both Windows and Linux. Android debug/release lint now treats warnings as errors, release builds reject non-HTTPS API URLs, cleartext is restricted to the emulator host in debug, storage backup/transfer is disabled, and the read-only interface provides density-independent targets, a bounded result scroller, a heading, and concise load announcements. The native desktop now exposes tested assistive-technology metadata across navigation, status, evidence tables, capability inspection, and settings; its 900 by 620 Windows rendering and offscreen launch were checked. Request creation uses server-provided, versioned input contracts persisted in workflow schema version 4 alongside generic durable analysis receipts. PostgreSQL adapters provide shared account/session/workflow/receipt state, transaction-locked bootstrap/rate limits/execution reservation, multi-instance visibility, fail-closed SQLite migration, and dump/restore evidence. The SWR route remains the first execution-gate-backed workflow. Atlas Replay and Atlas Projections are separate direct human analysis workflows: both are session/CSRF/permission checked, deterministic, bounded, non-actuating, and visibly subordinate to governance. Projection receipts persist canonical input/output evidence and audit hashes without copying raw claim or evidence content into the audit relay. Human decision, analysis, and execution receipts remain visibly distinct.
**Latest shell/state acceptance:** The former hard-coded green `Live query` indicator has been removed. The shell now polls the liveness endpoint and presents checking, live, stale, unavailable, and browser-offline states without claiming that process liveness proves whole-system health. The Command Center independently elevates partial surface evidence, failed-refresh stale snapshots, and offline retained snapshots with explicit last-observed times. Work Queue, Evidence, Account Security, Administration, and module catalogs now distinguish pending, failed, successful-empty, and populated initial reads. Failed reads hide record counts and consequential controls rather than fabricating zero, disabled-authenticator, or available-action state. SWR, TAAR, Atlas Projections, and Atlas Replay now gate consequential controls on successful initial reads; permission-denied module reads are labeled as access restricted rather than generic outages, and SWR/request submission controls are withheld for roles without the matching interface permission. System Health elevates every non-healthy surface as partial evidence. Deterministic operator coverage is 64/64 and total web coverage is 106/106 from the current 64 operator tests plus docs 5, proof 4, and Triumvirate 33; the full Chromium browser suite passes 21/21 with eight functional checks, repository web lint, and the operator production build passed. The pinned Linux browser run passed all five functional checks and a clean 18/18 candidate generation, but rejected all thirteen concurrently modified Linux image baselines; those images were preserved and remain a current release-gate issue, not accepted as new truth.
**Superseding 2026-07-22 role-state/runtime evidence:** Administration direct-link `403` responses now render an explicit role-restricted state and withhold account controls and security-event content. SWR viewer mode, Atlas Projections viewer denial, Security fail-closed initial reads, temporary-password routing, degraded System Health evidence, module-catalog failure, and partial replay/DOI evidence are now covered by real-browser checks alongside the existing module boundaries. Operator-console coverage is 66/66; total web coverage is 108/108; full Chromium is 30/30 with seventeen functional checks. The operator-console Compose image was rebuilt from current source, all nine local services are healthy, and the API/docs/proof/operator health probes returned HTTP 200. No visual baseline was regenerated.
**Superseding 2026-07-22 authentication evidence:** The production-built sign-in route now has real-browser coverage for the 401 session boundary, visible server-authentication/governance assurances, and failed-credential alert handling. The local recovery route has real-browser coverage for recovery-code submission, successful completion status, and the absence of an email workflow. The full Windows Chromium suite and the digest-pinned Linux read-only suite both pass 32/32, comprising thirteen screenshot comparisons and nineteen functional checks. The rebuilt Compose operator image and all nine local services remain healthy; no visual baseline was regenerated.
**Superseding 2026-07-22 first-run/auth-service evidence:** The production-built auth surface now also covers bootstrap-status failure, with the protected workspace withheld behind an explicit account-service-unavailable state, and the required first-run Owner setup flow through recovery-code acknowledgement before Control Center entry. The full Windows Chromium suite and digest-pinned Linux read-only suite now pass 34/34: thirteen screenshot comparisons plus twenty-one functional checks. No visual baseline was regenerated.
**Superseding 2026-07-22 TAAR workflow evidence:** The production-built TAAR console now has real-browser coverage for a successful registered-reader run, the sealed evidence and audit hashes, refreshed history, and the explicit report-only/no-source-mutation/no-governance boundary. The full Windows and digest-pinned Linux suites now pass 35/35, comprising thirteen screenshot comparisons and twenty-two functional checks. No visual baseline was regenerated.
**Superseding 2026-07-22 SWR workflow evidence:** The production-built Sovereign War Room now has real-browser coverage for a reviewed scenario request submitted through the execution-gate endpoint, the durable execution receipt, and the explicit browser boundary that no capability token is exposed. The full Windows and digest-pinned Linux suites now pass 36/36, comprising thirteen screenshot comparisons and twenty-three functional checks. No visual baseline was regenerated.
**Superseding 2026-07-22 Atlas workflow evidence:** The production-built Atlas Projections and Atlas Replay routes now have real-browser coverage for successful durable projection receipts, replay verification, and their analysis-only/no-governance/no-execution boundaries. The full Windows and digest-pinned Linux suites now pass 38/38, comprising thirteen screenshot comparisons and twenty-five functional checks. No visual baseline was regenerated.
**Superseding 2026-07-22 role-matrix evidence:** The production-built console now also covers a reviewer Inbox with a submitted human-review record and no request-submission composer, plus an administrator account-management surface with role/status controls and server-authorized boundary text. The full Windows and digest-pinned Linux suites now pass 40/40, comprising thirteen screenshot comparisons and twenty-seven functional checks. No visual baseline was regenerated.
**Superseding 2026-07-22 mutation-state evidence:** The highest-authority Sovereign War Room submission control, durable work-queue request/review controls, and account-security mutations now disable during in-flight requests and expose operation-specific pending labels, preventing duplicate human records, reviews, execution-gate submissions, session revocations, and security changes while durable receipts are pending. Operator-console component coverage passes 71/71; aggregate web coverage passes 113/113. The full Windows and digest-pinned Linux browser suites remain green at 40/40, and no visual baseline was regenerated.
**Superseding 2026-07-22 accessibility evidence:** Shared loading, warning, and error panels now expose their visible title as the accessible name of the corresponding `status` or `alert` landmark. The named administration restriction is covered by component and Chromium assertions; manual NVDA/TalkBack remains unverified because no device or emulator is available.
**Superseding 2026-07-22 Android evidence:** The Android read-only result surface retains a polite live region and now uses operation-specific visible loading text, removing deprecated accessibility announcement APIs. Clean debug/release unit, lint, and APK gates pass; manual TalkBack and release signing remain open.
**Superseding 2026-07-22 Android concurrency evidence:** The Android read-only client now serializes the two surface actions at the UI boundary by disabling both buttons while either read is pending, preventing completion-order races from overwriting the shared result surface. The clean debug/release unit, lint, and APK gate passes again; manual TalkBack and release signing remain open.
**Superseding 2026-07-22 Linux visual evidence:** The tracked Linux baselines match the preserved candidate set byte-for-byte (13/13 SHA-256 matches), and the digest-pinned read-only Linux run passes the full 30-test suite: 13 screenshot comparisons plus 17 functional checks. No baseline replacement was performed.
**Superseding 2026-07-22 desktop distribution evidence:** The existing Windows installer bundle passed the complete local smoke path after isolating the Docker API on port 8000: custom-prefix install, Add/Remove Programs visibility, installed desktop launch, bundled API spawn, graceful child termination, and silent uninstall. The API service was restored and the 9-service Compose stack remains healthy. Code signing and manual NVDA acceptance remain open.
**Superseding 2026-07-22 notification accessibility evidence:** The work-notifications popover is now a named dialog linked to its trigger, moves focus to its close control on open, and restores focus on Escape or close. Component and real-browser checks cover the transfer and restoration.
**Recommended next action:** Complete manual NVDA/TalkBack acceptance, then extend durable notification-history, remaining consequential module, and cross-client acceptance before release sign-off. Local Android debug/release lint, unit tests, and APK builds pass with a polite live-region result surface; the desktop offscreen suite passes 28/28.
