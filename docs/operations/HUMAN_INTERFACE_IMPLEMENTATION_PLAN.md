...
**Latest shell/state acceptance:** The former hard-coded green `Live query` indicator has been removed. The shell now polls the liveness endpoint and presents checking, live, stale, unavailable, and browser-offline states without claiming that process liveness proves whole-system health. The Command Center independently elevates partial surface evidence, failed-refresh stale snapshots, and offline retained snapshots with explicit last-observed times. Cross-platform Playwright coverage now contains thirteen reviewed screenshots and five functional checks, including Viewer navigation without Administration, multi-workspace main-content focus transfer, and complete Administration form tab order.
**Recommended next action:** Complete manual NVDA/TalkBack acceptance, then extend role/state coverage through the remaining consequential module workflows and cross-client surfaces.
