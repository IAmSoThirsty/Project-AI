...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('security/vault_audit.log'),
...
class SecurityVault:
    ...
    def authenticate(self, user_id: str, role: str, auth_token: str, 
        ...
        logger.warning(f"VAULT ACCESS GRANTED - User: {user_id}, Session: {session_token[:8]}...")
    ...
    def _log_failed_attempt(self, user_id: str, reason: str):
        ...
        recent = [a for a in self._failed_attempts[user_id] 
                 if time.time() - a['timestamp'] < 600]  # 10 minutes
        
        if len(recent) >= 3:
            logger.critical(f"SECURITY ALERT - Multiple failed attempts from: {user_id}")
