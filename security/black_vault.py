...
MAX_VAULT_SIZE = int(os.environ.get('MAX_VAULT_SIZE', 1 * 1024 * 1024 * 1024))  # 1GB default
...
def check_and_rotate_vault_key(force: bool = False) -> bool:
            ...
            try:
                with open(rotation_file, 'r') as f:
                    ...
                    if datetime.now() < next_rotation:
                        logger.debug(f"Vault key rotation not due until {next_rotation}")
                        return False
            except Exception as e:
    ...
    try:
        ...
        if vault_path.exists():
            ...
            backup_path = backup_dir / f"vault_{datetime.now().strftime('%Y%m%d_%H%M%S')}.bak"
        ...
        with open(rotation_file, 'w') as f:
            json.dump({
                ...
                'next_rotation': (datetime.now() + timedelta(days=ROTATION_INTERVAL_DAYS)).isoformat(),
...
class BlackVault:
    ...
    def __init__(self, vault_store: Optional[str] = None, max_size: Optional[int] = None):
        """
    ...
    def _rotate_vault_if_needed(self):
        ...
        if vault_path.exists() and vault_path.stat().st_size > self.max_size:
            ...
            vault_path.rename(backup_path)
            logger.info(f"Rotated vault to {backup_path} (size: {backup_path.stat().st_size} bytes)")
