...
2. **`bandit-security-scan`** - Bandit Python security scan with SARIF upload
3. **`secret-scanning`** - detect-secrets + TruffleHog secret detection
4. **`dependency-security`** - pip-audit + Safety dependency vulnerability scan
...
2. **`mypy-type-checking`** - Static type checking
3. **`black-formatting`** - Code format validation
4. **`super-linter`** - Multi-language linter (Python, YAML, JSON, Markdown, Bash, Docker)
...
2. **`node-tests`** - Node.js test suite

3. **`cli-tests`** - CLI integration tests

4. **`cerberus-submodule-tests`** - Cerberus submodule validation

5. **`integration-tests`** - Full integration test suite

6. **`e2e-tests`** - End-to-end tests (if e2e/ directory exists)
...
2. **`trivy-image-scan`** - Trivy Docker image security scan
3. **`trivy-config-scan`** - Trivy IaC configuration scan
4. **`checkov-iac-scan`** - Checkov Infrastructure as Code security
...
2. **Daily 2 AM** - Security scans, dependency audits
3. **Daily 3 AM** - Issue triage, auto-fixes
4. **Weekly Sunday 5 AM** - Artifact cleanup, comprehensive audits
5. **Weekly Monday 3 AM** - Nightly security verification
...
2. Set `needs:` to depend on correct jobs
3. Add conditional `if:` based on initialization outputs
4. Set appropriate `timeout-minutes`
5. Add job status to Phase 15 reporting
...
2. **Smart Conditionals**: Jobs skip when not needed (path-based detection)
3. **BuildKit Caching**: Docker builds use GitHub Actions cache
4. **Pip/npm Caching**: Dependencies cached via composite actions
5. **Matrix Strategies**: Tests/builds parallelized across Python versions/platforms
6. **Fail-Fast: false**: Matrix jobs continue even if one fails
7. **Continue-on-error**: Security scans don't block entire workflow
...
3. security-consolidated.yml
4. pr-automation-consolidated.yml
5. issue-management-consolidated.yml
6. coverage-threshold-enforcement.yml
7. adversarial-redteam.yml
8. periodic-security-verification.yml
9. build-release.yml
10. sbom.yml
11. trivy-container-security.yml
12. checkov-cloud-config.yml
13. post-merge-validation.yml
14. prune-artifacts.yml
15. node-ci.yml
16. tarl-ci.yml
17. sign-release-artifacts.yml
18. release.yml
19. validate-guardians.yml
20. validate-waivers.yml
21. ai-model-security.yml
22. auto-create-branch-prs.yml
23. dependabot.yml (automation only)
24. main.yml (if duplicate of ci.yml)
25. codex-deus-monolith.yml (predecessor)
26. snn-mlops-cicd.yml
27. gpt_oss_integration.yml
28. jekyll-gh-pages.yml (if not needed)
