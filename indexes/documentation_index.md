# Documentation Index

> **📍 Location**: `indexes/documentation_index.md`  
> **🎯 Purpose**: Searchable catalog of all project documentation  
> **👥 Audience**: All users  
> **🔄 Status**: Production-Ready ✓

---

## 🔍 Quick Navigation

| Category | Count | Entry Point |
|----------|-------|-------------|
| **MOCs** | 16 | [[docs/00_INDEX.md|Master Index]] |
| **Architecture** | 20+ | [[docs/architecture/00_ARCHITECTURE_MOC.md|Architecture MOC]] |
| **Security** | 30+ | [[docs/security_compliance/00_SECURITY_MOC.md|Security MOC]] |
| **Developer** | 25+ | [[docs/developer/00_DEVELOPER_MOC.md|Developer MOC]] |
| **Operations** | 15+ | [[docs/operations/00_OPERATIONS_MOC.md|Operations MOC]] |
| **Governance** | 12+ | [[relationships/governance/00_GOVERNANCE_MOC.md|Governance MOC]] |
| **Core AI** | 18+ | [[relationships/core-ai/00-INDEX.md|Core AI MOC]] |

---

## 📚 Documentation by Type

### Maps of Content (MOCs)
- [[docs/00_INDEX.md|00_INDEX - Master Navigation Hub]]
- [[relationships/core-ai/00-INDEX.md|Core AI MOC]]
- [[docs/security_compliance/00_SECURITY_MOC.md|Security MOC]]
- [[relationships/governance/00_GOVERNANCE_MOC.md|Governance MOC]]
- [[docs/architecture/00_ARCHITECTURE_MOC.md|Architecture MOC]]
- [[docs/developer/00_DEVELOPER_MOC.md|Developer MOC]]
- [[docs/operations/00_OPERATIONS_MOC.md|Operations MOC]]
- [[relationships/agents/00_AGENTS_MOC.md|Agents MOC]]
- [[relationships/gui/00_MASTER_INDEX.md|GUI MOC]]
- [[relationships/data/00_DATA_MOC.md|Data MOC]]
- [[relationships/temporal/00_TEMPORAL_MOC.md|Temporal MOC]]
- [[relationships/integrations/00_INTEGRATION_MOC.md|Integration MOC]]
- [[relationships/testing/00_TESTING_MOC.md|Testing MOC]]
- [[docs/developer/00_DEPLOYMENT_MOC.md|Deployment MOC]]
- [[relationships/configuration/00_CONFIG_MOC.md|Configuration MOC]]
- [[relationships/monitoring/00_MONITORING_MOC.md|Monitoring MOC]]

### Quick Start Guides
- [[DESKTOP_APP_QUICKSTART.md|Desktop App Quickstart]]
- [[docs/developer/OPERATOR_QUICKSTART.md|Operator Quickstart]]
- [[docs/MCP_QUICKSTART.md|MCP Quickstart]]
- [[DEVELOPER_QUICK_REFERENCE.md|Developer Quick Reference]]
- [[API_QUICK_REFERENCE.md|API Quick Reference]]

### Architecture Documentation
- [[PROGRAM_SUMMARY.md|Program Summary]]
- [[.github/instructions/ARCHITECTURE_QUICK_REF.md|Architecture Quick Reference]]
- [[docs/reports/AGENT-080-CONCEPT-CODE-MAP.md|Concept-Code Traceability (421 links)]]
- [[DESIGN_PATTERN_USAGE_MATRIX.md|Design Pattern Usage Matrix]]

### Security Documentation
- [[SECURITY.md|Security Policy]]
- [[docs/security_compliance/AI_SECURITY_FRAMEWORK.md|AI Security Framework]]
- [[docs/security_compliance/THREAT_MODEL_SECURITY_WORKFLOWS.md|Threat Model]]
- [[docs/security_compliance/INCIDENT_PLAYBOOK.md|Incident Playbook]]

---

## 📊 Documentation Statistics

- **Total Documents**: 500+
- **MOCs**: 16
- **Relationship Maps**: 50+
- **Technical Guides**: 30+
- **API Documentation**: 15+
- **Security Reports**: 25+

---

## 📋 Metadata

```yaml
---
title: "Documentation Index"
type: index
category: documentation
audience: [all]
status: production
version: 1.0.0
created: 2025-01-20
tags:
  - index
  - documentation
  - guides
---
```

---

**Status**: Production-Ready ✓
