...
class MultiPartyApproval:
    ...
    def request_approval(
        ...
        request_id = hashlib.sha256(
            f"{action}:{user_id}:{time.monotonic()}".encode()
        ).hexdigest()[:16]
        ...
        logger.warning("APPROVAL REQUESTED: %s by %s", action, user_id)
    ...
    def approve(self, request_id: str, approver_id: str, approver_role: Role) -> bool:
        ...
        if approver_role not in request.required_approvers:
            logger.warning("Approver role %s not required", approver_role)
            return False
    ...
    def deny(self, request_id: str, denier_id: str) -> bool:
        ...
        logger.warning("Request DENIED: %s", request_id)
...
class RBACEngine:
    ...
    def assign_role(self, user_id: str, role: Role):
        ...
        logger.info("Role assigned: %s to %s", role.value, user_id)
