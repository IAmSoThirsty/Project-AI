...
def test_file_extensions():
    ...
    for ext in extensions:
        ...
        try:
            ...
            print(f"  ✗ .gitattributes not found")
            failed += 1
            break
...
def test_example_files():
    ...
    if len(thirsty_files) > 0:
        ...
        print(f"  ✗ No .thirsty files found")
        return 0, 1
...
def test_vendoring_config():
    ...
    try:
        result = subprocess.run(
            ["git", "check-attr", "linguist-vendored", "src/thirsty_lang/examples/hello.thirsty"],
            capture_output=True,
            text=True,
            check=True
        )
        ...
        if "false" in result.stdout or "unspecified" in result.stdout:
            print(f"  ✓ Examples are not vendored")
            passed += 1
        else:
            print(f"  ✗ Examples incorrectly marked as vendored")
            failed += 1
    ...
    with open(".gitattributes", "r") as f:
        ...
        if "node_modules/**" in content and "linguist-vendored" in content:
            print(f"  ✓ Vendor directories are configured")
            passed += 1
        else:
            print(f"  ✗ Vendor directories not properly configured")
            failed += 1
