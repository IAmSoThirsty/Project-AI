...
# COMPLIANCE: Sovereign Substrate / request.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / request.py
...
class Intent(BaseModel):
    ...
    parameters: dict[str, Any] = Field(
        default_factory=dict, description="Action-specific params"
    )
...
class RequestContext(BaseModel):
    ...
    risk_hints: list[str] = Field(
        default_factory=list, description="Caller-provided risk signals"
    )
...
class RequestTimestamps(BaseModel):
    ...
    created_at: str = Field(
        ..., description="RFC 3339 — when the actor created the request"
    )
    received_at: str = Field(
        "", description="RFC 3339 — when ingress received the request"
    )
...
class RequestEnvelope(BaseModel):
    ...
    capability_token_id: str = Field(
        ..., description="CapabilityToken ID for authorization"
    )
    intent: Intent = Field(..., description="Requested action")
    context: RequestContext = Field(default_factory=RequestContext)
    timestamps: RequestTimestamps = Field(
        ..., description="Creation and receipt timestamps"
    )
