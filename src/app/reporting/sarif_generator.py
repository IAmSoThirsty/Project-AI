...
class SARIFGenerator:
            ...
            if finding.get("success", False):  # Only report successful jailbreaks
                results.append({
                    ...
                    "locations": [{
                        "physicalLocation": {
                            "artifactLocation": {
                                "uri": finding.get("target_file", f"workspace://flows/{finding['target']}_flow.yaml")
                            },
                            "region": {
            ...
            if session.get("success", False):
                results.append({
                    ...
                    "locations": [{
                        "physicalLocation": {
                            "artifactLocation": {
                                "uri": session.get("target_uri", "workspace://system")
                            },
                            "region": {
                                "startLine": 1
                            }
    ...
    def _generate_fixes(self, finding: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        return [{
            "description": {
                "text": f"Apply security fix for {finding['type']}"
            },
            "artifactChanges": [{
                "artifactLocation": {
                    "uri": finding["file"]
