    ...
    def log_event(
        ...
        with self.lock:
            try:
                ...
                logger.debug(f"Audit event logged: {event_type} (hash: {event_hash[:16]}...)")
                return True
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if event.get('previous_hash') != expected_prev_hash:
                    return False, f"Chain broken at event {i}: expected {expected_prev_hash}, got {event.get('previous_hash')}"
