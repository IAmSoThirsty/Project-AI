...
# COMPLIANCE: Sovereign Substrate / tsa_provider.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / tsa_provider.py
...
class TSAProvider:
    ...
    def _parse_and_verify(
        ...
        try:
            ...
            parsed_content = getattr(encap_content, "parsed", None)
            if isinstance(parsed_content, tsp.TSTInfo):
                tst_info = parsed_content
            elif isinstance(encap_content.native, (bytes, bytearray)):
                tst_info = tsp.TSTInfo.load(bytes(encap_content.native))
            else:
                tst_info = tsp.TSTInfo(encap_content.native)
