...
# COMPLIANCE: Sovereign Substrate / kernel_integration.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / kernel_integration.py
...
from app.core.council import CouncilMemberInterface, CouncilRole
...
class KernelRoutedAgent(CouncilMemberInterface):
    """
    ...
    def get_role(self) -> CouncilRole:
        """Default role is EXECUTOR, should be overridden by subclasses."""
        return CouncilRole.EXECUTOR

    def execute_action(self, action_name: str, params: Dict[str, Any]) -> Any:
        """Implementation of CouncilMemberInterface action execution."""
        # This routes back to the internal _execute_through_kernel
        # Subclasses should provide the actual callable maps
        raise NotImplementedError("Subclasses must implement execute_action mapping")

    def receive_message(self, from_id: str, message: str) -> None:
        """Default message handler."""
        logger.info(f"Agent {self.__class__.__name__} received message from {from_id}: {message}")
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            *args: Positional arguments for the action
            action_name: Human-readable action name (keyword only)
            operation_name: Alias for action_name (keyword only)
            action_args: Explicit positional arguments (backward compatibility)
            action_kwargs: Explicit keyword arguments (backward compatibility)
        ...
        """
        # Resolve action name (prefer action_name, fallback to operation_name, then qualname)
        actual_action_name = action_name or operation_name or getattr(action, "__qualname__", str(action))
        
        # Resolve arguments (prefer explicit action_args/kwargs for backward compatibility)
        actual_args = action_args if action_args is not None else args
        actual_kwargs = action_kwargs if action_kwargs is not None else kwargs
        ...
        if self.kernel is None:
            logger.warning("Bypassing kernel for %s (no kernel available)", actual_action_name)
            return action(*actual_args, **actual_kwargs)
        ...
        full_metadata = {
            **metadata,
            ...
            "user_id": user_id,
            # Internal representation for kernel processing
            "_action_callable": action,
            "_action_args": actual_args,
            "_action_kwargs": actual_kwargs,
        }
