"""
Kernel Integration Utilities - Decorators and wrappers for routing through CognitionKernel.

These utilities make it easy to route agent, tool, and system executions through
the CognitionKernel without extensive code changes. They provide decorator patterns
and wrapper classes for seamless integration.
"""
...
_global_kernel: Optional[CognitionKernel] = None
...
def get_global_kernel() -> Optional[CognitionKernel]:
    """
...
class KernelRoutedAgent:
    ...
    Agents that inherit from this class automatically get kernel routing
    for their execute/act/solve methods.
    ...
    def _execute_through_kernel(
        ...
        """
        Execute an action through the CognitionKernel.
        
        This is a helper method for agents to easily route actions through
        the kernel without manually handling ExecutionResult unwrapping.
...
def kernel_safe_wrapper(func: Callable, kernel: Optional[CognitionKernel] = None) -> Callable:
    """
