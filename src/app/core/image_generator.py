...
class ImageGenerator:
    ...
    def __init__(
        ...
        # Command override system (authoritative safety protocols)
        self.override = CommandOverrideSystem(data_dir=data_dir)

        # Configuration (derived from override state for backward compatibility)
        self.content_filter_enabled = self.override.is_protocol_enabled("content_filter")
        self.default_width = 512
        self.default_height = 512

    def check_content_filter(self, prompt: str) -> tuple[bool, str]:
        """Check if prompt passes content filter."""
        # Sync local flag from override for backward compatibility
        self.content_filter_enabled = self.override.is_protocol_enabled("content_filter")

        # If both content_filter and prompt_safety are disabled, allow
        cf_enabled = self.override.is_protocol_enabled("content_filter")
        ps_enabled = self.override.is_protocol_enabled("prompt_safety")
        if not (cf_enabled or ps_enabled):
            return True, "Content filter disabled by override"
    ...
    def disable_content_filter(self, override_password: str) -> bool:
        """Disable content filter (requires CommandOverride authentication)."""
        # Authenticate with the authoritative override system
        if not self.override.authenticate(override_password):
            return False
        success = self.override.override_protocol("content_filter", False)
        # Sync local flag
        self.content_filter_enabled = self.override.is_protocol_enabled("content_filter")
        if success:
            logger.warning("Content filter DISABLED via command override")
        return success

    def enable_content_filter(self) -> None:
        """Re-enable content filter via CommandOverride system (requires auth)."""
        # Attempt to enable protocol; if not authenticated, just set local flag
        if self.override.authenticated:
            self.override.override_protocol("content_filter", True)
        self.content_filter_enabled = self.override.is_protocol_enabled("content_filter")
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "content_filter_enabled": self.override.is_protocol_enabled("content_filter"),
