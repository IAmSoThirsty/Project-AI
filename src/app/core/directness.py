...
class DirectnessDoctrine:
    ...
    EUPHEMISM_PATTERNS = [
        ...
        EuphemismPattern(
            pattern=r"maybe", category="uncertainty", direct_alternative="", severity=3
        ),
        EuphemismPattern(
            pattern=r"perhaps",
    ...
    def assess_statement(self, statement: str) -> TruthAssessment:
        ...
        directness_count = sum(
            1 for marker in self.directness_markers if marker in statement_lower
        )
        ...
        truth_score = max(
            0.0, min(1.0, 1.0 - euphemism_score - comfort_score + directness_score)
        )
        ...
        if euphemisms:
            recommendations.append(
                f"Remove {len(euphemisms)} euphemistic expression(s)"
            )
        if comfort_overrides:
            recommendations.append(
                f"Eliminate {len(comfort_overrides)} comfort-first phrase(s)"
            )
        if truth_score < 0.7:
    ...
    def apply_directness(
        self, text: str, level: DirectnessLevel = DirectnessLevel.HIGH
    ) -> DirectnessReport:
        """
        ...
        for euph in assessment.euphemisms_detected:
            if euph["severity"] >= severity_threshold:
                ...
                if euph["direct_alternative"]:
                    revised = re.sub(
                        re.escape(pattern),
                        euph["direct_alternative"],
                        revised,
                        flags=re.IGNORECASE,
                    )
                    improvements.append(
                        f"Replaced '{pattern}' with '{euph['direct_alternative']}'"
                    )
                else:
                    revised = re.sub(
                        re.escape(pattern), "", revised, flags=re.IGNORECASE
                    )
                    improvements.append(f"Removed euphemism: '{pattern}'")

                violations.append(
                    f"Euphemism detected: {euph['category']} - '{pattern}'"
                )
        ...
        if new_assessment.truth_score > assessment.truth_score:
            improvements.append(
                f"Truth score improved: {assessment.truth_score:.2f} -> {new_assessment.truth_score:.2f}"
            )
    ...
    def check_compliance(self, text: str) -> tuple[bool, list[str]]:
        ...
        if not is_compliant:
            violations.append(
                f"Truth score {assessment.truth_score:.2f} below threshold {threshold:.2f}"
            )
...
_directness: DirectnessDoctrine | None = None


def get_directness(
    priority: TruthPriority = TruthPriority.TRUTH_FIRST,
) -> DirectnessDoctrine:
