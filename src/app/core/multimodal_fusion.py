...
from app.core.voice_models import VoiceModel, VoiceResponse, VoiceEmotionType
from app.core.voice_bonding_protocol import EngagementProfiler, VoiceBondingProtocol
from app.core.visual_cue_models import VisualCueData, EmotionType as VisualEmotionType, FocusLevel
from app.core.visual_bonding_controller import VisualController, VisualEvent, VisualEventData
from app.core.conversation_context_engine import (
    ConversationContextEngine, PolicyManager, Intent, ConversationTurn
)
...
class MultiModalFusionEngine:
    ...
    def _register_visual_events(self) -> None:
        ...
        self.visual_controller.register_event_handler(
            ...
            self._on_visual_emotion_changed
        )
        self.visual_controller.register_event_handler(
        ...
        self.visual_controller.register_event_handler(
            VisualEvent.USER_PRESENT,
            self._on_user_presence_changed
        )
        self.visual_controller.register_event_handler(
            VisualEvent.GAZE_DIRECTED,
            self._on_gaze_changed
        )

    def process_multimodal_input(self, user_id: str, session_id: str,
                                 input_data: MultiModalInput) -> FusedContext:
        """
        ...
        try:
            with self._lock:
                ...
                if input_data.text_input:
                    self._process_text_modality(user_id, session_id, 
                                              input_data.text_input, fused)
            ...
            return FusedContext(user_id=user_id, session_id=session_id,
                              timestamp=input_data.timestamp)

    def _process_text_modality(self, user_id: str, session_id: str,
                              text: str, fused: FusedContext) -> None:
        ...
        try:
            ...
            fused.voice_emotion = voice_analysis.get("recommended_emotion", 
                                                     VoiceEmotionType.NEUTRAL)
    ...
    def _process_visual_modality(self, user_id: str, frame: np.ndarray,
                                fused: FusedContext) -> None:
    ...
    def _apply_hybrid_fusion(self, fused: FusedContext) -> None:
        ...
        combined_emotion = (voice_emotion_val * voice_weight + 
                          visual_emotion_val * visual_weight)
    ...
    def get_current_fusion(self, user_id: str) -> Optional[FusedContext]:
...
# Global instance
_default_fusion_engine: Optional[MultiModalFusionEngine] = None
...
def get_default_fusion_engine() -> MultiModalFusionEngine:
    ...
    if _default_fusion_engine is None:
        ...
        from app.core.conversation_context_engine import (
            get_default_context_engine, get_default_policy_manager
        )
