...
try:
    ...
    from utf.tscg_b.core import TSCGBError as _TSCGBError
    from utf.tscg_b.core import pack_text as _tscg_b_pack
except ImportError:
...
class HumanGapCalculator:
    ...
    def calculate_gap(
        self, last_session_end: float, current_session_start: float
    ) -> tuple[float, str]:
        """
    ...
    def _format_gap_description(self, seconds: float, category: str) -> str:
        ...
        elif category == "brief" or category == "short":
            return f"{int(seconds / 60)} minutes have passed"
        elif category == "moderate" or category == "significant":
            return f"{int(seconds / 3600)} hours have passed"
        elif category == "substantial" or category == "major":
...
class StateRegister:
    ...
    def __init__(self, data_dir: str | None = None):
        ...
        self.data_dir = (
            Path(data_dir)
            if data_dir
            else Path.home() / ".project_ai" / "state_register"
        )
    ...
    def start_session(self, context: dict[str, Any] | None = None) -> SessionMetadata:
        """
        ...
        last_timestamp = (
            previous_timestamp or datetime.fromtimestamp(start_time, tz=UTC).isoformat()
        )
    ...
    def end_session(self, context: dict[str, Any] | None = None) -> SessionMetadata:
        """
    ...
    def get_gap_announcement(self) -> str | None:
        """
        ...
        _, description = self.gap_calculator.calculate_gap(
            self.current_session.start_time - gap, self.current_session.start_time
        )
    ...
    def _compute_continuity_hash(self, session: SessionMetadata) -> str:
        ...
        user_id = (
            session.context.get("user_id", "anonymous")
            if session.context
            else "anonymous"
        )
...
_state_register: StateRegister | None = None


def get_state_register(data_dir: str | None = None) -> StateRegister:
...
def start_session(context: dict | None = None) -> SessionMetadata:
    """Start a new session using default register."""
    return get_state_register().start_session(context)


def end_session(context: dict | None = None) -> SessionMetadata:
