...
def _resolve_jwt_secret() -> str:
    """Resolve JWT secret with strict production requirements.

    In normal runtime, JWT secret is mandatory. During tests, a deterministic
    ...
    if allow_test_fallback:
        ...
        return "project-ai-test-only-jwt-secret"
...
# CRITICAL: JWT secret MUST be set in production environments.
...
_refresh_token_store: dict[
    str, dict
] = {}  # {refresh_token: {username, issued_at, expires_at}}
...
def revoke_all_user_tokens(username: str) -> int:
    ...
    tokens_to_revoke = [
        token
        for token, metadata in _refresh_token_store.items()
...
def get_mfa_backup_codes(username: str, verification_code: str) -> list[str] | None:
    ...
    if verify_mfa_code(username, verification_code):
        ...
        logger.warning(
            f"Failed to retrieve backup codes for user: {username} - invalid code"
        )
        return None


def regenerate_mfa_backup_codes(
    username: str, verification_code: str
) -> list[str] | None:
    """
    ...
    if verify_mfa_code(username, verification_code):
        ...
        logger.warning(
            f"Failed to regenerate backup codes for user: {username} - invalid code"
        )
        return None
