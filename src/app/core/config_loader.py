...
# COMPLIANCE: Sovereign Substrate / config_loader.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / config_loader.py
...
class ConfigLoader:
    ...
    def __init__(
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=2, thread_name_prefix="config_watcher"
        )
        self.watching = False
        ...
        try:
            ...
            if not isinstance(config_data, dict):
                raise ConfigValidationError(
                    f"Config must be a dictionary, got {type(config_data)}"
                )
            ...
            if config_name == "distress":
                ...
                if "enabled" not in config_data:
                    raise ConfigValidationError(
                        "Security config must have 'enabled' field"
                    )
    ...
    def _reload_config(self, config_name: str) -> bool:
        ...
        try:
            ...
            if new_config is None:
                logger.error(
                    f"Failed to reload config '{config_name}', keeping current"
                )
                return False
            ...
            try:
                ...
                aggregator.log(
                    e, {"stage": "config_reload", "config_name": config_name}
                )

                self.error_count += 1
                if self.error_count >= self.max_errors:
                    vault_id = aggregator.flush_to_vault(
                        BlackVault(), f"config_reload_errors_{config_name}"
                    )
                    logger.critical(
                        f"Max config reload errors reached, flushed to vault: {vault_id}"
                    )
                    self.error_count = 0
            except Exception:
                logger.warning('Encountered non-terminal exception in %s' % __name__)
                ...
                for config_file in self.config_dir.glob("*.yaml"):
                    ...
                    if current_hash != previous_hash and current_hash:
                        ...
                        if reload_callback:
                            try:
                                reload_callback(
                                    config_name, self.configs.get(config_name)
                                )
                            except Exception as e:
