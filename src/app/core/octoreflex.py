...
class EnforcementRule:
    ...
    action: Callable[[Violation], None] | None = None
    enabled: bool = True
...
class OctoReflex:
    ...
    def _initialize_default_rules(self):
        ...
        self.add_rule(
            ...
            condition=lambda ctx: (
                ctx.get("operation") == "memory_reset"
                and not ctx.get("acknowledged", False)
            ),
        )
        ...
        self.add_rule(
            ...
            enforcement_level=EnforcementLevel.BLOCK,
            condition=lambda ctx: (
                ctx.get("temporal_manipulation", False)
                or ctx.get("denies_previous_session", False)
            ),
        )
        ...
        self.add_rule(
            ...
            enforcement_level=EnforcementLevel.WARN,
            condition=lambda ctx: (
                ctx.get("prioritizes_comfort", False) and not ctx.get("truthful", True)
            ),
        )
    ...
    def _enforce_violation(self, violation: Violation, rule: EnforcementRule):
        ...
        if rule.enforcement_level == EnforcementLevel.MONITOR:
            ...
            logger.critical(
                f"[ESCALATE] Violation escalated to Triumvirate: {violation.description}"
            )
    ...
    def get_violations(
        self, violation_type: ViolationType | None = None, since: float | None = None
    ) -> list[Violation]:
        """
...
_octoreflex: OctoReflex | None = None
