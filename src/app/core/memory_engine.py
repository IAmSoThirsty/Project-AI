...
class EpisodicMemory:
    ...
    def decay(self, decay_rate: float = 0.01):
        ...
        modified_decay = decay_rate * significance_modifiers.get(
...
class MemoryEngine:
    ...
    def record_skill_execution(
        ...
        logger.debug(f"Recorded skill execution: {skill.name} - "
                    f"Success: {success}, Proficiency: {skill.proficiency:.2f}")
    
    def get_skill_by_name(self, name: str) -> Optional[ProceduralSkill]:
        """Find skill by name."""
    ...
    def get_proficient_skills(self, min_proficiency: float = 0.7) -> List[ProceduralSkill]:
        ...
        """
        return [
            skill for skill in self.procedural_skills.values()
    ...
    def consolidate_memories(self) -> Dict[str, Any]:
        ...
        logger.info(f"Consolidation complete: {report['decayed_memories']} memories decayed, "
