    ...
    def release(self, node_id: str) -> bool:
        ...
        with self._lock:
            if self.holder != node_id:
                logger.warning(f"Node {node_id} tried to release lock {self.lock_name} held by {self.holder}")
                return False
...
class FederatedRegistry:
    ...
    def register_service(self, node_id: str, service_name: str, 
...
class ClusterCoordinator:
            ...
            for node_id, node_info in self._nodes.items():
                ...
                if time_since_heartbeat > self.node_timeout:
                    if node_info.state != NodeState.OFFLINE:
                        logger.warning(f"Node {node_id} timed out (no heartbeat for {time_since_heartbeat:.1f}s)")
    ...
    def _trigger_election(self) -> None:
        ...
        with self._lock:
            logger.info(f"Node {self.node_id} triggering election (term {self._election_term + 1})")
    ...
    def _become_leader(self) -> None:
        ...
        with self._lock:
            logger.info(f"Node {self.node_id} became LEADER for term {self._election_term}")
            self.role = NodeRole.LEADER
            self._leader_id = self.node_id
            self._last_leader_contact = time.time()
            self._trigger_event("leader_elected", {"leader_id": self.node_id, "term": self._election_term})
    ...
    def _assign_task(self, task: ClusterTask) -> None:
        ...
        if active_nodes:
            ...
            assigned_node = min(active_nodes, 
                              key=lambda n: node_task_counts[n.node_id])
    ...
    def get_cluster_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "active_nodes": len([n for n in self._nodes.values() 
                                    if n.state not in [NodeState.OFFLINE, NodeState.MAINTENANCE]]),
...
def create_cluster_coordinator(node_id: Optional[str] = None,
                               bind_port: int = 7777) -> ClusterCoordinator:
    """
...
_default_coordinator: Optional[ClusterCoordinator] = None
