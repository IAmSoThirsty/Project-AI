...
# COMPLIANCE: Sovereign Substrate / cognition_kernel.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / cognition_kernel.py
...
class CognitionKernel:
    ...
    def commit(self, context: ExecutionContext) -> None:
        ...
        if self.memory_engine:
            try:
                ...
                # SOVEREIGN INTEGRATION: Log to immutable audit trail via UTF Bridge
                try:
                    bridge = get_utf_bridge()
                    bridge.log_sovereign_event(
                        event_type="KERNEL_COMMIT",
                        data={
                            "trace_id": context.trace_id,
                            "action": context.proposed_action.action_name,
                            "status": context.status.value
                        },
                        severity="INFO" if context.status == ExecutionStatus.COMPLETED else "WARNING"
                    )
                    logger.info("[%s] Sovereign audit commit successful", context.trace_id)
                except Exception as se:
                    logger.error("[%s] Sovereign audit commit FAILED: %s", context.trace_id, se)
    ...
    def _check_governance(
        self, action: Action, context: ExecutionContext, identity_snapshot: Any
    ) -> Decision:
        """
        Check governance approval for the action via T.A.R.L. Master.
        DELEGATED: Governed by kernel.tarl
        """
        from src.app.core.utf_bridge import get_utf_bridge
        bridge = get_utf_bridge()
        ...
        # Prepare data for T.A.R.L. Master
        action_data = {
            "action_name": action.action_name,
            "risk_level": action.risk_level,
            "mutation_intent": mutation_intent.value
        }
        ...
        # Logic Master Execution
        result = bridge.execute_thirsty_file(
            "src/app/core/kernel.tarl",
            {
                "bridge_action": "evaluate_governance",
                "bridge_action_data": action_data,
                "bridge_context": bridge_context,
                "bridge_identity": {"self_awareness": getattr(identity_snapshot, "self_awareness_score", 0.6)}
            }
        )
        ...
        # Fallback for safety
        logger.error("Sovereign Master Logic Failure: Falling back to local governance.")
        return Decision(
            decision_id=decision_id,
            action_id=action.action_id,
            approved=False,
            reason="Sovereign Logic Master unreachable. Execution blocked for safety.",
            mutation_intent=mutation_intent
        )
