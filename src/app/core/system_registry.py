...
class SystemRegistry:
    ...
    def __init__(self, data_dir: str = "data", config_dir: str = "config"):
        ...
        logger.info(f"System Registry initialized (data_dir={data_dir}, config_dir={config_dir})")
    ...
    def register_subsystem(
        ...
        with self._lock:
            if subsystem_id in self._subsystems:
                logger.warning(f"Subsystem {subsystem_id} already registered, updating...")
            ...
            logger.info(f"Registered subsystem: {name} ({subsystem_id}) v{version} [Priority: {priority.value}]")
            return True
        ...
        with self._lock:
            ...
            for dep_id in subsystem.dependencies:
                if dep_id not in self._subsystems:
                    logger.error(f"Missing dependency {dep_id} for subsystem {subsystem_id}")
                    return False
                
                dep_subsystem = self._subsystems[dep_id]
                if dep_subsystem.state not in [SubsystemState.ACTIVE, SubsystemState.DEGRADED]:
                    logger.error(f"Dependency {dep_id} not active for subsystem {subsystem_id}")
                    return False
    ...
    def get_subsystem(self, subsystem_id: str) -> Optional[Any]:
        """
        ...
        with self._lock:
            ...
            try:
                if subsystem.health_check_fn:
                    ...
                    health_status = subsystem.state in [SubsystemState.ACTIVE, SubsystemState.DEGRADED]
                ...
                if not health_status:
                    ...
                    if subsystem.failure_count >= self._max_failure_threshold:
                        subsystem.state = SubsystemState.FAILED
                        logger.error(f"Subsystem {subsystem_id} marked as FAILED after {subsystem.failure_count} failures")
                else:
                ...
                return HealthCheckResult(
                    ...
                    details={"state": subsystem.state.value, "failure_count": subsystem.failure_count}
                )
            ...
            try:
                ...
                for subsystem_id in subsystem_ids:
                    ...
                    if not result.healthy:
                        logger.warning(f"Health check failed for {subsystem_id}: {result.error}")
