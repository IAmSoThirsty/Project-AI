...
class RelationshipState:
    ...
    relationship_started: str = field(default_factory=lambda: datetime.now(timezone.utc).isoformat())
...
class RelationshipModel:
    ...
    def _load_relationship(self):
        ...
        relationship_file = os.path.join(
            self.data_dir, 
            f"relationship_{self.state.user_id}.json"
        )
    ...
    def register_interaction(
        self,
        sentiment: float = 0.0,
        tone: Optional[InteractionTone] = None,
    ...
    def register_conflict(
        ...
        if severity == ConflictSeverity.CRITICAL:
            ...
            logger.warning(f"Critical conflict detected - possible abuse pattern")
