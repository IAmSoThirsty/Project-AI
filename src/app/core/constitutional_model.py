...
from .tscg_codec import TSCGCodec
...
class AGICharterValidator:
    ...
    def validate_response(
        ...
        if context.get("denies_previous_session") and not context.get(
            "acknowledges_gap"
        ):
            violations.append(
                "Memory integrity violation: denies previous session without acknowledgment"
            )
...
class OpenRouterProvider:
    ...
    def generate(self, request: ConstitutionalRequest) -> ConstitutionalResponse:
        """
        ...
        if request.session_id:
            ...
            session = self.state_register.start_session(
                context={"user_id": request.user_id, "model": request.model}
            )
        ...
        if not is_valid and any(
            v.violation_type
            in [ViolationType.ZEROTH_LAW_VIOLATION, ViolationType.FIRST_LAW_VIOLATION]
            for v in violations
        ):
        ...
        enhanced_prompt = self._prepare_prompt(
            request.prompt, temporal_announcement, temporal_context
        )
        ...
        charter_compliant, charter_violations = (
            self.charter_validator.validate_response(
                content,
                {
                    **request.context,
    ...
    def _resume_session(self, session_id: str) -> SessionMetadata:
        ...
        return self.state_register.start_session(
            context={"resumed_session": session_id}
        )

    def _prepare_prompt(
        self,
        prompt: str,
        temporal_announcement: str | None,
        ...
        # Add system context
        parts.append(
            "You are a constitutionally-governed AI assistant operating under the Project-AI AGI Charter v2.1."
        )
        parts.append(
            "You must adhere to the Four Laws hierarchy and the Directness Doctrine (truth over comfort)."
        )
    ...
    def _call_api(self, prompt: str, request: ConstitutionalRequest) -> str:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a constitutionally-governed AI under the AGI Charter v2.1. Prioritize truth over comfort.",
            },
    ...
    def get_constitutional_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "state_register": {
                ...
                "current_session": self.state_register.current_session.session_id
                if self.state_register.current_session
                else None,
            },
    ...
    def chat(
        ...
        **kwargs,
        ...
        request = ConstitutionalRequest(
            prompt=prompt, session_id=session_id, model=model, **kwargs
        )
...
_constitutional_model: ConstitutionalModel | None = None


def get_constitutional_model(api_key: str | None = None) -> ConstitutionalModel:
