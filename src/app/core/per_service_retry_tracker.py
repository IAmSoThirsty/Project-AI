    ...
    def can_retry(self, service_name: str) -> tuple[bool, str]:
        ...
        with self.lock:
            ...
            if self.global_counter >= self.global_limit:
                ...
                return False, f"Global retry limit exceeded ({self.global_counter}/{self.global_limit})"
            ...
            if service_name in self.service_configs:
                ...
                if service_count >= config.max_per_minute:
                    self.stats['throttled_count'] += 1
                    return False, f"Service '{service_name}' retry limit exceeded ({service_count}/{config.max_per_minute})"
    ...
    def get_global_stats(self) -> Dict:
        ...
        with self.lock:
            return {
                ...
                'active_services': len([s for s, c in self.service_counters.items() if c > 0]),
                'open_circuit_breakers': len([s for s, state in self.circuit_breaker_states.items() if state == 'OPEN']),
...
def get_retry_tracker() -> PerServiceRetryTracker:
    ...
    if _global_retry_tracker is None:
        ...
        _global_retry_tracker.register_service('vault_access', ServiceRetryConfig(
            max_retries=3,
            max_per_minute=20,
            timeout_seconds=10,
        ...
        _global_retry_tracker.register_service('pii_redaction', ServiceRetryConfig(
            max_retries=2,
            max_per_minute=100,
            timeout_seconds=5,
        ...
        _global_retry_tracker.register_service('signal_processing', ServiceRetryConfig(
            max_retries=5,
            max_per_minute=200,
            timeout_seconds=30,
            backoff_multiplier=2.0,
        ...
        _global_retry_tracker.register_service('transcription', ServiceRetryConfig(
            max_retries=3,
            max_per_minute=10,
            timeout_seconds=60,
        ...
        _global_retry_tracker.register_service('audit_logging', ServiceRetryConfig(
            max_retries=10,
            max_per_minute=500,
            timeout_seconds=60,
            backoff_multiplier=1.5,
