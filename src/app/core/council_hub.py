...
# COMPLIANCE: Sovereign Substrate / council_hub.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / council_hub.py
...
from src.app.core.council import CouncilInterface, CouncilMemberInterface
...
class CouncilHub(CouncilInterface):
    ...
    def register_member(self, member: CouncilMemberInterface) -> None:
        """Requirement for CouncilInterface - wraps register_agent."""
        # Use child class name or ID if available
        agent_id = getattr(member, "agent_id", member.__class__.__name__)
        self.register_agent(agent_id, member)

    def route_transmission(self, from_id: str, to_id: str, transmission: str) -> bool:
        """Requirement for CouncilInterface - wraps route_message."""
        result = self.route_message(from_id, to_id, transmission)
        return result.get("delivered", False)

    def enforce_quorum(self, proposal: Any) -> bool:
        """Requirement for CouncilInterface - stub for Triumvirate check."""
        # This will be integrated with the actual Triumvirate evaluation later
        logger.info(f"Enforcing quorum for proposal: {proposal}")
        return True
    ...
    def route_message(self, from_id: str, to_id: str, message: str) -> dict[str, Any]:
        ...
        # SOVEREIGN INTEGRATION: Enforce Security Constitution via UTF Bridge (Council Master)
        try:
            bridge = get_utf_bridge()
            result = bridge.execute_thirsty_file(
                "src/app/core/council.thirsty",
                {
                    "bridge_action": "route_message",
                    "bridge_from_id": from_id,
                    "bridge_to_id": to_id,
                    "bridge_message_len": len(message)
                }
            )
            
            if result and "enforcement_result" in result:
                enforcement = result["enforcement_result"]
                if not enforcement.get("allowed", True):
                    logger.warning(
                        "Sovereign Council blocked message from %s to %s: %s",
                        from_id, to_id, enforcement.get("reason")
                    )
                    return {
                        "delivered": False,
                        "reason": "constitutional_violation",
                        "detail": enforcement.get("reason")
                    }
        except Exception as se:
            logger.error("Sovereign Council check FAILED: %s", se)
