...
# COMPLIANCE: Sovereign Substrate / dependency_checker_robust.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / dependency_checker_robust.py
...
def check_dependency(
    ...
    if constraint.exact_version:
        if compare_versions(installed_version, constraint.exact_version) == 0:
            return (
                True,
                f"Module '{module_name}' version matches exactly: {installed_version}",
                installed_version,
            )
        else:
            return (
                False,
                f"Module '{module_name}' version mismatch: {installed_version} != {constraint.exact_version}",
                installed_version,
            )
    ...
    if constraint.min_version:
        if compare_versions(installed_version, constraint.min_version) < 0:
            return (
                False,
                f"Module '{module_name}' version too old: {installed_version} < {constraint.min_version}",
                installed_version,
            )
    ...
    if constraint.max_version:
        if compare_versions(installed_version, constraint.max_version) > 0:
            return (
                False,
                f"Module '{module_name}' version too new: {installed_version} > {constraint.max_version}",
                installed_version,
            )
    ...
    return (
        True,
        f"Module '{module_name}' version OK: {installed_version}",
        installed_version,
    )
...
def check_python_version(constraint: VersionConstraint) -> Tuple[bool, str]:
    ...
    if constraint.exact_version:
        if compare_versions(python_version, constraint.exact_version) == 0:
            ...
            return (
                False,
                f"Python version mismatch: {python_version} != {constraint.exact_version}",
            )
    ...
    if constraint.min_version:
        if compare_versions(python_version, constraint.min_version) < 0:
            return (
                False,
                f"Python version too old: {python_version} < {constraint.min_version}",
            )
    ...
    if constraint.max_version:
        if compare_versions(python_version, constraint.max_version) > 0:
            return (
                False,
                f"Python version too new: {python_version} > {constraint.max_version}",
            )
...
if __name__ == "__main__":
    ...
    required = {
        "python": VersionConstraint(
            min_version="3.11.0", max_version="3.12.999", required=True
        ),
        "pydantic": VersionConstraint(min_version="2.0.0", required=True),
    }

    optional = {
        "cv2": VersionConstraint(
            min_version="4.8.0", feature_flag="enable_video", required=False
        ),
        "whisper": VersionConstraint(
            min_version="1.0.0", feature_flag="enable_transcript", required=False
        ),
    }
