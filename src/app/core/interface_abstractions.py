...
class ISubsystem(ABC):
    ...
    def set_operational_mode(self, mode: OperationalMode) -> bool:
        ...
        """
        logger.info(f"Setting operational mode to {mode.value} for {self.__class__.__name__}")
        return True
...
class BaseSubsystem(ISubsystem):
    ...
    def __init__(self, data_dir: str = "data", config: Dict[str, Any] = None):
        ...
        self.context = SubsystemContext(
            subsystem_id=self.SUBSYSTEM_METADATA['id']
        )
...
def create_subsystem_proxy(subsystem_id: str, registry) -> Optional[Any]:
    """
