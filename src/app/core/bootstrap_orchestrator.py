...
class BootstrapOrchestrator:
    ...
    def __init__(
        ...
        self.bootstrap_start_time: Optional[datetime] = None
    ...
    def _load_config(self):
        ...
        try:
            ...
            logger.info(f"Discovered {len(self.subsystem_definitions)} subsystem definitions")
    ...
    def _parse_subsystem_definition(self, subsystem_id: str, config: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Invalid subsystem definition for {subsystem_id}: missing {e}")
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        if hasattr(obj, 'SUBSYSTEM_METADATA'):
                            ...
                            if subsystem_id not in self.subsystem_definitions:
                                ...
                                logger.info(f"Discovered subsystem: {name} ({subsystem_id})")
    ...
    def bootstrap(self, auto_start_monitoring: bool = True) -> bool:
        ...
        try:
            ...
            logger.info(f"Initialization order determined: {len(initialization_order)} subsystems")
            ...
            for subsystem_id in initialization_order:
                ...
                if not definition.enabled or not definition.auto_init:
                    logger.info(f"Skipping {subsystem_id} (enabled={definition.enabled}, auto_init={definition.auto_init})")
                    continue
                ...
                if self.registry.initialize_subsystem(subsystem_id):
                    ...
                    if failure_mode == 'stop':
                        logger.error(f"Failure mode is 'stop', aborting bootstrap")
                        return False
                    elif failure_mode == 'rollback':
                        logger.error(f"Failure mode is 'rollback', rolling back bootstrap")
                        self._rollback_bootstrap()
                        return False
                    else:
                        logger.warning(f"Failed to initialize {subsystem_id}, continuing...")
