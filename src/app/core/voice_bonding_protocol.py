...
from app.core.voice_models import (
    VoiceModel, VoiceModelRegistry, VoiceEmotionType,
    VoiceResponse, VoiceModelType
)
...
class BondingScore:
    ...
    def calculate_overall_score(self) -> float:
        ...
        awareness_component = (
            self.emotional_match_score * 0.1 + 
            self.context_awareness_score * 0.1
        )
...
class UserEngagementProfile:
    ...
    def update_from_expression(self, expression_type: UserExpressionType,
...
class EngagementProfiler:
    ...
    def _compile_swear_patterns(self) -> List[re.Pattern]:
        ...
        swear_words = [
            r'\bf[u\*]ck', r'\bsh[i\*]t', r'\bd[a\*]mn', r'\bb[i\*]tch',
            r'\ba[s\*]{2}', r'\bcr[a\*]p', r'\bhe[l\*]{2}', r'\bb[a\*]stard'
        ]
    ...
    def _load_sensitive_topics(self) -> Set[str]:
        ...
        # Production system would load from configuration
        return {
            "death", "dying", "suicide", "violence", "abuse", "trauma",
            "politics", "religion", "race", "sexuality", "medical", "finance"
        }
    ...
    def analyze_user_input(self, user_id: str, text: str,
        ...
        # Detect swearing
        swear_count = sum(1 for pattern in self._swear_patterns 
        ...
        if swear_count > 0:
            ...
            profile.update_from_expression(
                UserExpressionType.SWEARING, text, context
            )
        ...
        detected_topics = [topic for topic in self._sensitive_topics 
    ...
    def get_adaptive_response_params(self, user_id: str,
    ...
    def load_profile(self, user_id: str) -> Optional[UserEngagementProfile]:
...
class VoiceBondingProtocol:
    ...
    def __init__(self, registry: VoiceModelRegistry,
    ...
    def experiment_with_model(self, user_id: str, model_id: str,
                            text: str, context: Optional[Dict[str, Any]] = None) -> Optional[VoiceResponse]:
        """
            ...
            if user_id in self._scores and model_id in self._scores[user_id]:
                ...
                score.avg_response_time_ms = (
                    (score.avg_response_time_ms * (score.total_interactions - 1) + 
                     response_time_ms) / score.total_interactions
                )
                
                # Context awareness score based on how well emotion matched
                if params["emotion"] == response.emotion:
                    score.context_awareness_score = min(1.0, 
                        score.context_awareness_score + 0.1
                    )
    ...
    def provide_feedback(self, user_id: str, model_id: str,
                        feedback: str) -> bool:
        """
        ...
        try:
            with self._lock:
                ...
                if feedback == "positive":
                    ...
                    score.user_preference_score = min(1.0, 
                        score.user_preference_score + 0.1
                    )
                elif feedback == "negative":
                    score.negative_feedback += 1
                    score.user_preference_score = max(0.0, 
                        score.user_preference_score - 0.1
                    )
                else:
    ...
    def evaluate_and_select(self, user_id: str) -> Optional[str]:
        ...
        try:
            with self._lock:
                ...
                logger.info(f"Selected model {best_model_id} for user {user_id} "
    ...
    def get_selected_model(self, user_id: str) -> Optional[VoiceModel]:
...
# Global instances
_default_profiler: Optional[EngagementProfiler] = None
_default_bonding_protocol: Optional[VoiceBondingProtocol] = None
...
def get_default_bonding_protocol(registry: Optional[VoiceModelRegistry] = None,
                                 profiler: Optional[EngagementProfiler] = None) -> VoiceBondingProtocol:
