...
class HardwareRegistry:
    ...
    def register_device(self, device: HardwareDevice) -> bool:
        ...
        with self._lock:
            ...
            logger.info(f"Registered device '{device.device_name}' ({device.device_id})")
            return True
    ...
    def update_device_status(self, device_id: str, status: HardwareStatus) -> bool:
        ...
        with self._lock:
            ...
            logger.info(f"Device '{device.device_name}' status: {old_status.value} -> {status.value}")
            return True
    ...
    def get_events(self, limit: Optional[int] = None) -> List[DiscoveryEvent]:
...
class HardwareAutoDiscoverySystem:
    ...
    def negotiate_capabilities(self, device_id: str, 
        ...
        if has_all:
            logger.info(f"Device '{device.device_name}' meets all capability requirements")
        else:
            missing = [req for req in required_capabilities if req not in device_capabilities]
            logger.warning(f"Device '{device.device_name}' missing capabilities: {missing}")
...
def create_auto_discovery_system(system_id: str = "auto_discovery",
                                 scan_interval: float = 5.0,
                                 data_dir: str = "data/hardware") -> HardwareAutoDiscoverySystem:
    """
