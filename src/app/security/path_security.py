...
def safe_path_join(base_dir: str, *user_paths: str) -> str:
    ...
    try:
        ...
        if common != base_abs:
            logger.warning(
                "Path traversal blocked: %s escapes base %s", user_paths, base_dir
            )
    ...
    for user_path in user_paths:
        if ".." in str(user_path):
            logger.warning("Path traversal blocked: '..' sequence in %s", user_path)
            raise PathTraversalError("Invalid path: '..' sequences not allowed")

    # Block absolute paths in user input (Unix and Windows)
    for user_path in user_paths:
        user_str = str(user_path)
        if user_str.startswith("/") or user_str.startswith("\\"):
            logger.warning("Path traversal blocked: absolute path %s", user_path)
            raise PathTraversalError("Invalid path: absolute paths not allowed")
        # Block Windows drive letters (C:, D:, etc.)
        if len(user_str) >= 2 and user_str[1] == ":":
            logger.warning("Path traversal blocked: drive letter in %s", user_path)
            raise PathTraversalError("Invalid path: drive letters not allowed")
...
def validate_filename(filename: str, max_length: int = 255) -> bool:
    ...
    if "/" in filename or "\\" in filename:
        raise PathTraversalError("Filename cannot contain path separators")

    # Block .. sequences
    if ".." in filename:
        raise PathTraversalError("Filename cannot contain '..' sequences")

    # Block hidden files on Unix
    if filename.startswith("."):
        raise PathTraversalError("Hidden filenames not allowed")

    # Block null bytes
    if "\x00" in filename:
        raise PathTraversalError("Filename cannot contain null bytes")

    # Check length
    if len(filename) > max_length:
        raise PathTraversalError(f"Filename too long (max {max_length} characters)")
