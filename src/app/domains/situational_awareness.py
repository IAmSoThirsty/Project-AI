...
class SituationalAwarenessSubsystem(BaseSubsystem, ICommandable, IMonitorable, IObservable, ISensorFusion, IThreatDetection):
    """
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to initialize Situational Awareness subsystem: {e}")
            return False
        ...
        with self._fused_state_lock:
            ...
            if last_update:
                ...
                if age > timedelta(seconds=self.sensor_fusion_interval * 10):
                    self.logger.warning(f"Fused state is stale (age={age.total_seconds()}s)")
                    return False
    ...
    def _track_threat(self, threat_dict: Dict[str, Any], sensor_data: SensorData):
        ...
        with self._threat_lock:
            ...
            if contact_id in self._threat_contacts:
                ...
                self.emit_event("threat_detected", {
                    "contact_id": contact_id,
