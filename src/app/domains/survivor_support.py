...
class SurvivorSupportSubsystem(BaseSubsystem, ICommandable, IMonitorable, IObservable):
    ...
    def execute_command(self, command: SubsystemCommand) -> SubsystemResponse:
        ...
        try:
            if command.command_type == "register_survivor":
                ...
                return SubsystemResponse(command.command_id, survivor is not None, 
    ...
    def unsubscribe(self, subscription_id: str) -> bool:
        with self._subscription_lock:
            for event_type in self._subscriptions:
                self._subscriptions[event_type] = [(sid, cb) for sid, cb in self._subscriptions[event_type] if sid != subscription_id]
            return True
    ...
    def _register_survivor(self, params: Dict[str, Any]) -> Optional[Survivor]:
        try:
            survivor = Survivor(str(uuid.uuid4()), params["name"], SurvivorStatus[params.get("status", "AT_RISK")])
            with self._lock:
