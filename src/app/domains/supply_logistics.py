...
class SupplyLogisticsSubsystem(BaseSubsystem, ICommandable, IMonitorable, IObservable, IResourceManager):
    ...
    def health_check(self) -> bool:
        ...
        critical_count = sum(
            1 for status in self._get_all_supply_statuses().values()
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status["supply_statuses"] = {
            rt.value: self._get_supply_status(rt).value
    ...
    def allocate_resource(self, resource_type: str, amount: float) -> Optional[str]:
        """Allocate a resource."""
        try:
            ...
            self.emit_event("resource_allocated", {
    ...
    def unsubscribe(self, subscription_id: str) -> bool:
        ...
        with self._subscription_lock:
            for event_type in self._subscriptions:
                self._subscriptions[event_type] = [
                    (sid, cb) for sid, cb in self._subscriptions[event_type]
            ...
            for item in self._inventory.values():
                if item.expiration_date and not item.reserved:
                    ...
                    if time_to_expiry < timedelta(days=0):
                        ...
                        self.emit_event("resource_expired", {
                        ...
                        self.emit_event("resource_expiring_soon", {
                            "item_id": item.item_id,
    ...
    def _add_resource(self, params: Dict[str, Any]) -> Optional[ResourceItem]:
        ...
        try:
            item = ResourceItem(
                ...
                expiration_date=datetime.fromisoformat(params["expiration_date"]) if "expiration_date" in params else None,
            ...
            self.emit_event("resource_added", {
                "item_id": item.item_id,
                "resource_type": item.resource_type.value,
    ...
    def _create_supply_request(self, params: Dict[str, Any]) -> Optional[SupplyRequest]:
        ...
        try:
            ...
            elif available >= request.quantity_requested * 2:
                request.approved = True
            ...
            self.emit_event("supply_request_created", {
    ...
    def _create_distribution_route(self, params: Dict[str, Any]) -> Optional[DistributionRoute]:
    ...
    def _find_available_resources(self, resource_type: ResourceType, amount: float) -> List[str]:
        ...
        with self._inventory_lock:
            for item_id, item in self._inventory.items():
                if (item.resource_type == resource_type and 
    ...
    def _initialize_rationing_rules(self):
        ...
        self._rationing_rules = {
            ...
            ResourceType.FOOD: {"daily_per_person": 2000.0, "critical_reserve": 10000.0},  # calories
