...
# COMPLIANCE: Sovereign Substrate / signal_flows.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / signal_flows.py
...
threading.Thread(
    target=reset_retry_tracker, daemon=True, name="retry_tracker_reset"
).start()
            ...
            if self.state == "OPEN":
                if self.last_failure_time:
                    ...
                    if elapsed >= self.recovery_timeout:
                        ...
                        raise Exception(
                            f"Circuit breaker '{self.name}' OPEN (retry after {self.recovery_timeout - elapsed:.0f}s)"
                        )
                else:
...
circuit_breakers = {
    "validation": CircuitBreaker(
        "validation", failure_threshold=10, recovery_timeout=30
    ),
    "transcription": CircuitBreaker(
        "transcription", failure_threshold=5, recovery_timeout=60
    ),
    "processing": CircuitBreaker(
        "processing", failure_threshold=5, recovery_timeout=45
    ),
}
...
def redact_ip(text: str) -> str:
    ...
    # IPv6 full format
    text = re.sub(
        r"\b(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}\b", "[REDACTED-IP6]", text
    )
    # IPv6 compressed format (with ::)
    text = re.sub(
        r"\b(?:[0-9a-fA-F]{1,4}:){0,7}:(?:[0-9a-fA-F]{1,4}:){0,7}[0-9a-fA-F]{1,4}\b",
        "[REDACTED-IP6]",
        text,
    )
    # IPv6 localhost
...
ENABLED_REDACTORS = os.environ.get(
...
def validate_signal(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not result.is_valid:
        ...
        if result.blocked_phrases:
            logger.warning(
                f"Blocked phrases detected: {', '.join(result.blocked_phrases[:3])}"
            )
