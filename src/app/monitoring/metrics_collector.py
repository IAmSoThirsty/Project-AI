...
# COMPLIANCE: Sovereign Substrate / metrics_collector.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / metrics_collector.py
...
class MetricsCollector:
    ...
    def record_four_laws_override(self, success: bool, user: str = "unknown") -> None:
        ...
        metrics.four_laws_overrides_total.labels(result=result, user=user).inc()
    ...
    def record_memory_error(self, error_type: str) -> None:
        ...
        """
        metrics.memory_query_errors_total.labels(error_type=error_type).inc()
    ...
    def record_black_vault_addition(self, reason: str = "denied") -> None:
        ...
        """
        metrics.learning_black_vault_additions_total.labels(reason=reason).inc()
    ...
    def record_cerberus_block(self, attack_type: str, gate: str = "unknown") -> None:
        ...
        """
        metrics.cerberus_blocks_total.labels(attack_type=attack_type, gate=gate).inc()
    ...
    def record_threat_score(self, threat_type: str, score: float) -> None:
        ...
        """
        metrics.threat_detection_score.labels(threat_type=threat_type).set(score)
        ...
        try:
            ...
            if cerberus_file.exists():
                ...
                for source, count in attack_counts.items():
                    ...
                    last_count = self._last_update.get(key, 0)
                    if count > last_count:
                        self.record_cerberus_block(attack_type="unknown", gate=source)
                        self._last_update[key] = count
