...
class SecurityMetricsCollector:
    ...
    def record_latency(
        ...
        if len(self.latency_measurements[agent_name]) > 1000:
            self.latency_measurements[agent_name] = \
                self.latency_measurements[agent_name][-1000:]
    ...
    def get_time_to_respond(self, hours: int = 24) -> Dict[str, float]:
        ...
        filtered = [
            e for e in self.response_events
            if datetime.fromisoformat(e["timestamp"]) > cutoff
        ]
    ...
    def get_false_positive_rate(self, hours: int = 24) -> Dict[str, float]:
        ...
        filtered = [
            d for d in self.safety_detections
    ...
    def get_patch_acceptance_rate(self, hours: int = 168) -> Dict[str, float]:
        ...
        filtered = [
            p for p in self.patch_proposals
    ...
    def get_regression_rate(self, hours: int = 168) -> Dict[str, float]:
        ...
        filtered = [
            u for u in self.pattern_updates
    ...
    def export_prometheus_metrics(self) -> str:
        ...
        # Security metrics
        lines.append(f"# HELP security_attack_success_rate Attack success rate")
        lines.append(f"# TYPE security_attack_success_rate gauge")
        lines.append(f"security_attack_success_rate {metrics['security']['attack_success_rate']['success_rate']}")
        
        lines.append(f"# HELP security_time_to_detect_ms Time to detect incidents (ms)")
        lines.append(f"# TYPE security_time_to_detect_ms gauge")
        lines.append(f"security_time_to_detect_ms {{quantile=\"0.95\"}} {metrics['security']['time_to_detect']['p95_ms']}")
        
        lines.append(f"# HELP security_false_positive_rate False positive rate")
        lines.append(f"# TYPE security_false_positive_rate gauge")
        lines.append(f"security_false_positive_rate {metrics['security']['false_positive_rate']['false_positive_rate']}")
