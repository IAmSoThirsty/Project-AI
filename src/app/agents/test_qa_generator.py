...
Security Note: This agent uses subprocess to run pytest, a trusted development tool.
Commands are hardcoded and do not accept external input beyond validated directory paths.
"""
from __future__ import annotations

import logging
import os
import subprocess  # nosec B404 - subprocess usage for trusted pytest tool only
import time
...
class TestQAGenerator:
    ...
    def run_tests(self, tests_dir: str | None = None) -> dict[str, Any]:
        ...
        Security: Runs pytest with hardcoded arguments. The directory path is
        validated to exist before being passed to subprocess.
        """
        ...
        try:
            # nosec B603 B607 - pytest is a trusted dev tool, directory validated to exist
            res = subprocess.run(["pytest", run_dir, "-q"], capture_output=True, text=True, timeout=60)
            return {"success": res.returncode == 0, "output": res.stdout + res.stderr, "returncode": res.returncode}
        except subprocess.TimeoutExpired:
            logger.warning("pytest command timed out after 60 seconds")
