...
class DesktopAdapter:
    ...
    def execute(self, action: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(f"Desktop action {action}: {response['status']}")
    ...
    def ai_chat(
        ...
        if response["status"] == "success":
            ...
            raise RuntimeError(
                f"AI chat failed: {response.get('error', 'Unknown error')}"
            )
    ...
    def ai_image(
        ...
        if response["status"] == "success":
            ...
            raise RuntimeError(
                f"Image generation failed: {response.get('error', 'Unknown error')}"
            )
    ...
    def persona_update(self, trait: str, value: float) -> bool:
        ...
        if response["status"] == "success":
            ...
            raise RuntimeError(
                f"Persona update failed: {response.get('error', 'Unknown error')}"
            )
