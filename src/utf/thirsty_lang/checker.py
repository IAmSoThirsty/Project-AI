...
class Checker:
    ...
    def check(self, program: ast.Program) -> None:
        ...
        for decl in program.declarations:
            if isinstance(decl, ast.ImportDecl):
                ...
                self.globals.define(
                    alias, Symbol(Type("Module", (Type(alias),))), decl.span
                )
        for decl in program.declarations:
        ...
        for decl in program.declarations:
            if isinstance(decl, (ast.FunctionDecl, ast.GovernedFunctionDecl)):
                ...
                result = (
                    ...
                    else from_type_node(decl.return_type)
                )
                fn_type = (
                    task(result)
                    if decl.is_async
        ...
        for member in decl.members:
            if isinstance(member, ast.VarDecl):
                ...
                result = (
                    ...
                    else from_type_node(member.return_type)
                )
                info.methods[member.name] = (
                    params,
                    task(result) if member.is_async else result,
                    member.is_async,
                )
    ...
    def _check_stmt(self, stmt: ast.Stmt) -> None:
        ...
        if isinstance(stmt, ast.VarDecl):
            ...
            if (
                stmt.is_field
                and isinstance(stmt.initializer, ast.LiteralExpr)
                and stmt.initializer.value is None
            ):
                init_t = t
            else:
                ...
                if not self._assignable(t, init_t):
                    raise ThirstyError(
                        "THIRSTY-E021",
                        f"type mismatch: expected {t}, got {init_t}",
                        stmt.span,
                    )
            self.scope.define(
                stmt.name,
                Symbol(t, mutable=stmt.mutable and not stmt.is_field),
                stmt.span,
            )
            return
        if isinstance(stmt, (ast.FunctionDecl, ast.GovernedFunctionDecl)):
            ...
            self.current_return = (
                VOID if stmt.return_type is None else from_type_node(stmt.return_type)
            )
            if stmt.is_method and self.current_class:
                fn_scope.define(
                    "this",
                    Symbol(Type(self.current_class.name), mutable=True),
                    stmt.span,
                )
            prev_scope = self.scope
            self.scope = fn_scope
        ...
        if isinstance(stmt, ast.ReturnStmt):
            ...
            if not self._assignable(self.current_return, value_t):
                raise ThirstyError(
                    "THIRSTY-E024",
                    f"return type mismatch: expected {self.current_return}, got {value_t}",
                    stmt.span,
                )
            return
        ...
        if isinstance(stmt, ast.DripStmt):
            ...
            if not sym.mutable:
                raise ThirstyError(
                    "THIRSTY-E020",
                    f"cannot drip immutable binding '{stmt.name}'",
                    stmt.span,
                )
            ...
            if sym.type_ not in (INT, FLOAT) or amt_t not in (INT, FLOAT):
                raise ThirstyError(
                    "THIRSTY-E021", "drip requires numeric mutable bindings", stmt.span
                )
            return
        if isinstance(stmt, ast.IfStmt):
            ...
            if not equals(cond_t, BOOL):
                raise ThirstyError(
                    "THIRSTY-E022", "if condition must be Bool", stmt.condition.span
                )
        ...
        if isinstance(stmt, ast.LoopStmt):
            ...
            if not equals(count_t, INT):
                raise ThirstyError(
                    "THIRSTY-E023", "loop count must be Int", stmt.count.span
                )
            self._check_stmt(stmt.body)
            return
    ...
    def _narrowings(self, expr: ast.Expr) -> tuple[dict[str, Type], dict[str, Type]]:
        if (
            isinstance(expr, ast.BinaryExpr)
            and isinstance(expr.left, ast.VariableExpr)
            and isinstance(expr.right, ast.LiteralExpr)
    ...
    def _check_expr(self, expr: ast.Expr) -> Type:
        ...
        if isinstance(expr, ast.ThisExpr):
            ...
            if not sym:
                raise ThirstyError(
                    "THIRSTY-E011", "'this' outside of class context", expr.span
                )
            return sym.type_
        ...
        if isinstance(expr, ast.ArrayExpr):
            ...
            for item in expr.items[1:]:
                if not equals(first, self._check_expr(item)):
                    raise ThirstyError(
                        "THIRSTY-E021",
                        "array items must share a common type",
                        item.span,
                    )
        ...
        if isinstance(expr, ast.AssignExpr):
            ...
            if isinstance(expr.target, ast.VariableExpr):
                ...
                if not sym.mutable:
                    raise ThirstyError(
                        "THIRSTY-E020",
                        f"cannot assign to immutable binding '{expr.target.name}'",
                        expr.span,
                    )
            if not self._assignable(target_t, value_t):
                raise ThirstyError(
                    "THIRSTY-E021",
                    f"type mismatch: expected {target_t}, got {value_t}",
                    expr.span,
                )
            return target_t
        if isinstance(expr, ast.UnaryExpr):
            ...
            raise ThirstyError(
                "THIRSTY-E021",
                f"invalid unary operator '{expr.op}' for {rt}",
                expr.span,
            )
        if isinstance(expr, ast.BinaryExpr):
            ...
            if expr.op in {"+", "-", "*", "/", "%"}:
                ...
                raise ThirstyError(
                    "THIRSTY-E021",
                    f"operator '{expr.op}' expects matching numeric/string operands",
                    expr.span,
                )
            if expr.op in {"==", "!=", "<", "<=", ">", ">="}:
                if equals(lt, rt) or rt == option(ANY) or lt == option(ANY):
                    return BOOL
                raise ThirstyError(
                    "THIRSTY-E021", f"cannot compare {lt} and {rt}", expr.span
                )
            if expr.op in {"&&", "||"}:
                if equals(lt, BOOL) and equals(rt, BOOL):
                    return BOOL
                raise ThirstyError(
                    "THIRSTY-E021", "logical operators expect Bool", expr.span
                )
        ...
        if isinstance(expr, ast.GuardExpr):
            ...
            if not equals(cond_t, BOOL):
                raise ThirstyError(
                    "THIRSTY-E022", "guard condition must be Bool", expr.condition.span
                )
    ...
    def _check_pipe(self, left_t: Type, right: ast.Expr) -> Type:
        if isinstance(right, ast.VariableExpr):
            ...
            if (
                sym
                and sym.type_.name in {"BuiltinFn", "Function"}
                and len(sym.type_.args) >= 2
            ):
                first = sym.type_.args[0]
                if not self._assignable(first, left_t):
                    raise ThirstyError(
                        "THIRSTY-E021",
                        f"pipe cannot feed {left_t} into {right.name}",
                        right.span,
                    )
        ...
        if isinstance(right, ast.CallExpr):
            ...
            if callee_t.name in {"BuiltinFn", "Function"} and len(callee_t.args) >= 2:
                ...
                if not self._assignable(expected[0], left_t):
                    raise ThirstyError(
                        "THIRSTY-E021", "pipe input type mismatch", right.span
                    )
    ...
    def _check_member(self, obj_t: Type, name: str, span) -> Type:
        if obj_t.name == "Module" and obj_t.args:
            ...
            members = (
                self.module_types.get(base) or self.module_types.get(mod_name) or {}
            )
            if name in members:
                return members[name]
        if obj_t.name == "Reservoir" and obj_t.args:
    ...
    def _check_call(self, expr: ast.CallExpr) -> Type:
        ...
        if callee_t.name == "BuiltinMethod" and isinstance(expr.callee, ast.MemberExpr):
            ...
            if name == "size":
                self._check_call_arity(expr.args, [], expr.span)
                return INT
            if name == "push":
                self._check_call_arity(expr.args, [inner], expr.span)
                return VOID
            if name == "get":
                self._check_call_arity(expr.args, [INT], expr.span)
                return inner
            if name == "strain":
                self._check_call_arity(expr.args, [ANY], expr.span)
                return obj_t
            if name == "transmute":
                self._check_call_arity(expr.args, [ANY], expr.span)
                return reservoir(ANY)
            if name == "distill":
                self._check_call_arity(expr.args, [ANY, ANY], expr.span)
                return ANY
            if name == "flood":
                self._check_call_arity(expr.args, [ANY], expr.span)
                return obj_t
        if callee_t.name == "BuiltinFn":
            ...
            for arg_expr, exp in zip(expr.args, params, strict=False):
                ...
                if not self._assignable(exp, got):
                    raise ThirstyError(
                        "THIRSTY-E021", f"expected {exp}, got {got}", arg_expr.span
                    )
            result = callee_t.args[-1]
