...
from app.core.global_scenario_engine import GlobalScenarioEngine, register_global_scenario_engine
from app.core.simulation_contingency_root import RiskDomain, SimulationRegistry
...
def main():
    ...
    print(f"  Countries: {len(set().union(*[data.keys() for data in engine.historical_data.values()]))}")
    ...
    for event in sorted(all_events, key=lambda e: e.severity, reverse=True)[:5]:
        ...
        print(f"    Severity: {event.severity:.1%}, Z-score: {event.context.get('z_score', 0):.2f}")
    ...
    for i, scenario in enumerate(scenarios[:10], 1):
        ...
        print(f"   Affected domains: {', '.join(d.value for d in list(scenario.impact_domains)[:3])}")
    ...
    if alerts:
        print("\nCRITICAL ALERTS:")
        for i, alert in enumerate(sorted(alerts, key=lambda a: a.risk_score, reverse=True)[:5], 1):
            print(f"\n{'='*70}")
            ...
            for link in alert.causal_activation[:3]:
                print(f"  • {link.source} → {link.target} (strength: {link.strength:.2f})")
