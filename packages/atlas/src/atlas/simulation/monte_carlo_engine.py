"""atlas.simulation.monte_carlo_engine - Layer 6: Coupled Monte
Carlo Dynamics Core

Production-grade Monte Carlo simulation with:
- System evolution: W_{t+1} = F(W_t, eps_t)
...
This is a simulation tool for analysis, not a
decision-making system.

Architectural notes (port from legacy):

The legacy used `from atlas.audit.trail import get_audit_trail`
which is a legacy submodule path. The canonical atlas exposes
`get_audit_trail` at the top level
(`from atlas import get_audit_trail`), per the
`atlas/__init__.py` exports. This port uses the canonical
import path.

The MonteCarloEngine is the most foundational of the 4
simulation modules (it provides the noise + coupling
primitives that the other 3 modules use). The other 3
modules (agent_simulator, contingency_triggers,
timeline_divergence) build on top of WorldState + NoiseVector
+ CouplingCoefficients.

Public surface:
- Domain enum (5 values)
- WorldState dataclass (with compute_hash + validate)
- NoiseVector dataclass (with generate factory)
- CouplingCoefficients dataclass (with validate)
- MonteCarloEngine class (with 8 public methods)
...
from atlas import get_audit_trail
...
@dataclass
class NoiseVector:
    """Seeded noise vector eps_t for stochastic dynamics.
    ...
    market_noise: np.ndarray[Any, np.dtype[Any]] = field(default_factory=lambda: np.array([]))
    governance_noise: np.ndarray[Any, np.dtype[Any]] = field(default_factory=lambda: np.array([]))
    regulation_noise: np.ndarray[Any, np.dtype[Any]] = field(default_factory=lambda: np.array([]))
    graph_noise: np.ndarray[Any, np.dtype[Any]] = field(default_factory=lambda: np.array([]))
    capital_noise: np.ndarray[Any, np.dtype[Any]] = field(default_factory=lambda: np.array([]))
...
class MonteCarloEngine:
    ...
    Implements system evolution: W_{t+1} = F(W_t, eps_t)
    ...
    def __init__(
        self,
        seed: str,
        coupling: CouplingCoefficients | None = None,
        audit_trail: Any = None,
    ) -> None:
        """Initialize Monte Carlo engine.

        Args:
            seed: Hex seed string for reproducibility
                (e.g., "0xA17F01")
        ...
        self.audit_trail: Any = audit_trail or get_audit_trail()
        ...
        try:
            ...
            # Audit trail may not be fully configured
            pass
    ...
    def _apply_coupling(self, state: WorldState, noise: NoiseVector) -> WorldState:
        ...
        market_effect = (
            self.coupling.governance_to_markets * _safe_mean(state.governance.values())
            + self.coupling.regulation_to_markets * _safe_mean(state.regulation.values())
            + self.coupling.graph_to_markets * _safe_mean(state.graph_topology.values())
            + self.coupling.capital_to_markets * _safe_mean(state.capital_distribution.values())
        )
        ...
        governance_effect = (
            self.coupling.markets_to_governance * _safe_mean(state.markets.values())
            + self.coupling.regulation_to_governance * _safe_mean(state.regulation.values())
            + self.coupling.graph_to_governance * _safe_mean(state.graph_topology.values())
            + self.coupling.capital_to_governance * _safe_mean(state.capital_distribution.values())
        )
        ...
        regulation_effect = (
            self.coupling.markets_to_regulation * _safe_mean(state.markets.values())
            + self.coupling.governance_to_regulation * _safe_mean(state.governance.values())
            + self.coupling.graph_to_regulation * _safe_mean(state.graph_topology.values())
            + self.coupling.capital_to_regulation * _safe_mean(state.capital_distribution.values())
        )
        ...
        graph_effect = (
            self.coupling.markets_to_graph * _safe_mean(state.markets.values())
            + self.coupling.governance_to_graph * _safe_mean(state.governance.values())
            + self.coupling.regulation_to_graph * _safe_mean(state.regulation.values())
            + self.coupling.capital_to_graph * _safe_mean(state.capital_distribution.values())
        )
        ...
        capital_effect = (
            self.coupling.markets_to_capital * _safe_mean(state.markets.values())
            + self.coupling.governance_to_capital * _safe_mean(state.governance.values())
            + self.coupling.regulation_to_capital * _safe_mean(state.regulation.values())
            + self.coupling.graph_to_capital * _safe_mean(state.graph_topology.values())
        )
...
def _safe_mean(values: Any) -> float:
    """Compute mean of values, returning 0.0 for empty iterables.

    Args:
        values: Any iterable of numbers (or empty)
    ...
    if not materialised:
        return 0.0
    return float(np.mean(materialised))
