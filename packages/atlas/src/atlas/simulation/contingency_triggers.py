"""atlas.simulation.contingency_triggers - Layer 8: Contingency
Trigger Framework (RS Only)

Production-grade trigger system with:
- Deterministic condition evaluation
  (metric > threshold for duration >= D)
...
Triggers are FOR ANALYSIS ONLY - they do not authorize
actions.
...
The legacy's `TriggerCondition.validate()` uses
`np.isnan` and `np.isinf` but does not import numpy. This
port adds the missing `import numpy as np` (the existing
atlas already depends on numpy for sensitivity + MonteCarlo
+ AgentSim).

Public surface:
- StackType enum (6 values)
- TriggerType enum (4 values)
- PlaybookAction enum (5 values)
- TriggerCondition dataclass (with evaluate + reset +
  validate)
- Playbook dataclass (with compute_hash + lock +
  verify_integrity)
- TriggerActivation dataclass
- ContingencyTriggerFramework class (with 6 public methods)
- get_contingency_trigger_framework() factory
"""
    ...
    def evaluate(self, metric_value: float) -> bool:
        ...
        if met:
            ...
            if self.first_triggered is None:
                self.first_triggered = datetime.now(UTC)
        else:
...
class Playbook:
    ...
    Playbooks are FOR ANALYSIS ONLY - they do not
    authorize actions.
    """
...
class ContingencyTriggerFramework:
    ...
    Evaluates deterministic trigger conditions and links to
    playbooks.

    CRITICAL: RS-only enforcement. Narrative triggers are
    BLOCKED.
    """

    def __init__(self, stack: StackType, audit_trail: Any = None) -> None:
        ...
        logger.info(
            "Contingency trigger framework initialized for %s",
            stack.value,
        )
    ...
    def reject_narrative_trigger(self, trigger_description: str) -> None:
        ...
        Narrative triggers are NOT allowed - only deterministic
        metrics.
        """
        try:
            trail: Any = self.audit_trail
    ...
    def evaluate_triggers(self, metrics: dict[str, float]) -> list[TriggerActivation]:
        """Evaluate all trigger conditions against current
        metrics.
        ...
        for playbook_id, playbook in self.playbooks.items():
            ...
            for condition in playbook.conditions:
                ...
                if metric_value is None:
                    logger.warning(
                        "Metric %s not found in current metrics",
...
_framework: ContingencyTriggerFramework | None = None


def get_contingency_trigger_framework(
    stack: StackType = StackType.RS,
    audit_trail: Any = None,
) -> ContingencyTriggerFramework:
    """Get singleton contingency trigger framework instance
    (RS only)."""
    global _framework
