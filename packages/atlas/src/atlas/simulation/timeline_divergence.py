"""atlas.simulation.timeline_divergence - Layer 7: Multi-Seed
...
The legacy imports `get_monte_carlo_engine` and `WorldState`
from `atlas.simulation.monte_carlo_engine`. The canonical
import is the same (J4.1 ported the MonteCarloEngine
module to the same location).

Public surface:
- UncertaintyAxis enum (2 values)
- ProjectionPoint dataclass
- TimelineDivergence dataclass
- ProjectionTensor dataclass (with get + set +
  compute_hash + get_statistics)
- TimelineDivergenceEngine class (with 4 public methods)
- get_timeline_divergence_engine() factory
"""
...
class ProjectionTensor:
    ...
    def compute_hash(self) -> str:
        ...
        canonical: dict[str, Any] = {
            ...
            "point_count": sum(
                len(years) for seed_data in self.data.values() for years in seed_data.values()
            ),
        }
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_stochastic_volatility": (float(np.mean(all_stochastic)) if all_stochastic else 0),
            "avg_structural_divergence": (float(np.mean(all_structural)) if all_structural else 0),
...
class TimelineDivergenceEngine:
    ...
    storing results in tensor format with uncertainty
    tracking.
    """
    ...
    def __init__(
        ...
        Args:
            seeds: List of hex seeds (uses STANDARD_SEEDS if
                None)
            horizons: List of horizon years (uses
                STANDARD_HORIZONS if None)
    ...
    def project_single_timeline(
        ...
        for year in range(horizon_years + 1):  # Include year 0
            ...
            if step_idx < len(states):
                ...
                markets_mean = float(np.mean(list(state.markets.values()))) if state.markets else 0
                governance_mean = (
                    float(np.mean(list(state.governance.values()))) if state.governance else 0
                )
                capital_mean = (
                    float(np.mean(list(state.capital_distribution.values())))
                ...
                point = ProjectionPoint(
                    ...
                    metrics={
                        ...
                        "market_avg": markets_mean,
                        "governance_avg": governance_mean,
                        "capital_concentration": capital_mean,
                    },
    ...
    def compute_divergence(
        ...
        """Compute divergence statistics between two
        timelines.
    ...
    def project_all_timelines(
        ...
        """Project all timelines (all seeds x all horizons).
        ...
        # Project each seed x horizon combination
    ...
    def analyze_uncertainty(self, tensor: ProjectionTensor) -> dict[str, Any]:
        ...
        for horizon in tensor.horizons:
            stochastic_vols: list[float] = []
            structural_divs: list[float] = []
...
# Singleton instance
_engine: TimelineDivergenceEngine | None = None
