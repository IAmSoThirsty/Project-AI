"""atlas.simulation.agent_simulator - Layer 5: Agent-Based
...
Public surface:
- AgentType enum (6 values)
- ResourceType enum (5 values)
- ResourceConstraints dataclass (with validate + can_act +
  consume_resources)
- UtilityFunction dataclass (with compute + validate_weights)
- AgentState dataclass (with compute_action_vector,
  update_perception, update_driver_pressure, record_action,
  tick, validate)
- AgentSimulator class (with 7 public methods)
- get_agent_simulator() factory
"""
...
class UtilityFunction:
    ...
    def compute(
        ...
        utility *= float(np.exp(-self.discount_rate * timestep))
...
class AgentState:
    ...
    No free will modeling - agents respond deterministically
    to inputs.
    """
    ...
    def compute_action_vector(
        ...
        probabilities: np.ndarray[Any, np.dtype[Any]] = exp_utilities / np.sum(exp_utilities)
    ...
    def validate(self) -> tuple[bool, list[str]]:
        ...
        # Validate resources
        _valid, resource_errors = self.resources.validate()
        errors.extend(resource_errors)

        # Validate utility function
        _valid, utility_errors = self.utility_function.validate_weights()
...
# Singleton instance
_simulator: AgentSimulator | None = None


def get_agent_simulator(
