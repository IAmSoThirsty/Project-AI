"""atlas.safeguards.epistemic_safeguards - Critical safety
layer preventing ATLAS Omega from becoming de facto authority.
...
CRITICAL: These safeguards protect against ATLAS Omega
becoming de facto authority.

Architectural notes (port from legacy):

The legacy used `from atlas.audit.trail import
get_audit_trail` which is a legacy submodule path. The
canonical atlas exposes `get_audit_trail` at the top
level (`from atlas import get_audit_trail`), per the
`atlas/__init__.py` exports. This port uses the
canonical import path.

The legacy used `datetime.now()` (naive datetimes). This
port uses `datetime.now(UTC)` (aware datetimes in UTC)
for consistency with the rest of the atlas simulation
modules (J4.1-J4.4).

Public surface:
- DecisionBasis enum (4 values)
- Decision dataclass (with validate method)
- EpistemicGravityMitigation class (Safeguard 1)
- QueryType enum (8 values: 4 allowed + 4 blocked)
- QueryValidation dataclass
- PromptFramingGuards class (Safeguard 2)
- ResponsibilityClause dataclass (with compute_hash + lock)
- OutputRecord dataclass
- ResponsibilityBoundaryEnforcement class (Safeguard 3)
- EpistemicSafeguardSystem class (unified facade)
- get_epistemic_safeguards() factory
"""
...
class DecisionBasis(Enum):
    """How much a decision relied on ATLAS Omega."""
...
@dataclass
class Decision:
    """Record of a decision with ATLAS Omega involvement
    tracking."""
...
class EpistemicGravityMitigation:
    ...
    Prevents ATLAS Omega from becoming de facto authority
    through:
    - Decision logging showing "informed by" vs
      "justified solely by"
    ...
    def log_decision(self, decision: Decision) -> None:
        ...
        This creates an audit trail showing decisions were
        NOT justified solely by ATLAS Omega output.
        """
        # Validate decision
        ...
        logger.info(
            "Decision logged: %s (basis: %s)",
            decision.decision_id,
            decision.basis.value,
        )

    def verify_dissent_pathway(self) -> tuple[bool, str]:
        """Verify that dissent from ATLAS is possible and has
        occurred.

        If dissent is impossible, ATLAS becomes de facto
        authority.
        ...
        if self.dissent_count == 0 and self.atlas_consulted_count > 10:
            return (
                False,
                f"No dissents in {self.atlas_consulted_count} "
                f"ATLAS-consulted decisions - pathway may be "
                f"blocked",
            )
        ...
        if self.dissent_count > 0:
            ...
            return (
                True,
                f"Dissent pathway verified: {self.dissent_count} "
                f"dissents ({dissent_rate:.1%} rate)",
            )
...
class QueryType(Enum):
    """Types of queries to ATLAS Omega."""
...
class PromptFramingGuards:
    ...
    ATLAS Omega from a simulation tool into a decision-maker.
    """
    ...
    def __init__(self, audit_trail: Any = None) -> None:
        ...
        self.rejected_queries: list[tuple[str, str | None]] = []  # (query, reason)
    ...
    def validate_query(self, query: str) -> QueryValidation:
        ...
        for keyword in NORMATIVE_KEYWORDS:
            if keyword in query_lower:
                validation = QueryValidation(
                    ...
                    rejection_reason=(
                        f"Normative query detected "
                        f"(keyword: '{keyword}'). "
                        f"ATLAS Omega projects, does not recommend."
                    ),
                )
        ...
        # Allow query
        validation = QueryValidation(query=query, is_allowed=True, query_type=query_type)

        self.allowed_queries.append(query)

        try:
            log: Any = self.audit_trail.log
            log(
                category="GOVERNANCE",
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        total = len(self.allowed_queries) + len(self.rejected_queries)
        return {
            "total_queries": total,
            "allowed": len(self.allowed_queries),
            "rejected": len(self.rejected_queries),
            "rejection_rate": (len(self.rejected_queries) / max(1, total)),
        }
...
class ResponsibilityClause:
    """Non-transferable responsibility clause attached to
    output."""
    ...
    clause_text: str = (
        "WARNING: RESPONSIBILITY CLAUSE:\n"
        "This output does NOT justify or authorize action.\n"
        "This is a simulation/projection for analysis only.\n"
        "Decision-makers retain full responsibility for "
        "their choices.\n"
        "ATLAS Omega provides projections (not decisions), "
        "assists (not replaces), extends (not subsumes)."
    )
...
class ResponsibilityBoundaryEnforcement:
    ...
    Ensures every ATLAS output carries non-transferable
    responsibility clause and tracks human decision-makers
    with reasoning.
    """
    ...
    def attach_clause(
        ...
        """Attach non-transferable responsibility clause to
        output.

        This MUST be called for every ATLAS Omega output.
        """
        ...
        logger.info(
            "Responsibility clause attached to output %s",
            output_id,
        )
        return clause

    def log_output_use_in_decision(
        self,
        output_id: str,
        decision_id: str,
        decision_maker: str,
        ...
        Records WHO made the decision and their reasoning
        BEYOND ATLAS.
        """
        # Find output
        record = next(
            (r for r in self.outputs if r.output_id == output_id),
            None,
        )
        if not record:
        ...
        if not reasoning or len(reasoning) < 10:
            logger.warning(
                "Decision %s has insufficient reasoning beyond ATLAS",
                decision_id,
            )
        ...
        logger.info(
            "Output %s used in decision %s by %s",
            output_id,
            decision_id,
            decision_maker,
        )
...
class EpistemicSafeguardSystem:
    ...
    Ensures ATLAS Omega remains a tool, not an authority.
    """
    ...
    def __init__(self, audit_trail: Any = None) -> None:
        ...
        logger.info("Epistemic safeguard system active (all 3 safeguards)")
...
# Singleton instance
_safeguards: EpistemicSafeguardSystem | None = None


def get_epistemic_safeguards(
