"""atlas.config.loader - Configuration Loader and Validator
for PROJECT ATLAS

Loads and validates all YAML configuration files with
schema enforcement, cryptographic verification, and
safety checks.
...
The legacy used `import yaml` directly. PyYAML is
already an installed dep in the canonical atlas
environment (per `uv pip list` showing pyyaml 6.0.3).

Public surface:
- ConfigurationError exception
- SafetyViolationError exception
- ConfigLoader class with 8 public methods:
  __init__, get, get_hash, get_all_hashes,
  verify_integrity, get_metadata, export_audit_log
- get_config_loader() factory (singleton)
- reset_config_loader() helper (for testing)
"""
...
class ConfigurationError(Exception):
    """Raised when configuration is invalid or cannot be
    loaded."""
...
class ConfigLoader:
    """Production-grade configuration loader for PROJECT
    ATLAS.

    Loads, validates, and provides access to all YAML
    configuration files with immutability enforcement.
    """
    ...
    def __init__(self, config_dir: Path | None = None) -> None:
        ...
        Args:
            config_dir: Path to configuration directory
                (defaults to atlas/config)
        """
        if config_dir is None:
            # Default to atlas/config (this file is in
            # config directory)
        ...
        logger.info(
            "Initializing ConfigLoader with directory: %s",
            self.config_dir,
        )
    ...
    def _validate_versions(self) -> None:
        """Validate that all configurations have consistent
        versions."""
    ...
    def _validate_safety(self) -> None:
        """Validate safety configuration with strict
        enforcement."""
        ...
        for section_name, section in safety.items():
            ...
            for rule_name, rule in section.items():
                if isinstance(rule, dict) and "bypass" in rule and rule.get("bypass", True):
                    logger.warning(
    ...
    def _validate_consistency(self) -> None:
        """Validate cross-configuration consistency."""
        # Validate that all stacks referenced in seeds exist
        # in stacks config
        ...
        if "timeline_seeds" in seeds:
            for stack_name in seeds["timeline_seeds"]:
                if stack_name not in stacks:
                    logger.warning(
                        "Seed defined for unknown stack: %s",
                        stack_name,
                    )
    ...
    def verify_integrity(self) -> bool:
        ...
        Returns:
            True if all configurations are unchanged,
            False otherwise
        """
        for config_name, original_hash in self._hashes.items():
            ...
            try:
                ...
                if current_hash != original_hash:
                    logger.error(
                        f"Configuration {config_name} has been "
                        f"modified! Original: {original_hash}, "
    ...
    def get_metadata(self) -> dict[str, Any]:
        ...
        return {
            ...
            "hashes": self._hashes,
            "versions": {
                name: config.get("version", "unknown") for name, config in self._configs.items()
            },
        }
