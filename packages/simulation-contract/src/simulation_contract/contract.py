...
Vendored from T:\\00-Active\\Project-AI-main\\src\\app\\core\\simulation_contingency_root.py
for the J2 alien_invaders scenario engine port. Same contract as the
J2 global_scenario and J2 ai_takeover ports; all three engines share
the simulation contract family. Follow-up: promote to a shared
``packages/simulation-contract/`` package and have global_scenario,
ai_takeover, and alien_invaders all depend on it. The legacy module is the
shared contract that the ``engines/global_scenario/`` engine depends on.

This vendored copy preserves the legacy contract exactly. The runtime
imports of ``engines.alien_invaders.modules.planetary_defense_monolith``
inside ``SimulationRegistry`` methods are preserved as-is; the
``global_scenario`` engine itself never calls them (it never uses the
``register``, ``unregister``, or mutable-``get`` paths).
"""
...
This is the shared simulation contract that all Project-AI
scenario engines (cognitive_warfare, global_scenario,
emp_defense, ai_takeover, alien_invaders, django_state)
depend on for register / lookup / authorization of
...
Public surface (9 types):
  ...
  - ThresholdEvent       (dataclass, threshold exceedance event)
  - CausalLink           (dataclass, edge in a causal graph)
  - ScenarioProjection   (dataclass, what-if scenario output)
  - CrisisAlert          (dataclass, fire-and-acknowledge alert)
  - RegistryAccessRequest (dataclass, request for registry access)
  - SimulationSystem     (ABC, base for all simulation engines)
  - SimulationRegistry   (classmethod-based registry)

The 8 types in the J2 ports' vendored copies (alert,
threshold, causal, projection, system, registry, plus
the 2 enums) are the historical contract. ``RegistryAccessRequest``
was added during the contract promotion to break the
backward dependency from the contract on
``alien_invaders.modules.planetary_defense_monolith``
(lazy import that violated the shared-package principle).

Vendoring history:

  - Originally at T:\\00-Active\\Project-AI-main\\src\\app\\core\\
    simulation_contingency_root.py (446 lines)
  - Vendored 3 times during J2 ports:
       packages/global-scenario/  (commit 4078747c)
       packages/ai-takeover/     (commit ec839b08)
       packages/alien-invaders/  (commit b8f43cfe)
  - Promoted to shared ``packages/simulation-contract/``
    (this commit)

Why this is a shared package instead of continuing to
vendor: the 3 vendored copies had drifted (different
docstrings, different lazy-import paths, different
line counts). A shared package makes future contract
changes atomic across all engines.

If you are porting a new engine and need this contract,
add ``project-ai-simulation-contract`` to your package's
dependencies and ``from simulation_contract import
SimulationRegistry, SimulationSystem, etc.`` in your
adapter.
"""
...
class RegistryAccessRequest:
    ...
    This dataclass used to live in
    ``alien_invaders.modules.planetary_defense_monolith``.
    It was moved to the shared contract when
    ``packages/simulation-contract/`` was created, to
    break the backward dependency from the contract on
    the alien_invaders engine (a shared package must
    not depend on a specific engine).

    Used by ``SimulationRegistry.register`` /
    ``unregister`` / mutable-``get`` paths when
    ``_projection_mode`` is True and a monolith
...
class SimulationRegistry:
    ...
    def register(cls, name: str, system: SimulationSystem, from_monolith: bool = False) -> None:
        ...
        if cls._projection_mode and not from_monolith:  # noqa: SIM102
            if cls._monolith_authority is not None:
                # Request authorization from monolith
                # (RegistryAccessRequest is in this module; no
                # lazy import needed — was lazy in the vendored
                # copies to break an import cycle that no longer
                # exists now that the contract is a shared package)
    ...
    def get(
        ...
        if mutable and cls._projection_mode and not from_monolith:  # noqa: SIM102
            if cls._monolith_authority is not None:
                # Request authorization from monolith
                # (RegistryAccessRequest is in this module; no
                # lazy import needed — was lazy in the vendored
