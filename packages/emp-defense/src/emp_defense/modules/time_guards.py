...
from emp_defense.modules.constants import TimeConstants
...
def validate_time_transition(
    ...
    if not is_early_phase(state) and hours_to_advance % 24 != 0:
        return (
            False,
            f"Post-early phase should use daily timesteps, got {hours_to_advance}h",
        )
...
def phase_allows_event(state: SectorizedWorldState, event_name: str) -> tuple[bool, str]:
    ...
    if is_governance_failure_phase(state):  # noqa: SIM102
