...
from emp_defense.schemas.config_schema import SimulationConfig
...
class EMPDefenseEngine:
    ...
    def init(self) -> bool:
        ...
        try:
            ...
            # Mark initialized BEFORE applying the initial EMP event so
            # that ``inject_event`` (called from ``_apply_emp_event``)
            # accepts the event. This is a one-line ordering fix for
            # a pre-existing legacy bug where the engine applied the
            # initial EMP event before setting self.initialized = True,
            # causing the initial event to be silently rejected.
