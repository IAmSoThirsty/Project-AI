...
class Position:
    ...
    side: str = "long"
    opened_at: int = field(default_factory=lambda: int(datetime.now(UTC).timestamp() * 1000))
...
class PortfolioManager:
    ...
    def __init__(
        ...
        self._balances: dict[str, Balance] = {
            currency: Balance(currency=currency, total=initial_balance, available=initial_balance)
        }
    ...
    def get_state(self) -> PortfolioState:
        ...
        cash_balance = self._balances.get(self.currency, Balance(self.currency, 0.0, 0.0)).total
        ...
        if current_position is None:
            ...
            if quantity > 0:
                ...
                if current_position.quantity == 0:
                    ...
                    logger.info(
                        f"Reduced position: {symbol} {quantity} @ {price}, realized PnL: {pnl:.2f}"
                    )
    ...
    def get_balance(self, currency: str | None = None) -> Balance:
        ...
        return self._balances.get(currency, Balance(currency=currency, total=0.0, available=0.0))
    ...
    def update_balance(
        ...
        if lock:
            ...
            if new_total < 0:
                raise ValueError(f"Insufficient balance: {current_balance.total} {currency}")
        ...
        logger.debug(
            f"Updated balance: {currency} {amount:+.2f}, new total: {current_balance.total:.2f}"
        )
