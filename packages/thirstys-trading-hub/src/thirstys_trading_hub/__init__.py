...
class TradingHub:
    ...
    def risk_manager(self):
        ...
        NOTE: The ``governance.risk_limits`` module
        was never created in the legacy code. The
        import below will fail at runtime until
        ``thirstys_trading_hub/governance/risk_limits.py``
        is added. The unit tests in this package
        do not exercise this code path; they call
        the core modules directly. Live trading via
        ``TradingHub.place_order`` will require the
        risk manager to be implemented first.
        """
        if self._risk_manager is None:
            from thirstys_trading_hub.governance.risk_limits import (
                RiskManager,
            )
