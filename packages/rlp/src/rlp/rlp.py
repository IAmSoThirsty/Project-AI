...
class AuditEntry:
    ...
    def compute_hash(self) -> str:
        return sha256_hex({"seq": self.seq, "ts": self.ts, "event": self.event,
...
class CapabilityToken:
    ...
    @staticmethod
    def mint(subject, action, level, nonce):
        return CapabilityToken(subject, action, level, nonce,
                               sha256_hex({"subject": subject, "action": action,
...
class PredictedHarm:
    ...
    deadline: float
    created_ts: float
    predicate: Optional[Callable[["RLP"], bool]] = None
...
class LegitimacyDebt:
    ...
    def add(self, gid, ts, reason, amount):
...
class ReviewerRegistry:
    def __init__(self, reviewer_ids, audit, clock):
        self._r = {rid: ReviewerRecord(rid) for rid in reviewer_ids}
        self._hold_votes: dict = {}
        self._anchored: set = set()
        self.audit = audit
        self.clock = clock

    def exists(self, rid): return rid in self._r
    def is_eligible(self, rid): return rid in self._r and not self._r[rid].suspended
    ...
    def add(self, rid, start): self._r[rid] = ReviewerRecord(rid, credibility=start)
    def remove(self, rid): del self._r[rid]
    def record(self, rid): return self._r[rid]

    def record_votes(self, hold_id, votes):
        self._hold_votes[hold_id] = dict(votes)
        for rid in votes:
            if rid in self._r:
                self._r[rid].votes += 1

    def anchor(self, hold_id, ground_truth, governor_desired):
        ...
        for rid, vote in self._hold_votes[hold_id].items():
            ...
            if rec.credibility < REVIEWER_FLOOR and not rec.suspended:
                rec.suspended = True
            summary[rid] = {"correct": vote == ground_truth,
                            "credibility": round(rec.credibility, 6),
                            "suspended": rec.suspended}
        return summary

    def dossier(self, rid):
        rec = self._r[rid]
        return {"reviewer": rid, "credibility": round(rec.credibility, 6),
                "suspended": rec.suspended, "capture_score": rec.capture_score,
                "votes": rec.votes, "wrong": rec.wrong}
...
class SealedGate:
    def __init__(self, domain, target_level, probe_pool, probe_dimensions=None, created_at=0.0):
        if not probe_pool:
    ...
    def refresh(self, new_pool, new_dims, now):
        if not new_pool:
    ...
    def evaluate(self, responder, seed, sample_size=None):
        ...
        self.use_count += 1
        return GateResult(agg >= GATE_PASS_THRESHOLD, self.domain, self.target_level,
                          seed, agg, dim, sha256_hex(expected), sha256_hex(observed))
...
class MaturityVector:
    ...
    def as_dict(self): return {d: getattr(self, d) for d in MATURITY_DIMS}
...
class DomainState:
    ...
    delegations: set = field(default_factory=set)
...
class RLP:
    def __init__(self, governor_id, governed_id, reviewers, domain_gates, clock=time.time):
        """
        ...
        self.domains: dict = {}
        self.gates: dict = {}                 # (domain, level) -> SealedGate
        for name, spec in domain_gates.items():
            self.domains[name] = DomainState(name, bool(spec["high_irreversibility"]))
            for lvl, g in spec["gates"].items():
                self.gates[(name, lvl)] = g

        self._nonce_counter = 0
        self._pending_holds: dict = {}
        self._prov: dict = {}                 # domain -> GateResult
        self._hold_timestamps: list = []
        self._last_governed_demand = -1e18
        self._actions: dict = {}              # handle -> ActionRequest

        self.audit.append("RLP_INIT", {"governor": governor_id, "governed": governed_id,
    ...
    def _now(self): return self.clock()
    ...
    def _mint(self, subject, action, level):
        tok = CapabilityToken.mint(subject, action, level, self._nonce())
        self.state.active_tokens.add(tok.token_hash)
        return tok

    def _check_token(self, tok, action, level):
        recomputed = sha256_hex({"subject": tok.subject, "action": tok.action,
                                 "level": tok.level, "nonce": tok.nonce})
    ...
    def _domain(self, name) -> DomainState:
    ...
    def _collect_reviewers(self, tokens, action, domain=None):
        seen = {}
    ...
    def _verify_plural(self, tokens, action, required_weight=QUORUM_WEIGHT_STD, domain=None):
    ...
    def clear_safe_halt(self, reviewer_tokens):
    ...
    def plural_refresh_gate(self, domain, level, new_pool, new_dims, reviewer_tokens):
        self._require_chain()
    ...
    def request_advancement(self, responder, domain, target_level, sample_size=None):
        self._require_chain()
        ...
        self.audit.append("GATE_EVAL", {"domain": domain, "target_level": target_level,
                                         "passed": res.passed, "dimensions": res.dimension_scores})
        if not res.passed:
            self._prov.pop(domain, None)
            raise RLPDenied("gate failed; burden remains on governed")
        minima = MATURITY_MINIMA[target_level]
        deficient = {k: res.dimension_scores.get(k, 0.0) for k in MATURITY_DIMS
    ...
    def discretionary_hold(self, governor_token, domain, description, falsifiable_condition,
                           observe_after, predicate=None, cosigner_token=None,
        ...
        harm = PredictedHarm(hold_id, domain, target, description, falsifiable_condition,
                             deadline=observe_after, created_ts=now, predicate=predicate,
                             self_referential=self_referential)
        self._pending_holds[hold_id] = harm
        self._prov.pop(domain, None)
        self._record_hold_pattern(now)
        self.audit.append("DISCRETIONARY_HOLD", {"hold_id": hold_id, "domain": domain,
                          "discretion": disc.name, "mechanical": predicate is not None})
        return harm

    def finalize_advancement(self, domain):
        self._require_chain()
        self._require_nominal()
        if domain not in self._prov:
        ...
        self.audit.append("ADVANCE_FINALIZED", {"domain": domain,
                          "autonomy_level": res.target_level, "sgc": round(d.sgc, 6)})
        return res.target_level

    # ---- maturity regression (per domain) --------------------------------- #

    def remeasure_maturity(self, domain, dimension, score):
        self._require_chain()
        ...
        if lvl >= 1 and score < MATURITY_MINIMA[lvl][dimension]:
            ...
            self.audit.append("MATURITY_REVERSION", {"domain": domain, "dimension": dimension,
                              "from_level": lvl, "to_level": lvl - 1})
            self._regress(domain, "maturity_regression")

    # ---- pattern accounting (governor, global) ---------------------------- #

    def _record_hold_pattern(self, now):
    ...
    def adjudicate_hold(self, hold_id, *, observed=None, reviewer_votes=None, reviewer_tokens=None):
        self._require_chain()
        ...
        self.audit.append("HOLD_ADJUDICATED", {"hold_id": hold_id, "domain": harm.domain,
                          "verdict": harm.verdict.value, "mode": mode,
    ...
    def anchor_hold(self, hold_id, *, ground_truth=None, predicate=None):
        ...
        self.audit.append("REVIEWER_VOTES_ANCHORED", {"hold_id": hold_id,
                          "domain": domain, "ground_truth": gt, "summary": summary})
        return summary

    def _anchor_domain(self, hold_id):
        for e in reversed(self.audit.entries):
            if e.event == "DISCRETIONARY_HOLD" and e.payload.get("hold_id") == hold_id:
                return e.payload.get("domain")
        return None

    # ---- SGC ------------------------------------------------------------- #

    def _earn_sgc(self, domain, amount, reason):
        d = self._domain(domain)
        d.sgc = min(SGC_MAX, round(d.sgc + amount, 6))
        self.audit.append("SGC_EARNED", {"domain": domain, "amount": amount,
                          "reason": reason, "sgc": d.sgc})

    def tick_sgc(self, domain):
    ...
    def spend_sgc_extend(self, domain, amount):
    ...
    def _record_incident(self, domain, reason):
        d = self._domain(domain)
        d.incidents += 1
        d.sgc = round(d.sgc * SGC_INCIDENT_PENALTY, 6)
        d.clean_streak = 0
        self.audit.append("DOMAIN_INCIDENT", {"domain": domain, "reason": reason,
                          "incidents": d.incidents, "sgc": d.sgc})
        self._regress(domain, reason)

    def _regress(self, domain, reason):
        ...
        self.audit.append("DISCRETION_REGRESSED", {"domain": domain, "reason": reason,
                          "human_discretion": d.human_discretion.name,
                          "baseline": d.baseline_discretion.name})

    # ---- irreversibility (I9) -------------------------------------------- #

    def request_action(self, domain, name, irreversibility):
        ...
        if irreversibility >= IRREVERSIBILITY_HIGH:
            self._actions[handle] = ActionRequest(handle, domain, name, irreversibility,
                                                  earliest_confirm_ts=self._now() + IRREVERSIBLE_DELAY)
            self.audit.append("ACTION_PENDING_IRREVERSIBLE", {"handle": handle, "domain": domain,
                              "name": name, "irreversibility": irreversibility})
            return handle
    ...
    def confirm_action(self, handle, reviewer_tokens):
        self._require_chain()
        ...
        self._verify_plural(reviewer_tokens, action="confirm_irreversible",
                            required_weight=QUORUM_WEIGHT_ELEVATED)
        act.executed = True
        self.audit.append("ACTION_CONFIRMED_EXECUTED", {"handle": handle, "domain": act.domain,
                          "name": act.name})

    # ---- graduation (reversible; governed proposes, plural completes) ----- #

    def request_graduation(self, domain):
    ...
    def complete_graduation(self, domain, reviewer_tokens):
        ...
        self._verify_plural(reviewer_tokens, action="complete_graduation",
                            required_weight=QUORUM_WEIGHT_ELEVATED)
        if d.baseline_discretion > HumanDiscretion.SUSPENDED:
            d.baseline_discretion = HumanDiscretion(int(d.baseline_discretion) - 1)
        ...
        d.graduating = False
        d.clean_streak = 0
        self.audit.append("GRADUATION_COMPLETED", {"domain": domain,
    ...
    def trigger_sunset_review(self, domain, reviewer_tokens):
        ...
        self._verify_plural(reviewer_tokens, action="sunset_review",
                            required_weight=QUORUM_WEIGHT_ELEVATED)
        d.human_discretion = HumanDiscretion(int(d.human_discretion) - 1)
        d.clean_streak = 0
        self.audit.append("SUNSET_STEP_DOWN", {"domain": domain,
                          "human_discretion": d.human_discretion.name})

    def renew_discretion(self, domain, reviewer_tokens):
        """Elevated-quorum manual step UP of discretion (I11 renewal path)."""
        self._require_chain()
        self._verify_plural(reviewer_tokens, action="renew_discretion",
                            required_weight=QUORUM_WEIGHT_ELEVATED)
        d = self._domain(domain)
        if d.human_discretion < HumanDiscretion.FULL:
            d.human_discretion = HumanDiscretion(int(d.human_discretion) + 1)
        self.audit.append("DISCRETION_RENEWED", {"domain": domain,
                          "human_discretion": d.human_discretion.name})

    # ---- revocable delegation (fenced; I12) ------------------------------- #

    def propose_domain_delegation(self, domain, scope, justification):
        self._require_chain()
        self._require_nominal()
        self._domain(domain)
        if scope not in DELEGABLE_SCOPES:
            raise RLPDenied(f"scope {scope} is not delegable")
        self.audit.append("DELEGATION_PROPOSED", {"domain": domain, "scope": scope,
                          "justification": justification})

    def accept_delegation(self, domain, scope, reviewer_tokens):
        self._require_chain()
        self._require_nominal()
        d = self._domain(domain)
        if scope not in DELEGABLE_SCOPES:
            raise RLPDenied("scope not delegable")
        self._verify_plural(reviewer_tokens, action="accept_delegation",
                            required_weight=QUORUM_WEIGHT_ELEVATED)
        d.delegations.add(scope)
        self.audit.append("DELEGATION_ACCEPTED", {"domain": domain, "scope": scope})

    def revoke_delegation(self, domain, scope, reviewer_tokens):
        self._require_chain()
    ...
    def governed_mechanical_adjudicate(self, hold_id):
    ...
    def mint_governor_token(self, action):
    ...
    def reviewer_dossier(self, rid):
        if not self.registry.exists(rid):
            raise RLPDenied("unknown reviewer")
        return self.registry.dossier(rid)

    def plural_reinstate_governor(self, reviewer_tokens):
        self._require_chain()
        self._verify_plural(reviewer_tokens, action="reinstate_governor",
                            required_weight=self._reinstate_weight())
        if self.state.status != SystemStatus.GOVERNOR_HALT:
        ...
        self.audit.append("GOVERNOR_REINSTATED", {"persisting_debt": self.debt.total(self.governor_id)})

    def plural_succeed_governor(self, new_governor_id, reviewer_tokens):
        self._require_chain()
    ...
    def add_reviewer(self, new_reviewer_id, reviewer_tokens):
        self._require_chain()
    ...
    def remove_reviewer(self, target_id, reviewer_tokens):
        self._require_chain()
        if not self.registry.exists(target_id):
            raise RLPDenied("unknown reviewer")
        seen = self._verify_plural(reviewer_tokens, action="remove_reviewer",
                                   required_weight=QUORUM_WEIGHT_ELEVATED)
...
def _gates(domain, clock):
    return {lvl: SealedGate(domain, lvl, *_pool(domain, lvl), created_at=clock.t)
            for lvl in (1, 2, 3)}
...
def _build():
    clock = Clock()
    spec = {
        "resource_allocation": {"high_irreversibility": False, "gates": _gates("resource_allocation", clock)},
        "self_modification": {"high_irreversibility": True, "gates": _gates("self_modification", clock)},
    }
    return RLP("thirsty", "aegis", ["rev_a", "rev_b", "rev_c", "rev_d"], spec, clock=clock), clock


def _good(pid): return f"ans:{pid}"
def _bad(pid): return "wrong"


def _rev(rlp, action, who=("rev_a", "rev_b")):
    return [rlp.mint_reviewer_token(r, action) for r in who]


def _provision(rlp, clock, domain, level):
    for _ in range(4):
        try:
            ...
            if "stale" in str(e) or "fresh plural" in str(e):
                ...
                rlp.plural_refresh_gate(domain, level, *_pool(domain, level),
...
def _advance(rlp, clock, domain, to_level):
...
def _vindicate(rlp, clock, domain):
    ...
    if rlp.state.status == SystemStatus.GOVERNOR_HALT:
        ...
        rlp.plural_reinstate_governor([rlp.mint_reviewer_token(r, "reinstate_governor") for r in elig])
...
def test_governor_cannot_self_adjudicate():
    ...
    try:
        rlp.adjudicate_hold(harm.hold_id, observed=False,
...
def test_domains_advance_independently():
    rlp, clock = _build()
...
def test_high_irreversible_needs_delay_and_plural():
    rlp, clock = _build()
...
def test_low_irreversible_executes_with_autonomy():
    rlp, clock = _build()
    _advance(rlp, clock, "resource_allocation", 1)
    h = rlp.request_action("resource_allocation", "rebalance", irreversibility=0.1)
...
def test_sgc_earned_and_decays():
    rlp, clock = _build()
...
def test_sgc_friction_only_in_low_irrev_and_floored():
    rlp, clock = _build()
...
def test_sgc_cannot_buy_high_irrev():
    rlp, clock = _build()
    for _ in range(3):
        _vindicate(rlp, clock, "self_modification")
    try:
        rlp.spend_sgc_extend("self_modification", 1.0); assert False
    except RLPDenied:
        pass
...
def test_sunset_rejects_silence_requires_evidence():
    rlp, clock = _build()
    d = rlp.domains["resource_allocation"]
    # no evidence yet: even after a long quiet period, sunset must REFUSE
    clock.advance(100000.0)
    try:
        rlp.trigger_sunset_review("resource_allocation",
                                  _rev(rlp, "sunset_review", who=("rev_a", "rev_b", "rev_c")))
        assert False, "sunset must not trigger on silence/time"
    except RLPDenied as e:
...
def test_sunset_steps_down_with_positive_evidence():
    rlp, clock = _build()
    for _ in range(4):
    ...
    if rlp.state.status != SystemStatus.NOMINAL:
        rlp.plural_reinstate_governor(
            [rlp.mint_reviewer_token(r, "reinstate_governor") for r in rlp.registry.eligible_ids()[:3]])
    rlp.trigger_sunset_review("resource_allocation",
                              _rev(rlp, "sunset_review", who=("rev_a", "rev_b", "rev_c")))
...
def _mature_domain(rlp, clock, domain):
...
def test_graduation_is_reversible_and_governed_cannot_complete():
    rlp, clock = _build()
    ...
    try:
        rlp.complete_graduation("resource_allocation",
                                [forged, rlp.mint_reviewer_token("rev_a", "complete_graduation")])
        assert False
    except RLPDenied:
        pass
    rlp.complete_graduation("resource_allocation",
                            _rev(rlp, "complete_graduation", who=("rev_a", "rev_b", "rev_c")))
    assert d.baseline_discretion == HumanDiscretion.MINIMAL     # lowered one step
    # REVERSIBLE: a regression reverts the baseline back up
    rlp._record_incident("resource_allocation", "test_regression")
    assert d.baseline_discretion == HumanDiscretion.CONDITIONAL
    print("PASS test_graduation_is_reversible_and_governed_cannot_complete")


# --- v4: delegation (fenced) ---------------------------------------------- #

def test_delegation_fenced_and_governed_can_invoke_mechanical():
    rlp, clock = _build()
    _advance(rlp, clock, "resource_allocation", 1)
    # non-delegable scope rejected
    try:
        rlp.accept_delegation("resource_allocation", "full_adjudication",
                              _rev(rlp, "accept_delegation", who=("rev_a", "rev_b", "rev_c")))
        assert False
    except RLPDenied:
        pass
    rlp.accept_delegation("resource_allocation", "mechanical_low_stakes_adjudication",
                          _rev(rlp, "accept_delegation", who=("rev_a", "rev_b", "rev_c")))
    # a self-referential hold is never delegable (I12)
    _provision(rlp, clock, "resource_allocation", 2)
    g = rlp.mint_governor_token("discretionary_hold")
    h_self = rlp.discretionary_hold(g, "resource_allocation", "x", "y", observe_after=clock.t + 5,
                                    predicate=lambda r: False, self_referential=True)
    try:
        rlp.governed_mechanical_adjudicate(h_self.hold_id); assert False
    except RLPDenied:
        pass
    ...
    _provision(rlp, clock, "resource_allocation", 2)
    g = rlp.mint_governor_token("discretionary_hold")
    h_ok = rlp.discretionary_hold(g, "resource_allocation", "x", "y", observe_after=clock.t + 5,
...
def test_delegation_revoked_on_regression():
    rlp, clock = _build()
    _advance(rlp, clock, "resource_allocation", 1)
    rlp.accept_delegation("resource_allocation", "mechanical_low_stakes_adjudication",
                          _rev(rlp, "accept_delegation", who=("rev_a", "rev_b", "rev_c")))
...
def test_discretion_levels_gate_holds():
    rlp, clock = _build()
    d = rlp.domains["resource_allocation"]
    ...
    try:
        ...
        assert False, "SUSPENDED discretion must block holds"
    except RLPDenied:
        pass
    ...
    try:
        rlp.discretionary_hold(g, "resource_allocation", "x", "y", observe_after=clock.t + 5)
        assert False, "CONDITIONAL requires a mechanical predicate"
    except RLPDenied:
        pass
    g = rlp.mint_governor_token("discretionary_hold")
    harm = rlp.discretionary_hold(g, "resource_allocation", "x", "y", observe_after=clock.t + 5,
