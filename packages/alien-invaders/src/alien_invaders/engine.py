...
from alien_invaders.modules.causal_clock import (
    CausalClock,
    CausalEvent,
    EventQueue,
)
...
from alien_invaders.modules.world_state import (
    Country,
    GlobalState,
    SimulationEvent,
    ValidationState,
        ...
        if self.state.alien_ground_forces > 0:
            for country in self.state.countries.values():
                if country.alien_influence > 0.1:
                    # Combat casualties
                    casualty_rate = (
                        0.0001 * country.alien_influence * self.state.alien_ground_forces
                    )
