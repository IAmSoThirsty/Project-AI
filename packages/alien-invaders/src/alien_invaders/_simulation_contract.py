...
imports in the alien_invaders engine
(``from alien_invaders._simulation_contract import ...``)
...
- The local ``RegistryAccessRequest`` dataclass
  (which was the duplicate of
  ``alien_invaders.modules.planetary_defense_monolith.\
RegistryAccessRequest``) was moved to
  ``simulation_contract.contract``. The monolith now
  imports ``RegistryAccessRequest`` from the shared
