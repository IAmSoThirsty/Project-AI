...
for the J2 ai_takeover scenario engine port. Same contract as the
J2 global_scenario port; both engines share the simulation
contract family. Follow-up: promote to a shared
``packages/simulation-contract/`` package and have both
``ai_takeover`` and ``global_scenario`` depend on it. The legacy module is the
