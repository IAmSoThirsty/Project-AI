...
#
#   - Legacy source: T:\\00-Active\\Project-AI-main\\engines\\global_scenario\\
#   - Canonical target: packages/global-scenario/src/global_scenario/
#   - Legacy import ``from app.core.simulation_contingency_root import
#     (...)`` is replaced with ``from global_scenario._simulation_contract
#     import (...)`` (the 446-LOC root is vendored as
#     global_scenario._simulation_contract for the same family
#     of engines).
        ...
        if cache_file.exists():
            try:
                with open(cache_file) as f:
                    ...
                    if datetime.fromisoformat(data["timestamp"]) > datetime.now(UTC) - timedelta(
                        days=30
                    ):
