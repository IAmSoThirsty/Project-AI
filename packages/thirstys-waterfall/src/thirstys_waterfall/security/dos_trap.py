    ...
    def get_syscall_table_hash(self) -> bytes | None:
        ...
        try:
            if self._is_linux:
                ...
                # SSDT inspection requires a separately installed and trusted
                # kernel-mode collector. Do not fabricate a stable hash that
                # would make hook detection appear active.
                self.logger.warning("Windows syscall-table measurement is unavailable")
                return None
...
class InterfaceDisabler:
    ...
    def disable_network_interfaces(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Disabled {len(self._disabled_interfaces)} network interfaces")
            return True
                                ...
                                for device in os.listdir(driver_path):
                                    if ":" in device:
                                        with open(unbind_path, "w") as f:
                                            f.write(device)
                                        self.logger.info(f"Unbound USB device: {device}")
                            except Exception as e:
...
class DOSTrapMode:
    ...
    def _execute_action(self, action: ResponseAction):
        ...
        try:
            ...
            elif action == ResponseAction.SHUTDOWN and self.config["emergency_shutdown"]:
