...
class PrivacyRiskEngine:
    ...
    def _classify_event(
        ...
        if event_type == "auth_failure" and self._metrics["failed_auth_attempts"] > 5:
            return ThreatEvent(
                timestamp=time.time(),
    ...
    def _analyze_behavior_patterns(self):
        ...
        with self._lock:
            ...
            recent_connections = [
                ts for ts in self._metrics["connection_count"] if current_time - ts < 60.0
            ]
    ...
    def _update_risk_level(self):
        ...
        with self._lock:
            ...
            if not recent_threats:
                ...
                critical_count = sum(
                    1 for t in recent_threats if t.risk_level.value >= RiskLevel.CRITICAL.value
                )

                new_level = RiskLevel.EXTREME if critical_count >= 3 else RiskLevel(max_risk)
            ...
            if new_level.value > self._current_risk_level.value:
                ...
                self.logger.warning(f"Risk level escalated: {old_level.name} -> {new_level.name}")
