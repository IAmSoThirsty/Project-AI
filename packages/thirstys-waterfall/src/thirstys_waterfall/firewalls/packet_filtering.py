...
class PacketFilteringFirewall(FirewallBase):
    ...
    def _load_default_rules(self):
        ...
        self.add_rule({"id": "default_established", "action": "allow", "state": "established"})
    ...
    def _match_rule(self, packet: dict[str, Any], rule: dict[str, Any]) -> bool:
        ...
        # Match source IP
        if "src_ip" in rule and not self._match_ip(packet.get("src_ip"), rule["src_ip"]):
            return False

        # Match destination IP
        if "dst_ip" in rule and not self._match_ip(packet.get("dst_ip"), rule["dst_ip"]):
            return False
        ...
        # Match state
        return not ("state" in rule and packet.get("state") != rule["state"])
    ...
    def _match_ip(self, ip: str, rule_ips) -> bool:
        ...
        for rule_ip in rule_ips:
            if ip == rule_ip or (
                rule_ip.endswith("/8") and ip.startswith(rule_ip.split("/")[0].rsplit(".", 3)[0])
            ):
                return True
