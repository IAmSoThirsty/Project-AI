...
class NftablesBackend(FirewallBackend):
    ...
    def add_rule(self, rule: dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Execute nft command
            cmd = ["sudo", "nft", "add", "rule", "ip", self.table_name, self.chain_name, *nft_rule]
...
class WindowsFirewallBackend(FirewallBackend):
    ...
    def check_availability(self) -> bool:
        ...
        try:
            result = subprocess.run(["where", "netsh"], capture_output=True, timeout=5, shell=True)
            return result.returncode == 0
        except Exception as e:
...
class FirewallBackendFactory:
    ...
    @staticmethod
    def create_backend(config: dict[str, Any] | None = None) -> FirewallBackend | None:
        """
