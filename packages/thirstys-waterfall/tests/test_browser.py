...
class TestEncryptedSearchEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.search_engine = EncryptedSearchEngine(
            self.cipher,
            provider=lambda _query: self.cipher.encrypt(b'{"status":"ok","results":[]}'),
        )
    ...
    def test_search_caching_encrypted(self):
        ...
        for key in self.search_engine._encrypted_cache:
            self.assertIsInstance(key, str)  # Hash strings as cache keys

    def test_missing_search_provider_is_explicitly_unavailable(self):
        engine = EncryptedSearchEngine(self.cipher)
        engine.start()
        try:
            result = engine.search("query")
            self.assertEqual(result["status"], "unavailable")
            payload = engine.decrypt_results(result["encrypted_results"])
...
class TestIncognitoBrowser(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = {
            ...
            "encrypted_search_provider": lambda _query: b"provider-encrypted-results",
        }
...
class TestBrowserSandbox(unittest.TestCase):
    ...
    def test_sandbox_reports_unavailable_os_boundaries(self):
        """The compatibility surface does not claim OS-level isolation."""
        self.sandbox.start()
        ...
        self.assertFalse(boundaries["filesystem_isolation"])
        self.assertFalse(boundaries["network_restrictions"])
