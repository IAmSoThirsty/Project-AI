...
class TestMicroVMInstance(unittest.TestCase):
    ...
    def test_vm_start_attempt(self, mock_popen):
        ...
        with (
            patch.object(self.vm._channel, "connect", return_value=True),
            patch.object(self.vm._channel, "send_message", return_value=True),
            patch.object(self.vm._channel, "receive_message", return_value={"type": "pong"}),
        ):
...
class TestMicroVMIsolationManager(unittest.TestCase):
    ...
    def test_custom_vm_id(self):
        ...
        vm_id = self.manager.create_vm(isolation_type=IsolationType.SESSION, vm_id=custom_id)

        self.assertEqual(vm_id, custom_id)

        # Should not be able to create duplicate
        vm_id2 = self.manager.create_vm(isolation_type=IsolationType.SESSION, vm_id=custom_id)
