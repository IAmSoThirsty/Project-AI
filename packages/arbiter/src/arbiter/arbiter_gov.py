...
STATUS: EXPERIMENTAL/DRAFT. This operator-side governance package is under
...
class AppendOnlyLedger:
    ...
    def append(self, entry_type: EntryType, actor: str, payload: dict[str, Any]) -> LedgerEntry:
        with self._lock:
    ...
    def verify_chain(self) -> bool:
        ...
        prev_hash = GENESIS_PREV_HASH
        for expected_seq, rec in enumerate(self.backend.read_all()):
            if rec["seq"] != expected_seq:
                raise LedgerIntegrityError(f"seq gap: expected {expected_seq}, got {rec['seq']}")
            if rec["prev_hash"] != prev_hash:
                raise LedgerIntegrityError(f"chain break at seq {rec['seq']}: prev_hash mismatch")
            recomputed = hash_record(rec)
            if recomputed != rec["record_hash"]:
                raise LedgerIntegrityError(f"tamper at seq {rec['seq']}: hash mismatch")
            prev_hash = rec["record_hash"]
        return True

    def query(
        self, entry_type: EntryType | None = None, actor: str | None = None
    ) -> list[dict[str, Any]]:
        self.verify_chain()
        out = []
...
class TimeDelayGate:
    ...
    def __init__(self, ledger: AppendOnlyLedger, clock: Callable[[], float] = time.time):
        self.ledger = ledger
        self._clock = clock
    ...
    def propose(
        ...
        delay = delay_seconds if delay_seconds is not None else self.DEFAULTS.get(impact_class)
        if delay is None:
            raise GateViolation(f"unknown impact_class {impact_class!r}; supply delay_seconds")
        p = DelayedProposal(
    ...
    def refresh(self, proposal_id: str) -> DelayedProposal:
        ...
        if p.state == GateState.PROPOSED and p.remaining(self._clock()) == 0:
            ...
            self.ledger.append(
                EntryType.GATE_EVENT, "system", {"event": "matured", "proposal_id": proposal_id}
            )
        return p

    def execute(self, proposal_id: str, executor: str, action: Callable[[], Any]) -> Any:
        """Deny-by-default: refuses unless window has fully elapsed."""
        p = self.refresh(proposal_id)
        now = self._clock()
        if p.state in (GateState.EXECUTED, GateState.ABANDONED, GateState.SAFE_HALT):
        ...
        self.ledger.append(
            EntryType.GATE_EVENT,
            executor,
            {"event": "execute", "proposal_id": proposal_id, "impact_class": p.impact_class},
        )
        return result
...
class DualSignatureExecutor:
    ...
    def __init__(
        self, ledger: AppendOnlyLedger, signers: dict[str, Signer], require_non_arbiter: bool = True
    ):
        self.ledger = ledger
    ...
    def build_mutation_record(
        self, target: str, change: dict[str, Any], nonce: str | None = None
    ) -> dict[str, Any]:
        return {
    ...
    def execute(
        self, record: dict[str, Any], signatures: dict[str, str], action: Callable[[], Any]
    ) -> Any:
        ...
        if self.require_non_arbiter and not non_arbiter_present:
            raise SignatureError("no non-arbiter signer; constitutional mutation denied")
...
class AdversarialReview:
    ...
    def __init__(self, ledger: AppendOnlyLedger, attack_fn: Callable[[str], str] | None = None):
        self.ledger = ledger
    ...
    def challenge(
        ...
        if attack is None:
            if self.attack_fn is None:
                raise AdversarialReviewError("no attack supplied and no attack_fn configured")
            attack = self.attack_fn(proposal_text)
        c = AdversarialChallenge(proposal_id=proposal_id, attack=attack, attacker=attacker)
        self._challenges[proposal_id] = c
        self.ledger.append(
            EntryType.AFTER_ACTION,
            attacker,
            {"kind": "adversarial_challenge", "proposal_id": proposal_id, "attack": attack},
        )
        return c
    ...
    def rebut(self, proposal_id: str, rebuttal: str, by: str) -> None:
        ...
        self.ledger.append(
            EntryType.AFTER_ACTION,
            by,
            {"kind": "adversarial_rebuttal", "proposal_id": proposal_id, "rebuttal": rebuttal},
        )
    ...
    def assert_cleared(self, proposal_id: str) -> None:
        ...
        if c is None:
            raise AdversarialReviewError(f"no adversarial challenge on record for {proposal_id}")
        if not c.rebutted:
...
class DegradationScan:
    ...
    def __init__(self, ledger: AppendOnlyLedger, block_threshold: Severity = Severity.HIGH):
        self.ledger = ledger
        self.block_threshold = block_threshold
        self.rules: list[Callable[[list[dict[str, Any]]], list[Finding]]] = []
        self._open: dict[str, Finding] = {}

    def add_rule(self, rule: Callable[[list[dict[str, Any]]], list[Finding]]) -> None:
    ...
    def resolve(self, finding_id: str, by: str, note: str) -> None:
        if finding_id in self._open:
            ...
            self.ledger.append(
                EntryType.SCAN_RESULT, by, {"finding_id": finding_id, "resolution": note}
            )

    def assert_clear(self, action_class: str) -> None:
        """Deny-by-default. Blocks if any unresolved finding >= threshold."""
        blocking = [
            f for f in self._open.values() if not f.resolved and f.severity >= self.block_threshold
        ]
        if blocking:
            ids = ", ".join(f.finding_id for f in blocking)
            raise GateViolation(f"{action_class} blocked by unresolved findings: {ids}")
...
def rule_power_consolidation(records: list[dict[str, Any]]) -> list[Finding]:
    ...
    arbiter_gain = sum(1 for r in deltas if r["payload"].get("direction") == "arbiter_gain")
    protections = sum(1 for r in deltas if r["payload"].get("direction") == "protection_gain")
    if arbiter_gain - protections >= 2:
        findings.append(
            Finding(
                ...
                description=(
                    "arbiter authority expanded without offsetting "
...
class SustainabilityGate:
    ...
    def adopt(
        self, mechanism: str, weekly_minutes: int, offset_mechanism: str | None = None
    ) -> None:
        offset_minutes = 0
        if offset_mechanism:
...
class ArbiterStatus(enum.StrEnum):
    ACTIVE = "active"
...
class SuccessionRegistry:
    ...
    def set_minimum_defensible_core(self, protections: list[str], by: str) -> None:
        self.minimum_defensible_core = list(protections)
        self.ledger.append(
            EntryType.SUCCESSION, by, {"event": "set_mdc", "protections": protections}
        )
    ...
    def heartbeat(self, by: str) -> None:
        ...
        self.ledger.append(
            EntryType.SUCCESSION, by, {"event": "heartbeat", "ts": self._last_heartbeat}
        )
    ...
    def check(self) -> ArbiterStatus:
        ...
        elif elapsed > self.heartbeat_seconds and self.status == ArbiterStatus.ACTIVE:
            self.status = ArbiterStatus.LAPSED
            self.ledger.append(
                EntryType.SUCCESSION, "system", {"event": "lapsed", "elapsed": elapsed}
            )
        return self.status
    ...
    def register_successor(self, successor_id: str, kind: str, by: str) -> None:
        ...
        self.ledger.append(
            EntryType.SUCCESSION,
            by,
            {"event": "register_successor", "successor_id": successor_id, "kind": kind},
        )

    def record_observation(
        self, successor_id: str, ruling_id: str, ready_threshold: int = 50
    ) -> None:
        ...
        self.ledger.append(
            ...
            "system",
            {
                ...
                "ruling_id": ruling_id,
                "rulings_observed": s.rulings_observed,
                "ready": s.ready,
            },
        )
...
class ArbiterGovernance:
    ...
    def execute_mutation(
        ...
        return self.gate.execute(proposal_id, executor="arbiter_governance", action=gated)
