...
class RuntimePolicyEngine:
    def __init__(
        ...
        cel_runtime: Any | None = None,
    ):
        self.manifest = manifest
        self.trusted_keys = trusted_keys
        self.nonce_store = nonce_store or InMemoryNonceStore()
        # ``cel_runtime`` may be injected (e.g. a CEL-free stub when cel-python is
        # absent). Defaults to the real CELRuntime, preserving upstream behavior.
        self.cel = cel_runtime if cel_runtime is not None else CELRuntime()
    ...
    def gate_action(
        ...
        *,
        ...
        if class_entry is None:
            return GateDecision(
                "deny", "Unknown action class fails closed.", action_class, ("Q-003-A",)
            )
        expected_class = self.action_type_classes.get(action_type)
        if expected_class is None:
            return GateDecision(
                "deny",
                "Unknown action type has no authoritative classification.",
                action_class,
                ("Q-003-A",),
            )
        ...
        if authority_proof is None:
            return GateDecision(
                "deny",
                "Missing authenticated authority proof.",
                action_class,
                ("Q-002-B", "Q-003-A"),
            )
        ...
        if (
            action.get("is_retry")
            and action.get("prior_outcome") == "unknown"
            and not action.get("external_state_verified")
        ):
            return GateDecision(
                "deny",
        ...
        if (
            ...
            return GateDecision(
                "deny",
                "State drift detected; expected revision does not match current revision.",
                action_class,
                ("Q-007-A",),
            )
        ...
        if rank >= 3:
            ...
            if approval_proof.get("action_id") not in (None, action.get("action_id")):
                return GateDecision(
                    "deny",
                    "Approval proof is bound to a different action ID.",
                    action_class,
                    ("Q-003-B",),
                )
            ...
            if self.nonce_store.contains(nonce):
                return GateDecision(
                    "deny",
                    "Approval proof nonce has already been consumed.",
                    action_class,
                    ("Q-003-B", "Q-007-B"),
                )
            if consume_approval:
        ...
        if rank == 1 and action.get("recovery_required") and not action.get("rollback_plan"):
            return GateDecision(
                "deny",
                "Reversible action lacks a credible rollback plan.",
                action_class,
                ("Q-003-C",),
            )
        ...
        return GateDecision(
            "allow",
            "Authenticated, scoped, and permitted by the action-class gate.",
            action_class,
            ("Q-002-B", "Q-003-A", "Q-003-B"),
        )
