...
def generate_keypair(
    key_id: str, principal_id: str, purposes: Iterable[str]
...
def sign_document(
    document: dict[str, Any], private_key_document: dict[str, Any], purpose: str
) -> dict[str, Any]:
...
def verify_signed_document(
    ...
    try:
        _public_key(key).verify(
            b64u_decode(signature["value"]), canonical_json_bytes(without_signature(document))
        )
