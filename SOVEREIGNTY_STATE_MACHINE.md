...
2. Generate completion seal
3. Append seal to ledger
4. Update system state to DEFENSE
5. Log transition to audit trail
...
4. Sign and append to override ledger
5. Update system state to SUSPENDED
6. Generate restoration plan
...
2. Verify violation resolution
3. Validate ledger integrity
4. Obtain dual confirmation
5. Update system state to ACTIVE
...
3. Copy all ledgers to archive
4. Generate new genesis seal
5. Derive new ORACLE_SEED
6. Generate new keypairs
7. Initialize new ledgers with genesis blocks
8. Link new genesis to old in metadata
9. Update system state to REFOUNDING
10. Log refoundation event
...
2. Verify new ORACLE_SEED derivation
3. Check new ledger integrity
4. Confirm stakeholder re-consent
5. Update system state to ACTIVE
6. Announce refoundation completion
...
2. Record violation (if applicable)
3. Maintain current state
4. Return error with specific failed precondition
5. Notify administrators

### State Corruption Detection
If state file is corrupted or inconsistent:
1. Load last valid state from ledger
2. Verify against ledger history
3. Reconstruct state from ledger if possible
4. Trigger invariant violation if reconstruction fails
5. Potential override if non-restorable
