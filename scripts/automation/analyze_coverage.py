...
def main():
    ...
    print(
        f"Function docstring coverage: {all_stats['with_docstring']}/{all_stats['total']} ({doc_pct:.1f}%)"
    )
    print(
        f"Type hint coverage: {all_stats['with_type_hints']}/{all_stats['total']} ({type_pct:.1f}%)"
    )
