...
class WikiLinkGenerator:
    ...
    def find_source_references(
        ...
        source_map = (
            self.SECURITY_SOURCE_MAP
            if doc_type == "security"
            else self.AGENT_SOURCE_MAP
        )
    ...
    def add_source_links_section(
        self, content: str, doc_path: str, doc_type: str
    ) -> str:
        """Add a 'Source Code References' section to documentation."""

        # Skip if section already exists
        if (
            "## Source Code References" in content
            or "## 📁 Source Code References" in content
        ):
            return content
        ...
        for source_path in sorted(unique_sources):
            ...
            self.link_mappings.append(
                LinkMapping(doc_path, source_path, "implementation")
            )
        ...
        for marker in insertion_markers:
            if marker in content:
                ...
                for i, line in enumerate(lines):
                    if marker in line:
                        content = "\n".join(
                            lines[:i] + relationship_section + lines[i:]
                        )
    ...
    def process_documentation_file(self, doc_path: Path, doc_type: str) -> bool:
        ...
        try:
            ...
            content = self.add_inline_wiki_links(
                content, str(doc_path.relative_to(self.repo_root)), doc_type
            )

            # Add source code references section
            content = self.add_source_links_section(
                content, str(doc_path.relative_to(self.repo_root)), doc_type
            )

            # Add relationship links for source-docs
            content = self.add_relationship_links(
                content, str(doc_path.relative_to(self.repo_root))
            )
    ...
    def generate_report(self) -> str:
        ...
        report_lines.extend(
            [
                ...
                self._check_quality_gate(
                    "Zero broken references", len(self.report.broken_links) == 0
                ),
                self._check_quality_gate(
                    "Minimum 200 links added", self.report.total_links_added >= 200
                ),
                self._check_quality_gate(
                    "Bidirectional navigation present",
        ...
        if self.report.total_links_added >= 200 and len(self.report.broken_links) == 0:
            ...
            report_lines.append("")
            report_lines.append(
                "All quality gates passed. Bidirectional wiki links successfully established."
            )
        else:
            report_lines.append("**STATUS:** ⚠️ IN PROGRESS")
            report_lines.append("")
            report_lines.append(
                f"Target: 400 links | Current: {self.report.total_links_added}"
            )
...
def main():
    ...
    if (
        generator.report.total_links_added >= 200
        and len(generator.report.broken_links) == 0
    ):
