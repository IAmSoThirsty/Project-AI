...
class LinkStatistics:
    ...
    def total_links(self) -> int:
        return (
            self.source_code_links
            + self.doc_to_doc_links
            + self.config_file_links
            + self.backlinks_added
        )
...
class WikiLinkGenerator:
    ...
    def _process_file_for_source_links(self, file_path: Path) -> int:
        ...
        # Web frontend files
        content = re.sub(
            r"`(web/frontend/src/[\w/]+\.(?:ts|tsx|js|jsx))`(?!\s*\[\[)",
            add_link,
            content,
        )
    ...
    def _add_backlinks(self) -> None:
        ...
        for source_dir, rel_dir in mappings:
            ...
            for doc_file in source_path.glob("*.md"):
                ...
                backlink_section += (
                    f"- **Relationship Map**: [[{rel_readme.relative_to(self.root)}]]\n"
                )
                ...
                for rel_file in rel_path.glob("*.md"):
                    ...
                    if any(kw in rel_base for kw in keywords):
                        ...
                        title = (
                            rel_file.stem.replace("-", " ").replace("_", " ").title()
                        )
    ...
    def _generate_report(self) -> None:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write("**Mission**: Infrastructure Code-to-Doc Links Specialist\n")
            ...
            if self.stats.broken_references:
                ...
                for broken in self.stats.broken_references[:50]:  # Limit to 50
                    ...
                    f.write(
                        f"  - **Reference**: `{broken.get('reference', broken.get('link', 'N/A'))}`\n"
                    )
            ...
            gates = {
                ...
                "Bidirectional navigation": self.stats.backlinks_added > 0
                and self.stats.see_also_sections > 0,
            }
            ...
            if all(gates.values()):
                ...
                f.write(
                    "All quality gates passed. Infrastructure documentation is fully cross-referenced.\n"
                )
            else:
        ...
        print("   ✅ Report saved to AGENT-075-LINK-REPORT.md")
