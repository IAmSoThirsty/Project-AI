...
def main():
    ...
    print("\n📊 OVERALL METRICS:")
    print(
        f"  Total Issues:  {original_stats['total']} → {new_stats['total']} (Δ {new_stats['total'] - original_stats['total']:+d})"
    )
    print(
        f"  HIGH severity: {original_stats['high']} → {new_stats['high']} (Δ {new_stats['high'] - original_stats['high']:+d})"
    )
    print(
        f"  MEDIUM:        {original_stats['medium']} → {new_stats['medium']} (Δ {new_stats['medium'] - original_stats['medium']:+d})"
    )
    print(
        f"  LOW:           {original_stats['low']} → {new_stats['low']} (Δ {new_stats['low'] - original_stats['low']:+d})"
    )
    ...
    print(
        f"  B602 (shell injection): {b602_before} → {b602_after} (Δ {b602_after - b602_before:+d})"
    )
    ...
    print(
        f"  B324 (weak MD5 hash):   {b324_before} → {b324_after} (Δ {b324_after - b324_before:+d})"
    )

    print("\n📈 ISSUE TYPE BREAKDOWN:")
    all_tests = set(original_stats["test_counts"].keys()) | set(
        new_stats["test_counts"].keys()
    )
    ...
    if new_stats["file_issues"]:
        ...
        for filepath in sorted(new_stats["file_issues"].keys()):
            ...
            severity_str = []
            if high:
                severity_str.append(f"{high}H")
            if medium:
                severity_str.append(f"{medium}M")
            if low:
                severity_str.append(f"{low}L")
