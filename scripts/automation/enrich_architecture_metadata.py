...
def extract_yaml_and_content(file_path: str) -> tuple[str | None, str | None, str]:
...
def map_related_systems(
    related_docs: list[str], uses: list[str], tags: list[str]
) -> list[str]:
...
def main():
    ...
    print(
        f"{Colors.BOLD}{Colors.CYAN}║  AGENT-010: P0 Architecture Metadata Enrichment         ║{Colors.RESET}"
    )
    print(
    ...
    if not arch_dir.exists():
        print(
            f"{Colors.RED}✗ Architecture directory not found: {arch_dir}{Colors.RESET}"
        )
        return
    ...
    print(
        f"{Colors.BLUE}📁 Processing {len(md_files)} architecture files...{Colors.RESET}\n"
    )

    results = []
    stats = {"created": 0, "enriched": 0, "up-to-date": 0, "error": 0, "skipped": 0}
    ...
    for file_path in md_files:
        ...
        if result["status"] == "created":
            ...
            print(
                f"  {Colors.GREEN}✓ Enriched: {', '.join(result['changes'])}{Colors.RESET}"
            )
