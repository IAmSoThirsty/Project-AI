...
class SecurityWikiLinker:
    ...
    def get_concepts_for_doc(self, doc_path: str) -> list[tuple[str, str, str]]:
        ...
        return [
            (row["id"], row["concept_name"], row["description"])
            for row in cursor.fetchall()
        ]

    def get_controls_for_concept(
        self, concept_id: str
        ...
        return [
            (row["id"], row["control_name"], row["link_type"], row["strength"])
            for row in cursor.fetchall()
        ]

    def add_implementation_section(
        self, content: str, concept_name: str, controls: list[tuple[str, str, str, str]]
    ) -> str:
    ...
    def find_concept_mentions(
        self, content: str, concept_name: str
    ) -> list[tuple[int, str]]:
    ...
    def add_inline_wiki_links(
        ...
        for control_id, control_name, _, strength in controls:
            ...
            wiki_link = self.control_link_map.get(
                control_id, f"[[{control_id}|{control_name}]]"
            )

            # Replace mentions of control name with wiki link
            # But be careful not to replace inside existing wiki links or code blocks
            pattern = re.compile(
                rf"(?<!\[\[)(?<!`)\b{re.escape(control_name)}\b(?!`|]])", re.IGNORECASE
            )

            new_content, count = pattern.subn(
                wiki_link, content, count=3
            )  # Limit to first 3 mentions
            if count > 0:
                content = new_content
                links_added += count
    ...
    def process_document(self, doc_path: str) -> tuple[str, int, int]:
        ...
        return (
            modified_content if modified_content != original_content else None,
            total_concepts,
            total_links,
        )
...
"""
        ...
        if unimplemented_rows:
            content += (
                "The following security concepts do not have implementing controls:\n\n"
            )
