...
class ComprehensiveLinkInjector:
    ...
    def _get_related_docs(self, directory: Path) -> list[tuple[str, str]]:
        ...
        if "relationships" in str(directory):
            ...
            if "data" in str(directory):
                docs.append(
                    ("source-docs/data-models/00-index.md", "Data Models Index")
                )
            elif "configuration" in str(directory):
                docs.append(
                    ("source-docs/configuration/INDEX.md", "Configuration Index")
                )
            elif "deployment" in str(directory):
                docs.append(
                    ("source-docs/deployment/README.md", "Deployment Documentation")
                )
            elif "integrations" in str(directory):
                docs.append(
                    ("source-docs/integrations/README.md", "Integration Documentation")
                )
            ...
            if "data" in str(directory):
                ...
                docs.append(
                    ("relationships/deployment/README.md", "Deployment Relationships")
                )
            elif "integrations" in str(directory):
                docs.append(
                    (
    ...
    def _find_related_modules(self, doc_file: Path, primary_module: str) -> list[str]:
        ...
        if "core" in primary_module:
            ...
            if core_dir.exists():
                ...
                if any(
                    x in doc_file.stem for x in ["ai", "persona", "memory", "learning"]
                ):
