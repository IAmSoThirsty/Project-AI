...
def get_git_creation_date(filepath: Path) -> str | None:
...
def format_frontmatter(metadata: dict) -> str:
    ...
    # Format as YAML
    yaml_str = yaml.dump(
        ordered_metadata, default_flow_style=False, sort_keys=False, allow_unicode=True
    )
    return f"---\n{yaml_str}---\n"
...
def process_file(filepath: Path, dry_run: bool = False) -> dict:
    ...
    result = {"file": filepath.name, "status": "unknown", "changes": [], "errors": []}
