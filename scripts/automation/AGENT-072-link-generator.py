...
class WikiLinkGenerator:
    ...
    def apply_links(self, mappings: list[LinkMapping]) -> int:
        ...
        for file_path, file_mappings in files_to_update.items():
            try:
                ...
                print(
                    f"Updated {file_path.relative_to(self.repo_root)} with {len(file_mappings)} links"
                )
            ...
            try:
                ...
                for link in wiki_links:
                    ...
                    if not target_path.exists():
                        broken_links.append(
                            (str(file_path.relative_to(self.repo_root)), link)
                        )
    ...
    def generate_report(self, validation: ValidationResult) -> str:
        ...
        for directory, count in sorted(validation.coverage_stats.items()):
            coverage_level = (
                "Excellent" if count > 50 else "Good" if count > 20 else "Moderate"
            )
...
def main():
    ...
    print(
        f"   {validation.total_links_added} wiki links across {validation.files_modified} files"
    )
    print(
        f"   {len(validation.broken_links)} broken links {'⚠️' if validation.broken_links else '✅'}"
    )
