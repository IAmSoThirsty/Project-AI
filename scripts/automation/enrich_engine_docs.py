...
class EngineDocMetadataEnricher:
    ...
    def classify_file(self, filepath: Path) -> dict:
        ...
        if "engines/emp_defense" in path_str:
            metadata.update(
                self._classify_emp_defense(filepath, content, first_100_lines)
            )
        elif "engines/ai_takeover" in path_str:
            metadata.update(
                self._classify_ai_takeover(filepath, content, first_100_lines)
            )
        elif "engines/django_state" in path_str:
            metadata.update(
                self._classify_django_state(filepath, content, first_100_lines)
            )
        elif "engines/alien_invaders" in path_str:
            metadata.update(
                self._classify_alien_invaders(filepath, content, first_100_lines)
            )
    ...
    def _classify_ai_takeover(self, filepath: Path, content: str, preview: str) -> dict:
        ...
        impl_status = (
            "complete"
            if "COMPLETE" in content or "CLOSED FORM" in content
            else "in-progress"
        )
    ...
    def _classify_django_state(
        self, filepath: Path, content: str, preview: str
    ) -> dict:
        ...
        impl_status = (
            "complete"
            if "COMPLETE" in content or "PRODUCTION READY" in content
            else "in-progress"
        )
    ...
    def _classify_kernel(self, filepath: Path, content: str, preview: str) -> dict:
        ...
        impl_status = (
            "complete"
            if ("COMPLETE" in content or "Day 1 Complete" in content)
            else "in-progress"
        )
    ...
    def _classify_tarl_os(self, filepath: Path, content: str, preview: str) -> dict:
        ...
        impl_status = (
            "complete"
            if ("100% COMPLETE" in content or "PRODUCTION READY" in content)
            else "in-progress"
        )
    ...
    def _classify_tarl(self, filepath: Path, content: str, preview: str) -> dict:
        ...
        impl_status = (
            "complete"
            if "Production" in content or "COMPLETE" in content
            else "in-progress"
        )
        ...
        for filepath in sorted(all_files):
            ...
            if metadata:
                if self.add_frontmatter(filepath, metadata):
                    success_count += 1
                    print("  [OK] Enriched with metadata")
                    ...
                    print("  [ERROR] Failed to enrich")
            else:
                print("  [SKIP] Skipped (see errors)")
            print()
...
"""
        ...
        for filepath, metadata in sorted(self.classification_map.items()):
            ...
            report += (
                f"| {rel_path} | {engine} | {status_icon} {status} | {doc_type} |\n"
            )
...
"""
        ...
        for component, engines in sorted(
            component_usage.items(), key=lambda x: len(x[1]), reverse=True
        ):
            if len(engines) > 1:
        ...
        for system in sorted(all_systems):
            engines_using = [
                eng
                for eng, systems in engine_relationships.items()
...
"""
        ...
        quality_gates = {
            "Engine types accurate": len(
                [
                    m
                    for m in self.classification_map.values()
                    if m.get("engine_type") != "unknown"
                ]
            ),
            "Implementation status reflects reality": len(
                [
                    m
                    for m in self.classification_map.values()
                    if m.get("implementation_status")
                    in ["complete", "in-progress", "planned"]
                ]
            ),
            "Languages correctly identified": len(
                [
                    m
                    for m in self.classification_map.values()
            ...
            "Component relationships mapped": len(
                [
                    m
                    for m in self.classification_map.values()
                    if m.get("related_systems")
                ]
            ),
        ...
        for gate, value in quality_gates.items():
            status = (
                "✅"
                if (gate == "Zero YAML errors" and value == 0)
                or (gate != "Zero YAML errors" and value > 0)
                else "❌"
            )
...
"""
    ...
    def _generate_completion_checklist(self) -> str:
        ...
        checklist_items = {
            "All engine docs enriched with metadata": len(self.enriched_files)
            == total_files,
            "Engine type classification complete": len(
                [
                    m
                    for m in self.classification_map.values()
                    if m.get("engine_type") != "unknown"
            ...
            == total_files,
            "Implementation status determined": len(
                [
                    m
                    for m in self.classification_map.values()
                    if m.get("implementation_status")
            ...
            == total_files,
            "Language identification complete": len(
                [
                    m
                    for m in self.classification_map.values()
                    if m.get("language") != "unknown"
