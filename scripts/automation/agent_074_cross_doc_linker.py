...
class CrossDocLinker:
    ...
    def _add_nav_links(self, source_doc: str, target_docs: list[str]):
        ...
        if (
            "## 📚 Related Documentation" in content
            or "## Related Documentation" in content
        ):
            # Section exists - add links to it
            section_pattern = (
                r"(## 📚 Related Documentation|## Related Documentation)\s*\n"
            )
            ...
            for target_doc in target_docs:
                # Get friendly name from file path
                friendly_name = (
                    Path(target_doc).stem.replace("_", " ").replace("-", " ").title()
                )
            ...
            # Insert section
            content = (
                content[:insertion_point] + section_text + content[insertion_point:]
            )
