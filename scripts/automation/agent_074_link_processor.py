...
class LinkProcessor:
    ...
    def _add_source_links(self, doc_path: Path, source_files: list[str], domain: str):
        ...
        if (
            "## 🔗 Source Code References" not in content
            and "## Source Code References" not in content
        ):
            ...
            # Insert the section
            content = (
                content[:insertion_point] + source_section + content[insertion_point:]
            )
        ...
        for component, source_file in component_refs.items():
            ...
            for pattern in patterns:
                ...
                if matches:
                    ...
                    content = (
                        content[: match.start()] + linked_text + content[match.end() :]
                    )
                    links_added += 1
        ...
        if content != original_content:
            ...
            print("    ⚠️  No changes made (links may already exist)")

    def _add_inline_code_references(
        self, doc_path: Path, content: str, domain: str
    ) -> int:
    ...
    def _add_source_code_backlinks(self):
        ...
        for source_file, doc_files in all_mappings.items():
            ...
            if (
                "📚 Documentation Links:" in content
                or "Documentation Links:" in content
            ):
            ...
            # Insert backlinks
            new_content = (
                content[:insertion_point] + backlink_section + content[insertion_point:]
            )
