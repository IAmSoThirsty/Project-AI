...
def main():
    ...
    stats, prereq_graph, impl_map, files_by_skill, files_by_framework = (
        analyze_metadata()
    )
    ...
    report.append("**Date:** 2026-04-20\n")
    report.append(f"**Total Files Processed:** {stats['total_files']}\n\n")

    report.append("---\n\n")
    report.append("## Executive Summary\n\n")
    report.append(
        f"Successfully added comprehensive YAML frontmatter metadata to **{stats['total_files']} developer documentation files** "
    )
    ...
    for fw, count in sorted(
        stats["by_framework"].items(), key=lambda x: x[1], reverse=True
    ):
    ...
    report.append("|-----------|-------|------------|\n")
    report.append(
        f"| Code Examples | {stats['with_code_examples']} | {stats['with_code_examples'] / total * 100:.1f}% |\n"
    )
    report.append(
        f"| API Reference | {stats['with_api_reference']} | {stats['with_api_reference'] / total * 100:.1f}% |\n"
    )
    report.append(
        f"| Has Prerequisites | {stats['with_prerequisites']} | {stats['with_prerequisites'] / total * 100:.1f}% |\n"
    )
    report.append(
        f"| Implements Code | {stats['with_implementations']} | {stats['with_implementations'] / total * 100:.1f}% |\n"
    )
    ...
    report.append("|-----------|-------|\n")
    for subdir, count in sorted(
        stats["by_subdirectory"].items(), key=lambda x: x[1], reverse=True
    ):
    ...
    report.append(
        f"- ✅ {stats['with_prerequisites'] / total * 100:.1f}% have prerequisite chains\n"
    )
    report.append(
        f"- ✅ {stats['with_implementations'] / total * 100:.1f}% map to source code\n"
    )
    report.append(
        f"- ✅ {stats['with_code_examples'] / total * 100:.1f}% include code examples\n"
    )
    report.append(
        f"- ✅ {stats['with_api_reference'] / total * 100:.1f}% are API references\n\n"
    )
    ...
    report.append(
        f"Successfully enhanced all {stats['total_files']} developer documentation files with production-grade "
    )
    report.append(
        "metadata. The documentation is now fully queryable, supports intelligent navigation, "
    )
    report.append(
        "and provides clear learning paths for developers at all skill levels.\n\n"
    )
    ...
    print(
        f"📊 Report length: {len(''.join(report))} characters ({len(''.join(report).split())} words)"
    )
