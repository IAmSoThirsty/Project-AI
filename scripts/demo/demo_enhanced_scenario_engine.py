...
# COMPLIANCE: Sovereign Substrate / demo_enhanced_scenario_engine.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / demo_enhanced_scenario_engine.py
...
def main():
    ...
    for country in ["USA", "CHN", "IND", "DEU", "BRA"]:
        ...
        print(
            f"  • {country}: {meta['development']} country, "
    ...
    print(
        f"  Countries with data: {len(set().union(*[data.keys() for data in engine.historical_data.values()]))}"
    )
    print(
    ...
    if scenarios:
        ...
        for i, scenario in enumerate(scenarios[:5], 1):
            ...
            print(
                f"     Likelihood: {scenario.likelihood:.1%}, Severity: {scenario.severity.value.upper()}"
            )
        print()
    ...
    if metrics["top_risks"]:
        ...
        for risk in metrics["top_risks"]:
            print(
                f"    • {risk['title']}: {risk['likelihood']:.1%} ({risk['severity']})"
            )
    print()
    ...
    print("  Original: 8 countries, 6 domains, 360 data points")
    print(
        f"  Enhanced: {validation['total_countries']} countries, "
        f"{validation['domains_loaded']} domains, "
        f"{validation['total_data_points']} data points"
    )
    print(
        f"  Improvement: {validation['total_countries'] / 8:.1f}x countries, "
