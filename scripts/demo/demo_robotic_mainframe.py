...
# COMPLIANCE: Sovereign Substrate / demo_robotic_mainframe.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / demo_robotic_mainframe.py
...
from app.core.robotic_hardware_layer import CommunicationProtocol, RobotConfiguration
...
def demo_system_initialization():
    ...
    if success:
        ...
        print(
            f"  - Triumvirate: {'ENABLED' if status.triumvirate_enabled else 'DISABLED'}"
        )
...
def demo_simple_motion(system: RoboticMainframeSystem):
    ...
    success = system.execute_motion(
        joint_targets=target_positions, duration=2.0, context={"is_user_order": True}
    )
...
def demo_api_usage():
    ...
    print(
        f"   Commands: {status.commands_executed} executed, {status.commands_rejected} rejected"
    )
