...
def check_hydra_integration():
    from app.security.asymmetric_enforcement_gateway import SecurityEnforcementGateway, OperationRequest, OperationType
...
def check_mobile_app_files():
    if not os.path.exists("android/app/src/main/java/com/projectai/app/MainActivity.java"):
        raise Exception("MainActivity.java missing")

def check_unity_files():
    if not os.path.exists("unity/ProjectAI/Scripts/VR/World/Presence/PresenceController.cs"):
