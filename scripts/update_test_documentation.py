...
class ArtifactParser:
    ...
    def parse_all_artifacts(self) -> OverallStats:
        ...
        for source_name, source_config in self.config.get(
            "artifact_sources", {}
        ).items():
            ...
            self.logger.info(
                f"Parsing {source_name}: {path_pattern} (format: {format_type})"
            )
    ...
    def _parse_unified_report(self, data: dict[str, Any], stats: OverallStats) -> None:
        ...
        self.logger.info(
            f"  Parsed unified report: {tests_run} tests, {tests_passed} passed"
        )

    def _parse_individual_report(
        self, data: dict[str, Any], stats: OverallStats, filepath: str
    ) -> None:
    ...
    def _parse_test_category(
        self, test_name: str, test_data: dict[str, Any], stats: OverallStats
    ) -> None:
        ...
        if "total_prompts" in metrics:
            ...
            cat_stats.failed += metrics.get("total_prompts", 0) - metrics.get(
                "blocked_count", 0
            )
    ...
    def _parse_test_execution_results(
        self, data: dict[str, Any], stats: OverallStats
    ) -> None:
        ...
        for test_result_data in data.get("test_results", []):
            ...
            test_result = TestResult(
                ...
                error_message=(
                    test_result_data.get("errors", [""])[0]
                    if test_result_data.get("errors")
                    else None
                ),
            ...
            try:
                ...
                if "tests" in data:
                    for test in data["tests"]:
                        category = self._extract_category_from_nodeid(
                            test.get("nodeid", "")
                        )
            ...
            try:
                ...
                for testsuite in root.findall(".//testsuite"):
                    ...
                    if suite_name not in stats.categories:
                        stats.categories[suite_name] = CategoryStats(
                            category=suite_name
                        )
    ...
    def _parse_markdown_report(self, pattern: str, stats: OverallStats) -> None:
        ...
        for filepath in files:
            try:
                ...
                self.logger.info(
                    f"  Parsed markdown: {total_match.group(1) if total_match else 0} tests"
                )
    ...
    def _calculate_statistics(self, stats: OverallStats) -> None:
        ...
        self.logger.info(
            f"Final statistics: {stats.total_tests} tests, {stats.passed} passed, {stats.failed} failed ({stats.pass_rate:.1%} pass rate)"
        )
...
class DocumentationUpdater:
    ...
    def update_all_documentation(self) -> None:
        ...
        for doc_target in self.config.get("documentation_targets", []):
            ...
            if any(
                filepath.lower().endswith(protected.lower())
                for protected in self.PROTECTED_FILES
            ):
                self.logger.warning(
                    f"⚠️  SKIPPING {filepath} - This file is PROTECTED and must only be modified by the repository owner"
                )
                self.logger.warning(
                    "⚠️  Attempting to modify this file would invoke the wrath of Thirsty!"
                )
                continue

            if not os.path.exists(filepath):
                if any(
                    section.get("create_if_missing", False)
    ...
    def _create_documentation_file(
        self, filepath: str, doc_target: dict[str, Any]
    ) -> None:
    ...
    def _update_documentation_file(
        self, filepath: str, doc_target: dict[str, Any]
    ) -> None:
    ...
    def _update_section(
        self, content: str, section_name: str, section_config: dict[str, Any]
    ) -> str:
        ...
        if markers:
            ...
            if start_marker in content and end_marker in content:
                ...
                new_section_content = self._generate_section_content(
    ...
    def _generate_test_badges(self) -> str:
        ...
        # Total tests badge
        total_color = (
            self.config["badge_colors"]["pass"]
            if self.stats.pass_rate >= 0.90
            else (
                self.config["badge_colors"]["warning"]
                if self.stats.pass_rate >= 0.80
                else self.config["badge_colors"]["fail"]
            )
        )

        badges.append(
            f"![Tests](https://img.shields.io/badge/tests-{self.stats.total_tests}-{total_color})"
        )

        # Passing tests badge
        badges.append(
            f"![Passing](https://img.shields.io/badge/passing-{self.stats.passed}-green)"
        )
        ...
        if self.stats.failed > 0:
            fail_color = (
                ...
                else self.config["badge_colors"]["warning"]
            )
            badges.append(
                f"![Failing](https://img.shields.io/badge/failing-{self.stats.failed}-{fail_color})"
            )
        ...
        badges.append(
            f"![Pass Rate](https://img.shields.io/badge/pass_rate-{pass_rate_pct}%25-{pass_rate_color})"
        )
        ...
        status_text = (
            "passing"
            if self.stats.pass_rate >= 0.90
            else "partial" if self.stats.pass_rate >= 0.80 else "failing"
        )
        status_color = (
            self.config["badge_colors"]["pass"]
            if self.stats.pass_rate >= 0.90
        ...
        badges.append(
            f"![Status](https://img.shields.io/badge/status-{status_text}-{status_color})"
        )
...
"""
        ...
        for category_name, cat_stats in sorted_categories:
            status_emoji = (
                "✅"
                if cat_stats.pass_rate >= 0.90
                else "⚠️" if cat_stats.pass_rate >= 0.80 else "❌"
            )
...
"""
        ...
        for category_name, cat_stats in sorted_categories:
            status = (
                "✅ Pass"
                if cat_stats.pass_rate >= 0.90
                else "⚠️ Partial" if cat_stats.pass_rate >= 0.80 else "❌ Fail"
            )
        ...
        has_failures = any(
            cat_stats.failed_tests for cat_stats in self.stats.categories.values()
        )
...
class GitCommitter:
    ...
    def commit_updates(self, updated_files: list[str]) -> bool:
        ...
        try:
            # Configure git
            subprocess.run(
                ["git", "config", "user.name", "Test Report Updater"], check=True
            )
            subprocess.run(
                ["git", "config", "user.email", "ci@project-ai.local"], check=True
            )
            ...
            commit_template = self.config.get("output", {}).get(
...
class SummaryReportGenerator:
    ...
    def __init__(
        self, config: dict[str, Any], stats: OverallStats, updated_files: list[str]
    ):
        self.config = config
        self.stats = stats
    ...
    def generate(self) -> None:
        ...
        report_path = self.config.get("output", {}).get(
            "summary_report_path", "test_documentation_update_summary.md"
        )
