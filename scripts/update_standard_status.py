...
# COMPLIANCE: Sovereign Substrate / update_standard_status.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / update_standard_status.py
...
class StandardStatusUpdater:
    ...
    def check_security_scans(self) -> dict[str, bool]:
        ...
        scans = {"bandit": False, "codeql": False, "trivy": False, "dependabot": False}
    ...
    def check_docker_security(self) -> dict[str, bool]:
        ...
        checks["pinned_base"] = bool(
            re.search(r"FROM .+:(?:\d+\.\d+(?:\.\d+)?|.*@sha256:)", content)
        )
        ...
        has_runtime_shell = any(
            "/bin/sh" in line or "/bin/bash" in line for line in entrypoint_cmd_lines
        )
    ...
    def check_documentation(self) -> dict[str, bool]:
        ...
        docs["architecture_diagram"] = (
            docs_dir / "PRODUCTION_ARCHITECTURE.md"
        ).exists()
        docs["threat_model"] = (
            docs_dir / "security_compliance" / "THREAT_MODEL.md"
        ).exists()
        docs["deployment_guide"] = (
            self.repo_root / "PRODUCTION_DEPLOYMENT.md"
        ).exists()
    ...
    def calculate_overall_status(self) -> dict:
        ...
        overall = {
            ...
            "overall_percentage": int(
                (coverage + security_pct + docker_pct + k8s_pct + docs_pct) / 5
            ),
            "categories": {
    ...
    def update_standard_document(self, status: dict):
        ...
        new_progress = (
            f"### 🎯 Overall Progress: {status['overall_percentage']}% Complete"
        )
