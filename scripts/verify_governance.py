...
class ScriptGovernanceVerifier:
    ...
    def get_script_classification(self, script_path: Path) -> tuple[str, bool]:
        ...
        try:
            ...
            if classification == "GOVERNED":
                ...
                has_implementation = (
                    "ADMIN-ONLY SCRIPT" in content or "display_admin_warning" in content
                )
            elif classification == "EXAMPLE":
                has_implementation = (
                    "EXAMPLE/DEMONSTRATION CODE" in content
                    or "DEMONSTRATION MODE" in content
                )
        ...
        for script_path in sorted(python_scripts):
            ...
            if expected is None:
                ...
                self.results["classification_mismatch"].append(
                    {"script": script_name, "expected": expected, "actual": actual}
                )
                ...
                if has_impl:
                    ...
                    print(
                        f"🔶 {script_name}: {actual} (marker only, no implementation)"
                    )
    ...
    def check_new_scripts(self) -> list[str]:
        ...
        python_scripts = [
            s.name for s in self.scripts_dir.glob("*.py") if s.name != "__init__.py"
        ]

        new_scripts = [s for s in python_scripts if s not in self.CLASSIFIED_SCRIPTS]
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"- **Compliance Rate**: {self.results['implemented'] / max(self.results['total_scripts'], 1) * 100:.1f}%"
        )
        ...
        if self.results["classification_mismatch"]:
            ...
            for item in self.results["classification_mismatch"]:
                report.append(
                    f"- {item['script']}: Expected {item['expected']}, found {item['actual']}"
                )
        ...
        if self.results["missing_implementation"]:
            ...
            for item in self.results["missing_implementation"]:
                report.append(
                    f"- {item['script']} ({item['expected']}): {item['issue']}"
                )
        ...
        if self.results["missing_implementation"]:
            report.append(
                "2. Implement governance patterns per IMPLEMENTATION_GUIDE.md"
            )
    ...
    def print_summary(self):
        ...
        print("Issues:")
        print(
            f"  Missing Classification:  {len(self.results['missing_classification'])}"
        )
        print(
            f"  Classification Mismatch: {len(self.results['classification_mismatch'])}"
        )
        print(
            f"  Missing Implementation:  {len(self.results['missing_implementation'])}"
        )
        print("=" * 60)

        # Exit code based on compliance
        total_issues = len(self.results["missing_classification"]) + len(
            self.results["classification_mismatch"]
        )
        ...
        if total_issues == 0:
            ...
            if len(self.results["missing_implementation"]) > 0:
                print(
                    f"⚠️  {len(self.results['missing_implementation'])} scripts need implementation"
                )
            return 0
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Verify script governance compliance")
    parser.add_argument(
        "--check-new", action="store_true", help="Check for new unclassified scripts"
    )
    parser.add_argument(
