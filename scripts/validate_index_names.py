...
def validate_index_filename(filename: str) -> tuple[bool, list[str]]:
    ...
    if len(filename) > 53:  # 50 + .md = 53
        errors.append(
            f"Maximum 50 characters (excluding .md), current: {len(filename) - 3}"
        )
...
def suggest_fixes(filename: str) -> list[str]:
    ...
    if (
        "domain" not in fixed
        and "type" not in fixed
        and "priority" not in fixed
...
Examples:
  ...
  %(prog)s path/to/_indexes/ --recursive      # Validate directory recursively
    ...
    parser.add_argument(
        "--fix", action="store_true", help="Interactively fix naming violations"
    )
    parser.add_argument(
        "--check-only", action="store_true", help="Only show failures, not passes"
    )
    parser.add_argument(
        "--recursive",
        action="store_true",
