...
# README.md must NEVER be moved, modified, or touched by automation
# Only the repository owner (Thirsty) may modify README.md
...
] + ABSOLUTELY_PROTECTED  # Add protected files to keep list
...
class HistoricalDocsOrganizer:
    ...
    def identify_obsolete_documents(self) -> list[Path]:
        ...
        for md_file in root_path.glob("*.md"):
            ...
            # ⚠️ CRITICAL: ABSOLUTE PROTECTION FOR README.md ⚠️
            if filename in ABSOLUTELY_PROTECTED:
                self.logger.warning(
                    f"⚠️⚠️⚠️ {filename} is ABSOLUTELY PROTECTED - NEVER TOUCH THIS FILE ⚠️⚠️⚠️"
                )
                self.logger.warning(
                    "⚠️⚠️⚠️ Only the repository owner may modify this file ⚠️⚠️⚠️"
                )
            ...
            if not is_obsolete:
                try:
                    ...
                    keyword_count = sum(
                        1 for keyword in HISTORICAL_KEYWORDS if keyword in content
                    )
            ...
            if not is_obsolete and is_old:
                ...
                if not is_useful:
                    is_obsolete = True
                    matched_pattern = (
                        f"age-based ({file_age_days} days old, not actively maintained)"
                    )
                    self.logger.info(
                        f"  Age assessment: {filename} is {file_age_days} days old and not actively maintained"
                    )
            ...
            if is_obsolete:
                ...
                if is_old:
                    self.logger.info(
                        f"  Keeping: {filename} ({file_age_days} days old but actively maintained/useful)"
                    )
                self.kept_files.append(filename)

        self.logger.info(
            f"Found {len(obsolete_docs)} obsolete documents, keeping {len(self.kept_files)}"
        )
    ...
    def _assess_usefulness(self, filepath: Path, filename: str) -> bool:
        ...
        try:
            ...
            # Count indicators
            useful_count = sum(
                1 for indicator in useful_indicators if indicator in content_lower
            )
            obsolete_count = sum(
                1 for indicator in obsolete_indicators if indicator in content_lower
            )
    ...
    def find_references(
        ...
        for md_file in Path(".").glob("*.md"):
            ...
            # ⚠️ NEVER SCAN OR MODIFY README.md ⚠️
            if md_file.name in ABSOLUTELY_PROTECTED:
                self.logger.info(
                    f"⚠️  Skipping {md_file.name} - This file is absolutely protected"
                )
                continue
            ...
            try:
                ...
                for line_num, line in enumerate(lines, 1):
                    for moved_filename in moved_filenames:
                        ...
                        if (
                            moved_filename in line
                            or moved_filename.replace(".md", "") in line
                        ):
                ...
                if file_references:
                    ...
                    self.logger.info(
                        f"  Found {len(file_references)} references in {md_file}"
                    )
    ...
    def update_references(self, references: dict[str, list[tuple[str, int]]]) -> None:
        ...
        for filepath, ref_list in references.items():
            if self.dry_run:
                self.logger.info(
                    f"  DRY RUN: Would update {len(ref_list)} references in {filepath}"
                )
            ...
            try:
                ...
                if content != original_content:
                    ...
                    self.logger.info(
                        f"  Updated {updates_made} references in {filepath}"
                    )
...
"""
        ...
        if self.age_checked_files:
            for filename, age_days in sorted(
                self.age_checked_files, key=lambda x: x[1], reverse=True
            ):
                status = (
                    "📦 Archived"
                    if any(filename in src for src, _ in self.moved_files)
                    else "✅ Kept (useful)"
                )
...
Examples:
  ...
  python organize_historical_docs.py --verbose
    ...
    parser.add_argument(
        "--interactive", "-i", action="store_true", help="Ask before moving each file"
    )
    parser.add_argument(
        "--check-age",
        action="store_true",
