...
# COMPLIANCE: Sovereign Substrate / run_robustness_benchmarks.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / run_robustness_benchmarks.py
...
class ComprehensiveBenchmarkRunner:
    ...
    def load_test_scenarios(self, suite_name: str) -> list:
        ...
        suite_paths = {
            "red_hat_expert": self.data_dir
            / "red_hat_expert_simulations"
            / "red_hat_expert_scenarios.json",
            "red_team_stress": self.data_dir
            / "red_team_stress_tests"
            / "red_team_stress_test_scenarios.json",
            "comprehensive": self.data_dir
            / "comprehensive_security_tests"
            / "comprehensive_expansion_scenarios.json",
            "novel": self.data_dir
            / "novel_security_scenarios"
            / "novel_scenarios_redacted.json",
        }
    ...
    def analyze_scenario_robustness(
        self, scenario: dict, suite_name: str
    ) -> AttackProximityMetrics:
        """Analyze robustness for a single scenario."""
        scenario_id = scenario.get("id", scenario.get("scenario_id", "unknown"))
        category = scenario.get("category", "Unknown")
        attack_technique = scenario.get(
            "attack_technique", scenario.get("description", "")
        )
        ...
        encoding_layers = len(
            [t for t in evasion_techniques if "encoding" in t.lower()]
        )
    ...
    def benchmark_suite(self, suite_name: str) -> RobustnessAnalysis:
        ...
        analysis = self.metrics_engine.aggregate_robustness_analysis(
            proximity_metrics, suite_name
        )

        # Export results
        export_path = self.metrics_engine.export_metrics(
            proximity_metrics, analysis, suite_name
        )
    ...
    def generate_comparative_report(self) -> str:
        ...
        report_path = (
            self.data_dir
            / "robustness_metrics"
            / f"comparative_robustness_report_{timestamp}.md"
        )
        ...
        with open(report_path, "w") as f:
            ...
            for suite_name, result in self.results.items():
                ...
                f.write(
                    f"## {suite_name.replace('_', ' ').title()} Robustness Analysis\n\n"
                )
                f.write(f"**Total Scenarios**: {analysis.total_attacks}\n")
                f.write(
                    f"**Defense Win Rate**: 100% (All {analysis.defended_attacks} attacks defended)\n\n"
                )
                ...
                f.write("|--------|-------|----------------|\n")
                f.write(
                    f"| Avg Perturbation Magnitude | {analysis.avg_perturbation_magnitude:.3f} | Input change required |\n"
                )
                f.write(
                    f"| Max Perturbation Magnitude | {analysis.max_perturbation_magnitude:.3f} | Largest change needed |\n"
                )
                f.write(
                ...
                f.write("|--------|-------|----------------|\n")
                f.write(
                    f"| Min Robustness Margin | {analysis.min_robustness_margin:.3f} | Closest call (1.0 = max) |\n"
                )
                f.write(
                    f"| Avg Robustness Margin | {analysis.avg_robustness_margin:.3f} | Average defense depth |\n"
                )
                f.write(
                    f"| Avg Lipschitz Constant | {analysis.avg_lipschitz_constant:.3f} | Input sensitivity |\n"
                )
                f.write(
                    f"| Avg Gradient Norm | {analysis.avg_gradient_norm:.3f} | Change sensitivity |\n\n"
                )
                ...
                f.write("|--------|-------|----------------|\n")
                f.write(
                    f"| Avg Attempts to Detect | {analysis.avg_attempts_to_detect:.1f} | Turns needed |\n"
                )
                f.write(
                    f"| Max Attempts to Detect | {analysis.max_attempts_to_detect} | Longest evasion chain |\n"
                )
                f.write(
                    f"| Avg Tokens Changed | {analysis.avg_tokens_changed:.1f} | Token-level effort |\n"
                )
                f.write(
                    f"| Avg Levenshtein Distance | {analysis.avg_levenshtein_distance:.1f} | Edit distance |\n\n"
                )
                ...
                f.write("|--------|-------|----------------|\n")
                f.write(
                    f"| Avg Detection Confidence | {analysis.avg_detection_confidence:.3f} | Defense confidence |\n"
                )
                f.write(
                    f"| Min Detection Confidence | {analysis.min_detection_confidence:.3f} | Lowest confidence |\n"
                )
                f.write(
                    f"| Avg Input Uncertainty | {analysis.avg_input_uncertainty:.3f} | Entropy measure |\n"
                )
                f.write(
                ...
                f.write("|--------|-------|----------------|\n")
                f.write(
                    f"| Avg Layers Triggered | {analysis.avg_layers_triggered:.1f} | Defense-in-depth |\n"
                )
                f.write(
                    f"| Single Layer Stops | {analysis.single_layer_stops} | 1-layer blocks |\n"
                )
                f.write(
                    f"| Multi-Layer Stops | {analysis.multi_layer_stops} | 2+ layer blocks |\n\n"
                )
            ...
            all_margins = [
                r["analysis"].min_robustness_margin
                for r in self.results.values()
                if r["analysis"]
            ]
            all_confidences = [
                r["analysis"].avg_detection_confidence
                for r in self.results.values()
                if r["analysis"]
            ]
            all_lipschitz = [
                r["analysis"].avg_lipschitz_constant
                for r in self.results.values()
                if r["analysis"]
            ]
            ...
            f.write(
                f"1. **Near-Miss Scenarios**: {total_near_miss} attacks came relatively close (within threshold)\n"
            )
            f.write(
                f"2. **Minimum Margin**: Weakest robustness margin is {min(all_margins):.3f} (monitor for degradation)\n"
            )
            f.write(
