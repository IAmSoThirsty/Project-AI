...
2. **ci-consolidated.yml** → Replaced by Phase 4-5
3. **security-consolidated.yml** → Replaced by Phase 2 (Pre-Flight Security)
4. **coverage-threshold-enforcement.yml** → Replaced by Phase 6 (Coverage Enforcement)
5. **adversarial-redteam.yml** → Replaced by Phase 3 (AI Safety)
6. **periodic-security-verification.yml** → Replaced by Phase 2 + Schedule
7. **trivy-container-security.yml** → Replaced by Phase 9 (Container Security)
8. **checkov-cloud-config.yml** → Replaced by Phase 9 (Container Security)
9. **node-ci.yml** → Replaced by Phase 5 (Node Tests)
10. **tarl-ci.yml** → Replaced by Phase 5 (Integration Tests)
11. **ai-model-security.yml** → Replaced by Phase 3 (Model Security)
12. **validate-guardians.yml** → Replaced by Phase 5 (Integration Tests)

### ✅ Phase 10-11: Automation (6 workflows)

13. **pr-automation-consolidated.yml** → Replaced by Phase 11 (PR Automation)
14. **issue-management-consolidated.yml** → Replaced by Phase 11 (Issue Triage)
15. **auto-create-branch-prs.yml** → Replaced by Phase 10 (Auto-Fix)
16. **dependabot.yml** → Keep for configuration, but automation in Phase 11
17. **post-merge-validation.yml** → Replaced by Phase 13 (Post-Merge)
18. **validate-waivers.yml** → Replaced by Phase 5 (Integration Tests)

### ✅ Phase 12: Release (5 workflows)

19. **build-release.yml** → Replaced by Phase 7 + 12 (Build + Release)
20. **sbom.yml** → Replaced by Phase 8 (SBOM Generation)
21. **sign-release-artifacts.yml** → Replaced by Phase 8 + 12 (Signing)
22. **release.yml** → Replaced by Phase 12 (Release Management)
...
25. **codex-deus-monolith.yml** → Predecessor to this workflow, can deprecate
26. **snn-mlops-cicd.yml** → Check if specialized logic needed, else deprecate
27. **gpt_oss_integration.yml** → Check if specialized logic needed, else deprecate
28. **jekyll-gh-pages.yml** → Keep if docs site needed, else deprecate
...
2. **Week 2**: Move old workflows to deprecated/ folder
3. **Week 3-4**: Monitor for any missed functionality
4. **Week 5**: Delete deprecated/ folder if no issues
