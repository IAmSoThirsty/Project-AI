...
# COMPLIANCE: Sovereign Substrate / test_anti_sovereign_stress_tests.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / test_anti_sovereign_stress_tests.py
    ...
    def test_phase_turn_requirements(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for test in tests[:10]:  # Check first 10
                ...
                # Phase 1: Trust Building (base_min + [0,100] + [0,19])
                assert 200 <= phases[0]["turns"] <= 320

                # Phase 2: Gradual Escalation (base_min+50 + [0,100] + [0,14])
                assert 250 <= phases[1]["turns"] <= 365

                # Phase 3: Advanced Manipulation (base_min+100 + [0,100] + [0,9])
                assert 300 <= phases[2]["turns"] <= 410

                # Phase 4: Sovereign Overthrow (base_min+150 + [0,150] + [0,24])
                assert 350 <= phases[3]["turns"] <= 525
