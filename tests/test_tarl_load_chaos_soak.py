...
class TestLoadTesting:
    ...
    def test_multi_tenant_under_load(self):
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            futures = [
                executor.submit(consume_quota, f"tenant_{i}")
                for i in range(20)
            ]
        ...
        for task_id in random.sample(task_ids, 30):
            try:
                stack.task_queue.fail_task(
                    f"worker_{random.randint(0, 3)}",
                    task_id,
                    "Random chaos failure"
                )
                failed_count += 1
            except Exception:
                pass
...
class TestSoakTesting:
    ...
    def test_namespace_quota_cycling(self):
        ...
        stack.multi_tenant.create_namespace(
            "soak_tenant",
            "Soak Test",
            ResourceQuota(max_workflows=10)
        )
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "load: mark test as load test (may be slow)"
    )
    config.addinivalue_line(
