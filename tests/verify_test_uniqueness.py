...
def verify_uniqueness():
    ...
    if len(all_tests) != len(unique_scenarios):
        print(f"   ⚠️  Some tests have identical step sequences")
    ...
    all_unique = (
        len(all_tests) == len(unique_ids) and
        len(all_tests) == len(unique_names) and
        len(all_tests) == len(unique_scenarios)
    )
    
    if all_unique:
        print(f"✅ ALL 2000 TESTS ARE COMPLETELY UNIQUE")
        print(f"   - Unique IDs: ✅")
        print(f"   - Unique names: ✅")
        print(f"   - Unique scenarios: ✅")
    else:
        print(f"⚠️  UNIQUENESS ISSUES DETECTED")
        if len(all_tests) != len(unique_ids):
            print(f"   - IDs: ❌ (duplicates found)")
        if len(all_tests) != len(unique_names):
            print(f"   - Names: ❌ (duplicates found)")
        if len(all_tests) != len(unique_scenarios):
            print(f"   - Scenarios: ⚠️  (some steps are identical)")
