...
class TestCrossComponentIntegration:
    ...
    def test_e2e_multi_user_governance_isolation(
        ...
        admin_login = flask_client.post("/api/auth/login", json={
            "username": "admin",
            "password": "open-sesame"
        })
        admin_token = admin_login.get_json()["token"]
        
        guest_login = flask_client.post("/api/auth/login", json={
            "username": "guest",
            "password": "letmein"
        })
...
class TestSystemHealthAndMonitoring:
    ...
    def test_e2e_complete_system_health_check(self, flask_client, governance_api_available):
        ...
        gov_health = requests.get(
            f"{GOVERNANCE_API_URL}/health",
            timeout=TIMEOUT
        )
        ...
        tarl_response = requests.get(
            f"{GOVERNANCE_API_URL}/tarl",
            timeout=TIMEOUT
        )
...
class TestCompleteUserJourneys:
    ...
    def test_e2e_new_user_onboarding_and_first_action(
        ...
        login_response = flask_client.post("/api/auth/login", json={
            "username": "guest",
            "password": "letmein"
        })
        ...
        audit_response = requests.get(
            f"{GOVERNANCE_API_URL}/audit?limit=50",
            timeout=TIMEOUT
        )
        assert audit_response.status_code == 200
        audit_data = audit_response.json()
    ...
    def test_e2e_security_denial_workflow(
        self, flask_client, governance_api_available
    ):
        """
...
class TestAuditAndCompliance:
    ...
    def test_e2e_complete_audit_trail(
        self, flask_client, governance_api_available
    ):
        """
        ...
        audit1 = requests.get(
            f"{GOVERNANCE_API_URL}/audit?limit=1000",
            timeout=TIMEOUT
        )
