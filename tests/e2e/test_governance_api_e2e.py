...
class TestGovernanceAPIEndToEnd:
    ...
    def test_e2e_multiple_intents_sequence(self, api_health_check):
        ...
        intents = [
            {"actor": "human", "action": "read", "target": "/data/file1.json", "origin": "test"},
            {"actor": "agent", "action": "read", "target": "/data/file2.json", "origin": "test"},
            {"actor": "human", "action": "read", "target": "/data/file3.json", "origin": "test"},
        ]
        ...
        audit_response = requests.get(
            f"{API_BASE_URL}/audit?limit=100",
            timeout=TIMEOUT
        )
        assert audit_response.status_code == 200
        audit_data = audit_response.json()
