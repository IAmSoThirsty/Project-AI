...
class TestBuildCognitionEngine:
    ...
    def test_deliberate_with_boundary_exceeded(self, mock_deliberation_engine):
        ...
        with patch("gradle_evolution.cognition.build_cognition.check_boundary", return_value=False):
            optimized, reasoning = engine.deliberate_build_plan(tasks, context)
    ...
    def test_invariant_validation(self, mock_deliberation_engine):
        ...
        with patch("gradle_evolution.cognition.build_cognition.check_boundary", return_value=True):
            with patch.object(engine, "_validate_task_order", return_value=False) as mock_validate:
                optimized, reasoning = engine.deliberate_build_plan(tasks, context)
...
class TestBuildStateIntegration:
    ...
    def test_get_build_metrics(self, temp_dir):
        ...
        integration.record_build_state("build-1", {
            "status": "completed",
            "duration": 30.5,
        })
        integration.record_build_state("build-2", {
            "status": "failed",
            "duration": 20.0,
        })
