...
# Load tests
with open('adversarial_stress_tests_2000.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
...
print(f"   Result: {'PASS - All scenarios unique' if len(all_tests) == len(unique_scenarios) else 'INFO - Some steps reused (expected for variations)'}")
...
if ids_unique and names_unique:
    print(f"PASS: All 2000 tests have unique IDs and names")
    ...
    print(f"FAIL: Duplicate tests found")
