...
# Reality Anchor
 ...
 this service provides a "pulse" of absolute system time (UTC+0) that services must check in with if they start drifting into asynchronous processing loops.
...
```bash
# Build image
docker build -t Reality Anchor:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Reality Anchor:latest
```
...
```bash
kubectl rollout status deployment/Reality Anchor
```

1. **View logs**

```bash
kubectl logs -f deployment/Reality Anchor
```
...
- `service_reality_anchor_requests_total` - Total requests
- `service_reality_anchor_request_duration_seconds` - Request latency
- `service_reality_anchor_requests_inflight` - In-flight requests
- `service_reality_anchor_db_query_duration_seconds` - Database query latency
