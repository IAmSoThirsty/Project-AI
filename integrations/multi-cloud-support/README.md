...
deploy to AWS, GCP, Azure, or on-prem with minimal config changes
...
```bash
# Build image
docker build -t Multi-cloud support:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Multi-cloud support:latest
```
...
```bash
kubectl rollout status deployment/Multi-cloud support
```

1. **View logs**

```bash
kubectl logs -f deployment/Multi-cloud support
```
...
- `service_multi_cloud_support_requests_total` - Total requests
- `service_multi_cloud_support_request_duration_seconds` - Request latency
- `service_multi_cloud_support_requests_inflight` - In-flight requests
- `service_multi_cloud_support_db_query_duration_seconds` - Database query latency
