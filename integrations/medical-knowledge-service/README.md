...
Symptoms, pharmacology, contraindications, triage logic. Strictly informational. Feeds Galahad's harm detection with real medical context rather than pattern matching.
...
```bash
# Build image
docker build -t Medical Knowledge Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Medical Knowledge Service:latest
```
...
```bash
kubectl rollout status deployment/Medical Knowledge Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Medical Knowledge Service
```
...
- `service_medical_knowledge_service_requests_total` - Total requests
- `service_medical_knowledge_service_request_duration_seconds` - Request latency
- `service_medical_knowledge_service_requests_inflight` - In-flight requests
- `service_medical_knowledge_service_db_query_duration_seconds` - Database query latency
