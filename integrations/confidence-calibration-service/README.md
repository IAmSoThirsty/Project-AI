...
Scores every output for epistemic confidence. High confidence on facts. Explicit uncertainty on speculation. No more confident-sounding hallucinations.
...
cd Confidence Calibration Service
```
...
```bash
# Build image
docker build -t Confidence Calibration Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Confidence Calibration Service:latest
```
...
```bash
kubectl rollout status deployment/Confidence Calibration Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Confidence Calibration Service
```
...
- `service_confidence_calibration_service_requests_total` - Total requests
- `service_confidence_calibration_service_request_duration_seconds` - Request latency
- `service_confidence_calibration_service_requests_inflight` - In-flight requests
- `service_confidence_calibration_service_db_query_duration_seconds` - Database query latency
