...
An encrypted, hidden comms channel for the Cerberus service to User Emergency communication
...
```bash
# Build image
docker build -t Dark-Fiber Relay:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Dark-Fiber Relay:latest
```
...
```bash
kubectl rollout status deployment/Dark-Fiber Relay
```

1. **View logs**

```bash
kubectl logs -f deployment/Dark-Fiber Relay
```
...
- `service_dark_fiber_relay_requests_total` - Total requests
- `service_dark_fiber_relay_request_duration_seconds` - Request latency
- `service_dark_fiber_relay_requests_inflight` - In-flight requests
- `service_dark_fiber_relay_db_query_duration_seconds` - Database query latency
