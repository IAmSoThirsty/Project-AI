...
# The Epoch Synchronizer

Function: It treats "Time" as a versioned database. If you change a parameter in the past (simulation-wise), it re-calculates the downstream effects on your Medical and Legal modules.
...
```bash
# Build image
docker build -t The Epoch Synchronizer:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  The Epoch Synchronizer:latest
```
...
```bash
kubectl rollout status deployment/The Epoch Synchronizer
```

1. **View logs**

```bash
kubectl logs -f deployment/The Epoch Synchronizer
```
...
- `service_the_epoch_synchronizer_requests_total` - Total requests
- `service_the_epoch_synchronizer_request_duration_seconds` - Request latency
- `service_the_epoch_synchronizer_requests_inflight` - In-flight requests
- `service_the_epoch_synchronizer_db_query_duration_seconds` - Database query latency
