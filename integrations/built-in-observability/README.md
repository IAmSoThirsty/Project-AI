...
automatic tracing, metrics, and logging integration for monitoring and debugging
...
```bash
# Build image
docker build -t Built-in observability:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Built-in observability:latest
```
...
```bash
kubectl rollout status deployment/Built-in observability
```

1. **View logs**

```bash
kubectl logs -f deployment/Built-in observability
```
...
- `service_built_in_observability_requests_total` - Total requests
- `service_built_in_observability_request_duration_seconds` - Request latency
- `service_built_in_observability_requests_inflight` - In-flight requests
- `service_built_in_observability_db_query_duration_seconds` - Database query latency
