...
generate Kubernetes manifests with auto-scaling and self-healing configurations out-of-the-box
...
cd Auto-scaling & self-healing configs
```
...
```bash
# Build image
docker build -t Auto-scaling & self-healing configs:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Auto-scaling & self-healing configs:latest
```
...
```bash
kubectl rollout status deployment/Auto-scaling & self-healing configs
```

1. **View logs**

```bash
kubectl logs -f deployment/Auto-scaling & self-healing configs
```
...
- `service_auto_scaling_&_self_healing_configs_requests_total` - Total requests
- `service_auto_scaling_&_self_healing_configs_request_duration_seconds` - Request latency
- `service_auto_scaling_&_self_healing_configs_requests_inflight` - In-flight requests
- `service_auto_scaling_&_self_healing_configs_db_query_duration_seconds` - Database query latency
