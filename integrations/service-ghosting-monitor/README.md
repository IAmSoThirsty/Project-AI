...
# Service Ghosting Monitor

Since you have Knowledge Decay, you need this to identify "zombie" services that are still running but no longer have any active upstream dependencies or purpose.
...
```bash
# Build image
docker build -t Service Ghosting Monitor:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Service Ghosting Monitor:latest
```
...
```bash
kubectl rollout status deployment/Service Ghosting Monitor
```

1. **View logs**

```bash
kubectl logs -f deployment/Service Ghosting Monitor
```
...
- `service_service_ghosting_monitor_requests_total` - Total requests
- `service_service_ghosting_monitor_request_duration_seconds` - Request latency
- `service_service_ghosting_monitor_requests_inflight` - In-flight requests
- `service_service_ghosting_monitor_db_query_duration_seconds` - Database query latency
