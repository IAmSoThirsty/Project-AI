...
When a primitive becomes vulnerable — T.A.M.S.-Ω Tier-Zero Constitutional Event — this service orchestrates the migration. Freeze, classify, transition, archive. The thing that keeps the cryptographic foundation from becoming a single point of failure
...
cd Cryptographic Agility Service
```
...
```bash
# Build image
docker build -t Cryptographic Agility Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Cryptographic Agility Service:latest
```
...
```bash
kubectl rollout status deployment/Cryptographic Agility Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Cryptographic Agility Service
```
...
- `service_cryptographic_agility_service_requests_total` - Total requests
- `service_cryptographic_agility_service_request_duration_seconds` - Request latency
- `service_cryptographic_agility_service_requests_inflight` - In-flight requests
- `service_cryptographic_agility_service_db_query_duration_seconds` - Database query latency
