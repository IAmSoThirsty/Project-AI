...
support for blue-green deployments and canary releases to minimize downtime during updates
...
```bash
# Build image
docker build -t Zero-downtime deploys:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Zero-downtime deploys:latest
```
...
```bash
kubectl rollout status deployment/Zero-downtime deploys
```

1. **View logs**

```bash
kubectl logs -f deployment/Zero-downtime deploys
```
...
- `service_zero_downtime_deploys_requests_total` - Total requests
- `service_zero_downtime_deploys_request_duration_seconds` - Request latency
- `service_zero_downtime_deploys_requests_inflight` - In-flight requests
- `service_zero_downtime_deploys_db_query_duration_seconds` - Database query latency
