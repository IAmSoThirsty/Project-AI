...
# Paradox Resolution Service

A companion to your Contradiction Detection. When the system detects two conflicting "truths" (e.g., two different data sources claiming to be the Master), this service executes a "coin-flip" or a "consensus-vote" to prevent system lock.
...
```bash
# Build image
docker build -t Paradox Resolution Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Paradox Resolution Service:latest
```
...
```bash
kubectl rollout status deployment/Paradox Resolution Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Paradox Resolution Service
```
...
- `service_paradox_resolution_service_requests_total` - Total requests
- `service_paradox_resolution_service_request_duration_seconds` - Request latency
- `service_paradox_resolution_service_requests_inflight` - In-flight requests
- `service_paradox_resolution_service_db_query_duration_seconds` - Database query latency
