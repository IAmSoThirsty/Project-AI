...
class TARLBridge:
    ...
    def __init__(
        ...
        self.runtime: Optional[TARLRuntime] = None
    ...
    async def _load_policies(self) -> None:
        ...
        policy_files = list(self.policy_dir.glob('*.yaml')) + \
                       list(self.policy_dir.glob('*.json'))
    ...
    async def evaluate_policy_batch(self, contexts: List[Dict[str, Any]]) -> List[SecurityDecision]:
        """
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            'policiesLoaded': len(self.policy_engine.policies) if self.policy_engine else 0,
            'cacheHitRate': (
    ...
    def _get_cached_decision(self, cache_key: str) -> Optional[SecurityDecision]:
...
class ThirstyLangSecurity:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.settings = {
            ...
            'fs_restricted_paths': self.config.get('fs_restricted_paths', [
                '/etc', '/sys', '/proc', '/dev'
            ]),
    ...
    async def check_operation(self, operation: str, resource: str, context: Dict[str, Any]) -> SecurityDecision:
        """
        ...
        if self.settings['sandbox_enabled']:
            ...
            if operation.startswith('network_') and not self.settings['network_allowed']:
                return SecurityDecision(
                    allowed=False,
                    decision_type=DecisionType.DENY,
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if self.tarl_bridge:
                ...
                self.logger.warning("TARL not available - using Thirsty-Lang security only")
