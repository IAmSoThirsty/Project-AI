...
# COMPLIANCE: Sovereign Substrate / Integrated Service: agents / architect_of_flowing.py
# ============================================================================ #
"""
...
CANONICAL_QUEUE: list[WorkUnit] = [
    ...
    WorkUnit(
        id="memory_agent",
        name="Substrate Memory Agent",
        script="tools/memory_agent.py",
        depends_on=["completion_tracker", "boot_verification"],
        output_files=["governance/memory/SESSION_MEMORY.json"],
        required=False,
    ),
]
...
class ArchitectOfFlowing:
    ...
    def _write_outputs(self):
        ...
        FLOW_LOG.write_text(json.dumps(manifest, indent=2))
    ...
    def _log_header(self):
        print(f"[Architect of Flowing] {self.flow_start.isoformat()}")
        print(f"[Architect of Flowing] {len(self.queue)} agents queued")
        print("-" * 60)

    def _log_result(self, r: ExecutionResult):
        icon = {"PASS": "✅", "FAIL": "❌", "SKIP": "⏭️", "MISSING_OUTPUT": "⚠️"}.get(r.status, "?")
        print(f"{icon} [{r.status:14s}] {r.name} ({r.duration_seconds}s)")
    ...
    def _log_halt(self, unit: WorkUnit, result: ExecutionResult):
        print("-" * 60)
        print(f"[HALT] Required agent failed: {unit.name}")
        print(f"[HALT] Downstream agents will not run.")
        print(f"[HALT] Fix {unit.script} and re-run.")
