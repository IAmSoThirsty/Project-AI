...
To automatically "resurrect" the Succession & Escrow service if it fails during a transition.
...
```bash
# Build image
docker build -t The Lazarus Hook:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  The Lazarus Hook:latest
```
...
```bash
kubectl rollout status deployment/The Lazarus Hook
```

1. **View logs**

```bash
kubectl logs -f deployment/The Lazarus Hook
```
...
- `service_the_lazarus_hook_requests_total` - Total requests
- `service_the_lazarus_hook_request_duration_seconds` - Request latency
- `service_the_lazarus_hook_requests_inflight` - In-flight requests
- `service_the_lazarus_hook_db_query_duration_seconds` - Database query latency
