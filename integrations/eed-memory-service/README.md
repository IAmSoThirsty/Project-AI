...
Experiential Episodic Data — Legion's persistent memory layer. Not just conversation history. Weighted, decaying memory that knows what mattered and what faded. The difference between a partner who remembers and a tool that retrieves.
...
```bash
# Build image
docker build -t EED memory Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  EED memory Service:latest
```
...
```bash
kubectl rollout status deployment/EED memory Service
```

1. **View logs**

```bash
kubectl logs -f deployment/EED memory Service
```
...
- `service_eed_memory_service_requests_total` - Total requests
- `service_eed_memory_service_request_duration_seconds` - Request latency
- `service_eed_memory_service_requests_inflight` - In-flight requests
- `service_eed_memory_service_db_query_duration_seconds` - Database query latency
