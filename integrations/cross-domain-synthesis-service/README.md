...
When a query touches multiple knowledge domains — say, the history of a medicinal plant and its chemical properties and its legal status in three jurisdictions — this service orchestrates the answer across services rather than forcing Legion to context-switch alone.
...
cd Cross-Domain Synthesis Service
```
...
```bash
# Build image
docker build -t Cross-Domain Synthesis Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Cross-Domain Synthesis Service:latest
```
...
```bash
kubectl rollout status deployment/Cross-Domain Synthesis Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Cross-Domain Synthesis Service
```
...
- `service_cross_domain_synthesis_service_requests_total` - Total requests
- `service_cross_domain_synthesis_service_request_duration_seconds` - Request latency
- `service_cross_domain_synthesis_service_requests_inflight` - In-flight requests
- `service_cross_domain_synthesis_service_db_query_duration_seconds` - Database query latency
