...
# Herbalogy & Ethnobotany Service

Plant identification, traditional medicinal use, contraindications, regional variants, indigenous knowledge attribution. Important: includes provenance — who knew this first and where.
...
cd Herbalogy & Ethnobotany Service
```
...
```bash
# Build image
docker build -t Herbalogy & Ethnobotany Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Herbalogy & Ethnobotany Service:latest
```
...
```bash
kubectl rollout status deployment/Herbalogy & Ethnobotany Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Herbalogy & Ethnobotany Service
```
...
- `service_herbalogy_&_ethnobotany_service_requests_total` - Total requests
- `service_herbalogy_&_ethnobotany_service_request_duration_seconds` - Request latency
- `service_herbalogy_&_ethnobotany_service_requests_inflight` - In-flight requests
- `service_herbalogy_&_ethnobotany_service_db_query_duration_seconds` - Database query latency
