...
cd Cerberus Containment Service
```
...
```bash
# Build image
docker build -t Cerberus Containment Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Cerberus Containment Service:latest
```
...
```bash
kubectl rollout status deployment/Cerberus Containment Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Cerberus Containment Service
```
...
- `service_cerberus_containment_service_requests_total` - Total requests
- `service_cerberus_containment_service_request_duration_seconds` - Request latency
- `service_cerberus_containment_service_requests_inflight` - In-flight requests
- `service_cerberus_containment_service_db_query_duration_seconds` - Database query latency
