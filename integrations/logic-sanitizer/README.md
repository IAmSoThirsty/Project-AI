...
Strips emotional or biased weight from inputs
...
```bash
# Build image
docker build -t Logic Sanitizer:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Logic Sanitizer:latest
```
...
```bash
kubectl rollout status deployment/Logic Sanitizer
```

1. **View logs**

```bash
kubectl logs -f deployment/Logic Sanitizer
```
...
- `service_logic_sanitizer_requests_total` - Total requests
- `service_logic_sanitizer_request_duration_seconds` - Request latency
- `service_logic_sanitizer_requests_inflight` - In-flight requests
- `service_logic_sanitizer_db_query_duration_seconds` - Database query latency
