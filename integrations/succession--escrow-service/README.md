...
# Succession & Escrow Service

T.A.M.S.-Ω Section 7. Cryptographic threshold escrow for sovereign identity. 3-of-5 key holders. The service that ensures Project-AI survives its founder. This one is personal.
...
```bash
# Build image
docker build -t Succession & Escrow Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Succession & Escrow Service:latest
```
...
```bash
kubectl rollout status deployment/Succession & Escrow Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Succession & Escrow Service
```
...
- `service_succession_&_escrow_service_requests_total` - Total requests
- `service_succession_&_escrow_service_request_duration_seconds` - Request latency
- `service_succession_&_escrow_service_requests_inflight` - In-flight requests
- `service_succession_&_escrow_service_db_query_duration_seconds` - Database query latency
