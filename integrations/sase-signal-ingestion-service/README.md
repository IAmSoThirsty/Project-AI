...
# SASE Signal Ingestion Service

The defensive layer. Receives telemetry, normalizes it, feeds the behavioral modeling pipeline. The ears of the system. Nothing happens in the environment that this service doesn't see and log.
...
cd SASE Signal Ingestion Service
```
...
```bash
# Build image
docker build -t SASE Signal Ingestion Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  SASE Signal Ingestion Service:latest
```
...
```bash
kubectl rollout status deployment/SASE Signal Ingestion Service
```

1. **View logs**

```bash
kubectl logs -f deployment/SASE Signal Ingestion Service
```
...
- `service_sase_signal_ingestion_service_requests_total` - Total requests
- `service_sase_signal_ingestion_service_request_duration_seconds` - Request latency
- `service_sase_signal_ingestion_service_requests_inflight` - In-flight requests
- `service_sase_signal_ingestion_service_db_query_duration_seconds` - Database query latency
