...
To work with the Knowledge Decay Service and decide exactly what to delete when memory hits 90%
...
```bash
# Build image
docker build -t Mnemonic Pruning:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Mnemonic Pruning:latest
```
...
```bash
kubectl rollout status deployment/Mnemonic Pruning
```

1. **View logs**

```bash
kubectl logs -f deployment/Mnemonic Pruning
```
...
- `service_mnemonic_pruning_requests_total` - Total requests
- `service_mnemonic_pruning_request_duration_seconds` - Request latency
- `service_mnemonic_pruning_requests_inflight` - In-flight requests
- `service_mnemonic_pruning_db_query_duration_seconds` - Database query latency
