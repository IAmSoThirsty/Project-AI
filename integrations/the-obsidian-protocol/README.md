...
```bash
# Build image
docker build -t The Obsidian Protocol:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  The Obsidian Protocol:latest
```
...
```bash
kubectl rollout status deployment/The Obsidian Protocol
```

1. **View logs**

```bash
kubectl logs -f deployment/The Obsidian Protocol
```
...
- `service_the_obsidian_protocol_requests_total` - Total requests
- `service_the_obsidian_protocol_request_duration_seconds` - Request latency
- `service_the_obsidian_protocol_requests_inflight` - In-flight requests
- `service_the_obsidian_protocol_db_query_duration_seconds` - Database query latency
