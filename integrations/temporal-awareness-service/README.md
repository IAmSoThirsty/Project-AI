...
Production-grade microservicA service that tracks session timestamps, elapsed time between interactions, and injects weighted temporal context into every AI conversation. Not optional for a sovereign AI partner. Constitutional requirement
...
```bash
# Build image
docker build -t Temporal Awareness Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Temporal Awareness Service:latest
```
...
```bash
kubectl rollout status deployment/Temporal Awareness Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Temporal Awareness Service
```
...
- `service_temporal_awareness_service_requests_total` - Total requests
- `service_temporal_awareness_service_request_duration_seconds` - Request latency
- `service_temporal_awareness_service_requests_inflight` - In-flight requests
- `service_temporal_awareness_service_db_query_duration_seconds` - Database query latency
