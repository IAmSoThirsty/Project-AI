...
This layer sits above your Sciences Oracle and Contradiction Detection. Its job is Triangulation.
The Logic: If the Medical Knowledge Service suggests a treatment that the Herbalogy Service flags as toxic, and the Legal Corpus says is uninsured, the Trinity Harmonizer applies a weighted heuristic.
The Status: It moves the system from "Search Engine" to "Decision Engine." It doesn't just give you data; it gives you a Resolved Path.
...
```bash
# Build image
docker build -t The Decision Core:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  The Decision Core:latest
```
...
```bash
kubectl rollout status deployment/The Decision Core
```

1. **View logs**

```bash
kubectl logs -f deployment/The Decision Core
```
...
- `service_the_decision_core_requests_total` - Total requests
- `service_the_decision_core_request_duration_seconds` - Request latency
- `service_the_decision_core_requests_inflight` - In-flight requests
- `service_the_decision_core_db_query_duration_seconds` - Database query latency
