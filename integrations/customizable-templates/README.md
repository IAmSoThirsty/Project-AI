...
generate services in different languages/frameworks (Go, Node.js, Python, etc) with customizable templates
...
```bash
# Build image
docker build -t Customizable templates:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Customizable templates:latest
```
...
```bash
kubectl rollout status deployment/Customizable templates
```

1. **View logs**

```bash
kubectl logs -f deployment/Customizable templates
```
...
- `service_customizable_templates_requests_total` - Total requests
- `service_customizable_templates_request_duration_seconds` - Request latency
- `service_customizable_templates_requests_inflight` - In-flight requests
- `service_customizable_templates_db_query_duration_seconds` - Database query latency
