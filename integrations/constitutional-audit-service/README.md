...
Immutable append-only ledger of every governance decision ever made. Merkle-chained. Cannot be edited, only appended. The permanent record that makes Project-AI's transparency claims actually verifiable rather than just stated.
...
cd Constitutional Audit Service
```
...
```bash
# Build image
docker build -t Constitutional Audit Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Constitutional Audit Service:latest
```
...
```bash
kubectl rollout status deployment/Constitutional Audit Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Constitutional Audit Service
```
...
- `service_constitutional_audit_service_requests_total` - Total requests
- `service_constitutional_audit_service_request_duration_seconds` - Request latency
- `service_constitutional_audit_service_requests_inflight` - In-flight requests
- `service_constitutional_audit_service_db_query_duration_seconds` - Database query latency
