...
inject failures and test resilience with built-in chaos engineering hooks
...
```bash
# Build image
docker build -t Chaos engineering hooks:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Chaos engineering hooks:latest
```
...
```bash
kubectl rollout status deployment/Chaos engineering hooks
```

1. **View logs**

```bash
kubectl logs -f deployment/Chaos engineering hooks
```
...
- `service_chaos_engineering_hooks_requests_total` - Total requests
- `service_chaos_engineering_hooks_request_duration_seconds` - Request latency
- `service_chaos_engineering_hooks_requests_inflight` - In-flight requests
- `service_chaos_engineering_hooks_db_query_duration_seconds` - Database query latency
