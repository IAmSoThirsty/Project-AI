...
# Linguistics & Etymology Service

Word origins, language family trees, semantic drift over time, translation confidence scoring. Thirsty-Lang could plug directly into this.
...
cd Linguistics & Etymology Service
```
...
```bash
# Build image
docker build -t Linguistics & Etymology Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Linguistics & Etymology Service:latest
```
...
```bash
kubectl rollout status deployment/Linguistics & Etymology Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Linguistics & Etymology Service
```
...
- `service_linguistics_&_etymology_service_requests_total` - Total requests
- `service_linguistics_&_etymology_service_request_duration_seconds` - Request latency
- `service_linguistics_&_etymology_service_requests_inflight` - In-flight requests
- `service_linguistics_&_etymology_service_db_query_duration_seconds` - Database query latency
