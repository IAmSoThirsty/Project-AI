...
# The Trinity Harmonizer

This is the top-level logic gate. When the Medical Knowledge Service suggests a procedure, but the Legal Corpus Service flags a liability, and the Temporal Dissonance Bridge shows a conflict in the timeline—this service breaks the tie.
...
```bash
# Build image
docker build -t The Trinity Harmonizer:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  The Trinity Harmonizer:latest
```
...
```bash
kubectl rollout status deployment/The Trinity Harmonizer
```

1. **View logs**

```bash
kubectl logs -f deployment/The Trinity Harmonizer
```
...
- `service_the_trinity_harmonizer_requests_total` - Total requests
- `service_the_trinity_harmonizer_request_duration_seconds` - Request latency
- `service_the_trinity_harmonizer_requests_inflight` - In-flight requests
- `service_the_trinity_harmonizer_db_query_duration_seconds` - Database query latency
