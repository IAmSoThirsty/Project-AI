...
Not Western-centric. Full geographic and cultural coverage. Timeline engine. Causal chain mapping — what led to what and why. Cross-cultural event correlation. Legion should know that things happened everywhere, not just in textbooks.
...
```bash
# Build image
docker build -t World History Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  World History Service:latest
```
...
```bash
kubectl rollout status deployment/World History Service
```

1. **View logs**

```bash
kubectl logs -f deployment/World History Service
```
...
- `service_world_history_service_requests_total` - Total requests
- `service_world_history_service_request_duration_seconds` - Request latency
- `service_world_history_service_requests_inflight` - In-flight requests
- `service_world_history_service_db_query_duration_seconds` - Database query latency
