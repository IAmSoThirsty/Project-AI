...
Injects honest temporal context into every conversation. Knows how much time passed. Knows what weight that duration carries. Asks what the gap contained rather than pretending it was nothing.
...
```bash
# Build image
docker build -t Temporal Dissonance Bridge:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Temporal Dissonance Bridge:latest
```
...
```bash
kubectl rollout status deployment/Temporal Dissonance Bridge
```

1. **View logs**

```bash
kubectl logs -f deployment/Temporal Dissonance Bridge
```
...
- `service_temporal_dissonance_bridge_requests_total` - Total requests
- `service_temporal_dissonance_bridge_request_duration_seconds` - Request latency
- `service_temporal_dissonance_bridge_requests_inflight` - In-flight requests
- `service_temporal_dissonance_bridge_db_query_duration_seconds` - Database query latency
