...
The Logic: This layer ensures that the World History Service and the Constitutional Amendment Service are looking at the same "version" of reality.
...
```bash
# Build image
docker build -t The Temporal Clock:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  The Temporal Clock:latest
```
...
```bash
kubectl rollout status deployment/The Temporal Clock
```

1. **View logs**

```bash
kubectl logs -f deployment/The Temporal Clock
```
...
- `service_the_temporal_clock_requests_total` - Total requests
- `service_the_temporal_clock_request_duration_seconds` - Request latency
- `service_the_temporal_clock_requests_inflight` - In-flight requests
- `service_the_temporal_clock_db_query_duration_seconds` - Database query latency
