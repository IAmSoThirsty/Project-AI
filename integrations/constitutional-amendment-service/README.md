...
Manages the three-tier review process from T.A.M.S.-Ω — 72 hour emergency, 30 day standard, 90 day constitutional. Enforces review periods. Auto-reverts emergency amendments if not ratified. The living governance engine.
...
cd Constitutional Amendment Service
```
...
```bash
# Build image
docker build -t Constitutional Amendment Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Constitutional Amendment Service:latest
```
...
```bash
kubectl rollout status deployment/Constitutional Amendment Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Constitutional Amendment Service
```
...
- `service_constitutional_amendment_service_requests_total` - Total requests
- `service_constitutional_amendment_service_request_duration_seconds` - Request latency
- `service_constitutional_amendment_service_requests_inflight` - In-flight requests
- `service_constitutional_amendment_service_db_query_duration_seconds` - Database query latency
