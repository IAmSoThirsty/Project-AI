...
Logs how the system "sees" itself via telemetry Built-in Observability
...
```bash
kubectl rollout status deployment/Cognitive Mirror
```

1. **View logs**

```bash
kubectl logs -f deployment/Cognitive Mirror
```
...
- `service_cognitive_mirror_requests_total` - Total requests
- `service_cognitive_mirror_request_duration_seconds` - Request latency
- `service_cognitive_mirror_requests_inflight` - In-flight requests
- `service_cognitive_mirror_db_query_duration_seconds` - Database query latency
