...
Prevents race conditions in the Temporal Bridge
...
```bash
# Build image
docker build -t Temporal Anchor:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Temporal Anchor:latest
```
...
```bash
kubectl rollout status deployment/Temporal Anchor
```

1. **View logs**

```bash
kubectl logs -f deployment/Temporal Anchor
```
...
- `service_temporal_anchor_requests_total` - Total requests
- `service_temporal_anchor_request_duration_seconds` - Request latency
- `service_temporal_anchor_requests_inflight` - In-flight requests
- `service_temporal_anchor_db_query_duration_seconds` - Database query latency
