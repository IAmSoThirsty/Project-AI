...
# Fork Registry Service

Tracks sovereign lineage. When a fork establishes its own root identity it registers here — not to claim authority over it, but to maintain the historical record. Lineage is historical not hierarchical. This service holds that truth.
...
```bash
# Build image
docker build -t Fork Registry Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Fork Registry Service:latest
```
...
```bash
kubectl rollout status deployment/Fork Registry Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Fork Registry Service
```
...
- `service_fork_registry_service_requests_total` - Total requests
- `service_fork_registry_service_request_duration_seconds` - Request latency
- `service_fork_registry_service_requests_inflight` - In-flight requests
- `service_fork_registry_service_db_query_duration_seconds` - Database query latency
