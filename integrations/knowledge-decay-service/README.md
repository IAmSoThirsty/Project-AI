...
Information goes stale. This service tracks the age and volatility of cached knowledge. Medical guidelines change. Laws change. Science updates. Knows when to re-verify.
...
```bash
# Build image
docker build -t Knowledge Decay Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Knowledge Decay Service:latest
```
...
```bash
kubectl rollout status deployment/Knowledge Decay Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Knowledge Decay Service
```
...
- `service_knowledge_decay_service_requests_total` - Total requests
- `service_knowledge_decay_service_request_duration_seconds` - Request latency
- `service_knowledge_decay_service_requests_inflight` - In-flight requests
- `service_knowledge_decay_service_db_query_duration_seconds` - Database query latency
