...
Physics, chemistry, biology, astronomy. Structured knowledge with citation chains. Not Wikipedia summaries — formal domain knowledge with confidence scoring per claim.
...
```bash
# Build image
docker build -t Sciences Oracle:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Sciences Oracle:latest
```
...
```bash
kubectl rollout status deployment/Sciences Oracle
```

1. **View logs**

```bash
kubectl logs -f deployment/Sciences Oracle
```
...
- `service_sciences_oracle_requests_total` - Total requests
- `service_sciences_oracle_request_duration_seconds` - Request latency
- `service_sciences_oracle_requests_inflight` - In-flight requests
- `service_sciences_oracle_db_query_duration_seconds` - Database query latency
