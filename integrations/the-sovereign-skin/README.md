...
The Logic: Instead of checking passwords, it checks Behavioral Integrity. If a request comes in that fits the Linguistics of a known threat but has the Identity of a valid user, the Cerberus service locks the "Temporal Bridge" to prevent data egress.
...
```bash
# Build image
docker build -t The Sovereign Skin:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  The Sovereign Skin:latest
```
...
```bash
kubectl rollout status deployment/The Sovereign Skin
```

1. **View logs**

```bash
kubectl logs -f deployment/The Sovereign Skin
```
...
- `service_the_sovereign_skin_requests_total` - Total requests
- `service_the_sovereign_skin_request_duration_seconds` - Request latency
- `service_the_sovereign_skin_requests_inflight` - In-flight requests
- `service_the_sovereign_skin_db_query_duration_seconds` - Database query latency
