...
# The Heuristic Sentinel

Function: If the Sciences Oracle and the Constitutional Audit disagree on a data point, the Sentinel puts the entire stack into "Read-Only" mode until the Confidence Calibration Service hits a >98% threshold.
...
```bash
# Build image
docker build -t The Heuristic Sentinel:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  The Heuristic Sentinel:latest
```
...
```bash
kubectl rollout status deployment/The Heuristic Sentinel
```

1. **View logs**

```bash
kubectl logs -f deployment/The Heuristic Sentinel
```
...
- `service_the_heuristic_sentinel_requests_total` - Total requests
- `service_the_heuristic_sentinel_request_duration_seconds` - Request latency
- `service_the_heuristic_sentinel_requests_inflight` - In-flight requests
- `service_the_heuristic_sentinel_db_query_duration_seconds` - Database query latency
