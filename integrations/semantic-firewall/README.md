...
Instead of blocking IP addresses, it blocks ideas or requests that don't fit the current linguistic model of your Linguistics & Etymology Service.
...
```bash
# Build image
docker build -t Semantic Firewall:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Semantic Firewall:latest
```
...
```bash
kubectl rollout status deployment/Semantic Firewall
```

1. **View logs**

```bash
kubectl logs -f deployment/Semantic Firewall
```
...
- `service_semantic_firewall_requests_total` - Total requests
- `service_semantic_firewall_request_duration_seconds` - Request latency
- `service_semantic_firewall_requests_inflight` - In-flight requests
- `service_semantic_firewall_db_query_duration_seconds` - Database query latency
