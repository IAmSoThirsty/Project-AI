...
Cryptographic root identity management. Issues and validates the identity tokens that T.A.M.S.-Ω says must underpin all constitutional authority. No identity service means no cryptographic sovereignty — everything else is built on sand.
...
```bash
# Build image
docker build -t Sovereign Identity Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Sovereign Identity Service:latest
```
...
```bash
kubectl rollout status deployment/Sovereign Identity Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Sovereign Identity Service
```
...
- `service_sovereign_identity_service_requests_total` - Total requests
- `service_sovereign_identity_service_request_duration_seconds` - Request latency
- `service_sovereign_identity_service_requests_inflight` - In-flight requests
- `service_sovereign_identity_service_db_query_duration_seconds` - Database query latency
