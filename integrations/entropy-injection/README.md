...
To feed random noise into your Cryptographic Agility service to prevent pattern-matching attacks
...
```bash
# Build image
docker build -t Entropy Injection:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Entropy Injection:latest
```
...
```bash
kubectl rollout status deployment/Entropy Injection
```

1. **View logs**

```bash
kubectl logs -f deployment/Entropy Injection
```
...
- `service_entropy_injection_requests_total` - Total requests
- `service_entropy_injection_request_duration_seconds` - Request latency
- `service_entropy_injection_requests_inflight` - In-flight requests
- `service_entropy_injection_db_query_duration_seconds` - Database query latency
