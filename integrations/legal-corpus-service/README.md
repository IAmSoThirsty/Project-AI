...
Jurisdiction-aware. Not legal advice — legal structure. Knows the difference between common law and civil law systems. Flags when something touches regulatory territory.
...
```bash
# Build image
docker build -t Legal Corpus Service:latest .

# Run container
docker run -p 8000:8000 \
  -e DATABASE_URL="your-database-url" \
  Legal Corpus Service:latest
```
...
```bash
kubectl rollout status deployment/Legal Corpus Service
```

1. **View logs**

```bash
kubectl logs -f deployment/Legal Corpus Service
```
...
- `service_legal_corpus_service_requests_total` - Total requests
- `service_legal_corpus_service_request_duration_seconds` - Request latency
- `service_legal_corpus_service_requests_inflight` - In-flight requests
- `service_legal_corpus_service_db_query_duration_seconds` - Database query latency
