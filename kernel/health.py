            ...
            if success:
                ...
                if cb.failures >= cb.failure_threshold:
                    if cb.state != "open":
                        ...
                        logger.warning(f"Circuit breaker {name} opened (threshold exceeded)")
                ...
                with self.lock:
                    for name, check in self.health_checks.items():
                        ...
                        if check.last_check_time is None or \
                           (now - check.last_check_time) >= check.interval_seconds:
                            self.run_check(name)
    ...
    def get_health_report(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            for probe_type in ProbeType:
                checks = [c for c in self.health_checks.values() if c.probe_type == probe_type]
