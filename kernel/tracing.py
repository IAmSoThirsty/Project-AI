...
class TracingSystem:
    ...
    def __init__(self):
        ...
        logger.info(f"Tracing system initialized (sampling: {self.sampling_rate * 100}%)")
    ...
    def end_span(
        ...
        with self.lock:
            ...
            logger.debug(f"Ended span {span_id} ({span.name}) - {duration_ms:.2f}ms - {status.value}")
    ...
    def analyze_critical_path(self, trace_id: str) -> List[Span]:
        ...
        with self.lock:
            ...
            # Return longest path
            return max(critical_paths, key=lambda p: sum(
                (s.end_time or 0) - s.start_time for s in p
            ))
    ...
    def _find_critical_path(self, span: Span, all_spans: List[Span]) -> List[Span]:
        ...
        longest_child_path = max(child_paths, key=lambda p: sum(
            (s.end_time or 0) - s.start_time for s in p
        ))
    ...
    def get_stats(self) -> Dict:
        ...
        with self.lock:
            return {
                ...
                "total_completed_spans": sum(len(spans) for spans in self.completed_spans.values()),
...
def console_exporter(span: Span):
    ...
    print(f"[SPAN] {span.name} | {span.kind.value} | {duration_ms:.2f}ms | {span.status.value}")
