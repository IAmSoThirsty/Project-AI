...
class ConfigurationManager:
    ...
    def __init__(self, config_dir: Optional[str] = None):
        ...
        logger.info(f"Configuration manager initialized (config_dir: {self.config_dir})")
    ...
    def _validate_value(self, key: str, value: Any) -> bool:
        ...
        if "type" in schema:
            ...
            if expected_type and not isinstance(value, expected_type):
                logger.error(f"Type mismatch for {key}: expected {schema['type']}, got {type(value).__name__}")
                return False
        ...
        if "enum" in schema and value not in schema["enum"]:
            logger.error(f"Value {value} for {key} not in allowed values {schema['enum']}")
            return False
        ...
        if "pattern" in schema and isinstance(value, str):
            ...
            if not re.match(schema["pattern"], value):
                logger.error(f"Value '{value}' for {key} doesn't match pattern {schema['pattern']}")
                return False
    ...
    def _record_change(self, key: str, old_value: Any, new_value: Any, source: str):
        ...
        logger.info(f"Config changed: {key} = {old_value} → {new_value} (source: {source})")
