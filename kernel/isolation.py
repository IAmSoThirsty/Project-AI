...
class IsolationConfig:
    ...
    namespaces: Set[NamespaceType] = field(default_factory=lambda: {
        NamespaceType.PID,
        NamespaceType.MOUNT,
        NamespaceType.IPC,
        NamespaceType.UTS
...
class ProcessIsolationManager:
    ...
    def create_isolated_process(
        ...
        with self.lock:
            ...
            logger.info(f"Created isolation for process {pid} with {len(config.namespaces)} namespaces")
    ...
    def _create_namespace(self, ns_type: NamespaceType, owner_pid: int) -> Namespace:
        ...
        logger.debug(f"Created {ns_type.value} namespace {ns.namespace_id} for process {owner_pid}")
        return ns
    ...
    def join_namespace(self, pid: int, namespace_id: int):
        ...
        with self.lock:
            ...
            logger.debug(f"Process {pid} joined {ns.namespace_type.value} namespace {namespace_id}")
    ...
    def check_syscall_allowed(self, pid: int, syscall_name: str) -> bool:
        ...
        with self.lock:
            ...
            if not allowed:
                ...
                logger.warning(f"Seccomp denied syscall {syscall_name} for process {pid}")
    ...
    def check_capability(self, pid: int, capability: Capability) -> bool:
        ...
        with self.lock:
            ...
            if not has_cap:
                ...
                logger.warning(f"Capability denial: process {pid} lacks {capability.value}")
    ...
    def revoke_capability(self, pid: int, capability: Capability):
        ...
        with self.lock:
            if pid in self.process_capabilities:
                ...
                logger.debug(f"Revoked capability {capability.value} from process {pid}")
    ...
    def _apply_cgroup_limits(self, pid: int, limits: ResourceLimits):
        ...
        if applied:
            logger.debug(f"Applied cgroup limits to process {pid}: {', '.join(applied)}")
    ...
    def check_isolation_violation(self, pid1: int, pid2: int, access_type: str) -> bool:
        ...
        with self.lock:
            ...
            if pid1 not in self.process_namespaces or pid2 not in self.process_namespaces:
            ...
            if access_type in ["signal", "ipc", "shm"]:
                if NamespaceType.IPC in ns1 and NamespaceType.IPC in ns2:
                    if ns1[NamespaceType.IPC] != ns2[NamespaceType.IPC]:
                        ...
                        logger.warning(f"Isolation violation: process {pid1} attempted {access_type} to process {pid2} in different namespace")
                        return False
            ...
            if access_type == "ptrace":
                if NamespaceType.PID in ns1 and NamespaceType.PID in ns2:
                    if ns1[NamespaceType.PID] != ns2[NamespaceType.PID]:
                        self.stats["isolation_violations"] += 1
                        logger.warning(f"Isolation violation: process {pid1} attempted ptrace on process {pid2} in different namespace")
                        return False
    ...
    def _get_all_syscalls(self) -> Set[str]:
        ...
        return {
            ...
            "setitimer", "alarm", "getpid", "sendfile", "socket", "connect",
    ...
    def get_isolation_stats(self) -> Dict:
        ...
        with self.lock:
            return {
                ...
                "namespaces_by_type": {
                    ns_type.value: sum(1 for ns in self.namespaces.values() if ns.namespace_type == ns_type)
