...
class MultiLevelFeedbackQueueScheduler:
    ...
    def _context_switch_out(self, pcb: ProcessControlBlock):
        ...
        pcb.context["quantum_used"] = self._calculate_quantum(pcb.priority) - pcb.quantum_remaining
    ...
    def _context_switch_in(self, pcb: ProcessControlBlock, cpu_id: int):
        ...
        if "saved_at" in pcb.context:
            logger.debug(f"Restored process {pcb.pid} (idle {time.time() - pcb.context['saved_at']:.3f}s)")
        ...
        for priority in [Priority.LOW, Priority.IDLE, Priority.BACKGROUND]:
            for pcb in self.ready_queues[priority]:
                ...
                if pcb.age >= self.config.aging_threshold:
                    ...
                    logger.debug(f"Aging boost: process {pcb.pid} {priority.name} → {new_priority.name}")
    ...
    def get_scheduler_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                "processes_by_state": {
                    state.name: sum(1 for p in self.processes.values() if p.state == state)
                    for state in ProcessState
                },
                "processes_by_priority": {
                    priority.name: sum(1 for p in self.processes.values() if p.priority == priority)
                    for priority in Priority
                },
                "ready_queue_depths": {
                    priority.name: len(queue) for priority, queue in self.ready_queues.items()
                },
                "running_processes": [
                    {"cpu": cpu, "pid": pcb.pid if pcb else None}
                    for cpu, pcb in self.running.items()
                ],
                "average_wait_time_ms": total_wait_time / total_processes if total_processes > 0 else 0,
                **self.stats
            }
    ...
    def change_priority(self, pid: int, new_priority: Priority):
        ...
        with self.lock:
            ...
            logger.debug(f"Changed priority for process {pid}: {old_priority.name} → {new_priority.name}")
