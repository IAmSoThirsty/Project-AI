...
class TelemetrySystem:
    ...
    def record(
        ...
        with self.lock:
            ...
            metric_value = MetricValue(
                timestamp=timestamp or time.time(), value=value, labels=labels or {}
            )
    ...
    def calculate_rate(
        ...
        with self.lock:
            ...
            recent_values = [
                ...
                and (
                    not labels or all(mv.labels.get(k) == v for k, v in labels.items())
                )
            ]
            ...
            for metric in self.metrics.values():
                ...
                if metric.metric_type == MetricType.HISTOGRAM:
                    ...
                    if metric.values:
                        ...
                        lines.append(
                            f"{metric.name}{label_str} {mv.value} {timestamp_ms}"
                        )
    ...
    def _get_histogram_export(self, metric: Metric) -> list[str]:
        ...
        for label_key, values in label_groups.items():
            # Reconstruct labels dict safely using ast.literal_eval
            import ast
            try:
                labels = dict(ast.literal_eval(label_key)) if label_key != "[]" else {}
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self.lock:
            return {
                ...
                "metrics_by_type": {
                    mt.value: sum(
                        1 for m in self.metrics.values() if m.metric_type == mt
                    )
