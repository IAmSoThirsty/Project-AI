...
class MemoryManager:
    ...
    def __init__(self, config: Optional[MemoryManagerConfig] = None):
        ...
        self.swap_pages = self.config.swap_size_bytes // self.config.page_size if self.config.enable_swapping else 0
        ...
        logger.info(f"Memory Manager initialized: {self.total_pages} pages ({self.config.total_memory_bytes / (1024**3):.2f} GB)")
    ...
    def allocate_memory(
        ...
        with self.lock:
            ...
            page_size = PageSize.LARGE if (prefer_huge_pages and self.config.enable_huge_pages and size_bytes >= PageSize.LARGE) else self.config.page_size
            ...
            if len(allocated_pages) < num_pages:
                ...
                if len(allocated_pages) < num_pages:
                    ...
                    raise MemoryError(f"Cannot allocate {size_bytes} bytes - out of memory")
            ...
            logger.debug(f"Allocated {size_bytes} bytes for process {pid} (alloc_id={allocation.allocation_id}, pages={num_pages})")
            return allocation
    ...
    def deallocate_memory(self, allocation_id: int):
        ...
        with self.lock:
            ...
            # Update accounting
            self.process_memory[pid] = self.process_memory.get(pid, 0) - allocation.size_bytes
            del self.allocations[allocation_id]
            self.stats["total_frees"] += 1
            
            logger.debug(f"Deallocated alloc_id={allocation_id} ({allocation.size_bytes} bytes)")
    ...
    def page_fault(self, pid: int, virtual_address: int) -> bool:
        ...
        with self.lock:
            ...
            logger.warning(f"Page fault at {hex(virtual_address)} for process {pid} - invalid access")
            return False
    ...
    def _lru_select(self, count: int) -> List[int]:
        ...
        allocated_pages = [
            (num, page) for num, page in self.pages.items()
    ...
    def _clock_select(self, count: int) -> List[int]:
        ...
        victims = []
        allocated = [num for num, page in self.pages.items() if page.state == PageState.ALLOCATED]
    ...
    def get_memory_stats(self) -> Dict:
        ...
        with self.lock:
            ...
            return {
                ...
                "memory_utilization": allocated_pages / self.total_pages if self.total_pages > 0 else 0,
                "fragmentation": self._calculate_fragmentation(),
                "swap_used": len(self.swap_map) if self.config.enable_swapping else 0,
                "swap_free": len(self.free_swap_slots) if self.config.enable_swapping else 0,
