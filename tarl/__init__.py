...
class TARLSystem:
    ...
    def _initialize_config(self) -> None:
        ...
        self.config = ConfigRegistry(
            config_path=self.config_path, overrides=self.config_overrides
        )
        self.config.load()
    ...
    def _initialize_stdlib(self) -> None:
        ...
        self.stdlib = StandardLibrary(config=self.config, diagnostics=self.diagnostics)
    ...
    def _initialize_ffi(self) -> None:
        ...
        self.ffi = FFIBridge(
            config=self.config, diagnostics=self.diagnostics, stdlib=self.stdlib
        )
        self.ffi.initialize()
    ...
    def execute_source(self, source: str, context: dict[str, Any] | None = None) -> Any:
        ...
        if not self._initialized:
            raise RuntimeError(
                "T.A.R.L. system not initialized. Call initialize() first."
            )
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "config_path": self.config_path,
            "diagnostics_status": (
                self.diagnostics.get_status() if self.diagnostics else None
            ),
...
_system_instance: TARLSystem | None = None


def get_system(config_path: str | None = None, **kwargs) -> TARLSystem:
    """
