# AGI-Charter

Mapped Source: `docs/governance`

## Representative Files

- `docs/governance/AGI_CHARTER.md`
- `docs/governance/AGI_IDENTITY_SPECIFICATION.md`
- `docs/governance/AI-INDIVIDUAL-ROLE-HUMANITY-ALIGNMENT.md`
- `docs/governance/CODEX_DEUS_QUICK_REF.md`
- `docs/governance/CODEX_DEUS_ULTIMATE_SUMMARY.md`
- `docs/governance/CONSTITUTIONAL_RESONANCE.md`
- `docs/governance/IDENTITY_SYSTEM_FULL_SPEC.md`
- `docs/governance/IRREVERSIBILITY_FORMALIZATION.md`
- `docs/governance/LEGION_COMMISSION.md`
- `docs/governance/LEGION_SYSTEM_CONTEXT.md`
- `docs/governance/LICENSING_GUIDE.md`
- `docs/governance/LICENSING_SUMMARY.md`
- `docs/governance/README.md`
- `docs/governance/policy/CODE_OF_CONDUCT.md`
- `docs/governance/policy/CONTRIBUTING.md`
