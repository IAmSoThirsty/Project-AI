...
class NegotiationService:
    ...
    async def submit_offer(self, session_id: UUID, offer: NegotiationOffer) -> NegotiationSession:
        """Process a new offer and update session state"""
        session = await self.repository.get_session(session_id)
        if not session:
            raise Exception("Session not found")

        logger.info(f"Negotiation: Offer from {offer.sender_id} in session {session_id}")
