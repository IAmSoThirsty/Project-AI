...
class ComplianceService:
    ...
    async def evaluate_compliance(self, trigger: AuditTrigger) -> List[ComplianceAudit]:
        ...
        for rule in rules:
            ...
            if not passed:
                logger.error(f"COMPLIANCE VIOLATION: Rule {rule.id} failed for {trigger.target_id}")
                violation = PolicyViolation(audit_id=audit.id, severity=rule.severity)
                await self.repository.record_violation(violation)

        return audits

    def _simulate_rule_eval(self, rule: ComplianceRule, context: Dict[str, Any]) -> bool:
