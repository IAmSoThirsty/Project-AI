...
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / Governance Host API                        #
...
logger = logging.getLogger("governance_host")
...
if os.getenv("ENABLE_RATE_LIMITING", "true").lower() == "true":
    try:
        ...
        logger.warning("RateLimitMiddleware not found. Rate limiting disabled.")
...
if os.getenv("ENABLE_REQUEST_VALIDATION", "true").lower() == "true":
    try:
        ...
        logger.warning("RequestValidationMiddleware not found. Request validation disabled.")
...
if os.getenv("ENABLE_OBSERVABILITY", "false").lower() == "true":
    try:
        ...
        logger.warning("Observability module setup failed. Tracing disabled.")
...
async def lifespan(application: FastAPI):
    ...
    try:
        ...
        logger.error("Failed to start auto-save: %s", e)
    ...
    try:
        ...
        logger.error("Failed to stop auto-save: %s", e)
...
class Cerberus:
    ...
    @staticmethod
    def evaluate(_intent: Intent, rule: dict[str, Any]) -> PillarVote:
...
def evaluate_tarl(intent: Intent) -> GovernanceResult:
    rules: list[dict[str, Any]] = TARL_V1["rules"]
    matching_rules = [r for r in rules if r["action"] == intent.action.value]
    ...
    return GovernanceResult(
        ...
        tarl_version=str(TARL_V1["version"]),
        votes=votes,
        final_verdict=final,
        timestamp=time.time(),
    )
...
@app.post("/intent")
async def submit_intent(intent: Intent, _request: Request):
