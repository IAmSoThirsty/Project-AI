...
2. **Collection Tests** - System metrics, dependencies, config
3. **Snapshot Tests** - YAML generation, content validation
4. **Report Tests** - PNG rendering, canonical path management
5. **Audit Tests** - Event logging, chain verification, tampering detection
6. **Integration Tests** - Config integration, audit integration
7. **Error Handling Tests** - Graceful degradation, error reporting
8. **CLI Tests** - Command execution, exit codes
...
2. **Alerting** - Trigger notifications on threshold breaches
3. **API Endpoint** - Expose health data via HTTP
4. **Export Formats** - Add JSON, CSV, HTML export
5. **Metric Plugins** - Allow custom metric collectors
6. **Dashboard Integration** - Real-time web dashboard
7. **Scheduled Reports** - Automatic periodic generation
...
**Issue:** `ImportError: cannot import name 'app' from 'app.cli'`
**Solution:** Use direct module execution: `python -m src.app.health.report`

**Issue:** Permission denied creating directories
**Solution:** Ensure write permissions on `data/` and `docs/assets/` directories

**Issue:** Audit chain verification fails
**Solution:** This indicates tampering. Review audit log history and investigate.
