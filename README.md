...
Run sovereign AI agents on your own hardware with cryptographic governance, no telemetry, and production-ready security primitives. Project-AI is a professional-grade substrate for developers and organizations that require absolute control over agent behavior. It replaces "best effort" prompt-based safety with **mathematically verifiable guardrails**, ensuring that every cognitive action is constitutionally bound, signed, and auditable. Whether you are building private RAG systems, autonomous infrastructure controllers, or ethical AI assistants, Project-AI provides the kernel-level containment and cryptographic proofs necessary to operate in high-stakes environments where "alignment" must be a technical guarantee, not a hope.
...
```mermaid
graph TD
    A[Health Check] --> B{All Pillars Healthy?}
    B -->|Yes| C[STABLE — Normal Operation]
    B -->|No| D{How Many Failed?}
    D -->|1 Pillar| E[Activate Liara Substitute]
    E --> F[SUBSTITUTED — Audit Logged]
    D -->|2+ Pillars| G[GOVERNANCE HOLD]
    G --> H[RuntimeError — System Halts]
    F --> I{Pillar Restored?}
    I -->|Yes| J[Revoke Liara → STABLE]
    I -->|No| K[Continue Substituted]
```
...
| **Policy State Binding** | Config hash → Policy hash → Role signature | Non-bypassable by design |
...
|----------|---------|-----------|
| **PlaneIsolationAnalyzer** | Ensures Shadow **cannot** mutate Canonical state | ✅ **CRITICAL** |
| **DeterminismAnalyzer** | Verifies deterministic execution for audit replay | ✅ |
| **PrivilegeEscalationAnalyzer** | Checks that constitutional validation is required | ✅ |
...
| **InvariantPurityChecker** | Blocks impure invariants (no side effects allowed) | ✅ **CRITICAL** |
...
```
Source → Lexer → Parser → AST → Semantic Analysis → Static Analyzers → Bytecode → VM
                                        │
                                  6 Analysis Passes
                                        │
                          ┌─────────────┼─────────────┐
...
|--------|------|-------|-----------|
| Ransomware | `lsm/file_open` | S2 ISOLATED | High-entropy writes → file_open blocked |
| Privilege Escalation | `lsm/task_fix_setuid` | S3 FROZEN | setuid attempt → freeze for forensics |
| C2 Beaconing | `lsm/socket_connect` | S2 ISOLATED | Outbound connects → network blocked |
| Lateral Movement | Gossip quorum | S boost | Multi-node reports → log-boosted quorum |
| Cryptominer | CPU pressure | S1→S2 | High CPU + unusual file access → escalation |
...
| 2 | Economic Suffocation Engine | Market manipulation → civilizational starvation |
...
| Layer | Name | Status |
|-------|------|--------|
| 0 | Constitutional Kernel | ✅ Production-Ready |
| 4 | Bayesian Claim Engine | ✅ Production-Ready |
| 9 | Sludge Narrative Sandbox | ✅ Production-Ready |
| 12 | CLI & Export Interface | ✅ Operational |
...
# Verify core systems
python -c "from governance.sovereign_runtime import SovereignRuntime; print('✅ Sovereign Runtime OK')"
python -c "from cognition.triumvirate import evaluate_triumvirate; print('✅ Triumvirate OK')"
```
...
🚨 PLANE ISOLATION GATE FIRED - Critical Violations:
   ❌ Shadow → Canonical mutation detected!
...
<p align="center">
  ...
  <a href="https://github.com/IAmSoThirsty/Project-AI/issues">🐛 Report Issues</a> &nbsp;|&nbsp;
  <a href="https://github.com/IAmSoThirsty/Project-AI/issues/new">💡 Request Features</a>
</p>
