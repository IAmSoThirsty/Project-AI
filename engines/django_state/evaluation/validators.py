    ...
    def validate_trust_ceiling(state_before: StateVector, state_after: StateVector) -> tuple[bool, str]:
        ...
        if state_after.trust.ceiling is not None:
            if state_after.trust.value > state_after.trust.ceiling:
                return False, f"Trust ceiling violated: {state_after.trust.value} > {state_after.trust.ceiling}"
        ...
        if state_before.trust.ceiling is not None and state_after.trust.ceiling is not None:
            if state_after.trust.ceiling > state_before.trust.ceiling:
                return False, f"Trust ceiling increased: {state_before.trust.ceiling} -> {state_after.trust.ceiling}"
    ...
    def validate_moral_injury_floor(state_before: StateVector, state_after: StateVector) -> tuple[bool, str]:
        ...
        if state_after.moral_injury.floor is not None:
            if state_after.moral_injury.value < state_after.moral_injury.floor:
                return False, f"Moral injury floor violated: {state_after.moral_injury.value} < {state_after.moral_injury.floor}"
        ...
        if state_before.moral_injury.floor is not None and state_after.moral_injury.floor is not None:
            if state_after.moral_injury.floor < state_before.moral_injury.floor:
                return False, f"Moral injury floor decreased: {state_before.moral_injury.floor} -> {state_after.moral_injury.floor}"
    ...
    def validate_all_irreversibility(state_before: StateVector, state_after: StateVector) -> tuple[bool, list[str]]:
        ...
        if state_after.legitimacy.ceiling is not None:
            if state_after.legitimacy.value > state_after.legitimacy.ceiling:
                errors.append(f"Legitimacy ceiling violated: {state_after.legitimacy.value} > {state_after.legitimacy.ceiling}")
        ...
        if state_after.epistemic_confidence.ceiling is not None:
            if state_after.epistemic_confidence.value > state_after.epistemic_confidence.ceiling:
                errors.append(f"Epistemic ceiling violated: {state_after.epistemic_confidence.value} > {state_after.epistemic_confidence.ceiling}")
