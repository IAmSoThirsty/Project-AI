...
class BaseSignal(BaseModel):
    ...
    priority: SignalPriority = Field(default=SignalPriority.NORMAL, description="Signal priority")
...
class DistressSignal(BaseSignal):
    ...
    escalation_path: List[str] = Field(default_factory=list, description="Escalation chain")
...
class IncidentSignal(BaseSignal):
    ...
    anomaly_score: float = Field(ge=0.0, le=1.0, default=0.5, description="Anomaly detection score")
...
class MediaSignal(BaseSignal):
    ...
    duration_seconds: Optional[float] = Field(default=None, description="Duration for audio/video")
    file_size_bytes: Optional[int] = Field(default=None)
    checksum: Optional[str] = Field(default=None, description="File checksum (SHA-256)")
    transcript: Optional[str] = Field(default=None, description="Transcription of audio/video")
...
def validate_signal(signal_data: Dict[str, Any]) -> ValidationResult:
    ...
    try:
        ...
        if forbidden:
            ...
            result.errors.append(f"Forbidden phrases detected: {', '.join(forbidden[:3])}")
