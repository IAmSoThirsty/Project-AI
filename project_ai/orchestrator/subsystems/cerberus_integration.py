...
cerberus_path = Path(__file__).parent.parent.parent.parent / "external" / "Cerberus" / "src"
...
class CerberusIntegration:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'validator_stats': self.validator.get_statistics() if hasattr(self.validator, 'get_statistics') else {},
            'threat_level': self.threat_detector.get_current_threat_level() if hasattr(self.threat_detector, 'get_current_threat_level') else 'UNKNOWN'
        }
    
    def analyze_input(self, user_input: str) -> Dict[str, Any]:
        """Analyze input for threats (main API)"""
        if not CERBERUS_AVAILABLE or not self.hub:
            return {'should_block': False, 'summary': 'Cerberus not available', 'active_guardians': 0}
