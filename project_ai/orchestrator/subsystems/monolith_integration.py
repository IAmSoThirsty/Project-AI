...
monolith_path = Path(__file__).parent.parent.parent.parent / "external" / "Thirstys-Monolith" / "src"
...
class MonolithIntegration:
    ...
    def start(self) -> None:
        ...
        if not MONOLITH_AVAILABLE or not self.guardian:
            self.logger.warning("Guardian not available - skipping schematic enforcement")
            self.active = False
            return
