...
# COMPLIANCE: Sovereign Substrate / capability_invoker.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / capability_invoker.py
...
class CapabilityInvoker:
    ...
    def _load_registry(self, config: dict) -> dict[str, dict[str, Any]]:
        ...
        registry["sensor_fusion"] = {
            "name": "sensor_fusion",
            "risk_level": 2,
            "requires_external": False,
            "fn": self._cap_sensor_fusion,
        }
        registry["robotic_synthesis"] = {
            "name": "robotic_synthesis",
            "risk_level": 3,
            "requires_external": True,
            "fn": self._cap_robotic_synthesis,
        }
        registry["neuromorphic_compiler"] = {
            "name": "neuromorphic_compiler",
            "risk_level": 3,
            "requires_external": False,
            "fn": self._cap_neuromorphic_compiler,
        }
    ...
    def _cap_sensor_fusion(self, inputs: dict) -> dict:
        """Invoke SensorFusion capability."""
        from .sensor_fusion import SensorFusion

        fusion = SensorFusion()
        return fusion.process_telemetry(inputs)

    def _cap_robotic_synthesis(self, inputs: dict) -> dict:
        """Invoke Robotic Synthesis discovery cycle."""
        from .robotic_synthesis import SelfDrivingLab, MaterialSpec

        lab = SelfDrivingLab(inputs.get("lab_id", "DEFAULT_LAB"))
        spec = MaterialSpec(
            name=inputs.get("name", "Unknown_Compound"),
            composition=inputs.get("composition", {}),
            predicted_properties=inputs.get("predicted_properties", {})
        )
        return lab.run_discovery_cycle(spec)

    def _cap_neuromorphic_compiler(self, inputs: dict) -> dict:
        """Invoke Neuromorphic SNN transpilation."""
        from .neuromorphic_compiler import NeuromorphicCompiler

        compiler = NeuromorphicCompiler()
        ast = inputs.get("thirsty_ast", {"type": "CognitiveService", "body": []})
        return compiler.transpile_to_snn(ast)
