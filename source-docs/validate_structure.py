...
class SourceDocsValidator:
    ...
    def _validate_readme_content(self, readme_path: Path, subdir: str):
        ...
        required_sections = ["Purpose", "Related Documentation"]

        for section in required_sections:
            if f"## {section}" in content or f"### {section}" in content:
                self.successes.append(
                    f"  ✓ '{subdir}/README.md' has '{section}' section"
                )
            else:
                self.warnings.append(
                    f"  ⚠ '{subdir}/README.md' missing '{section}' section"
                )
    ...
    def _check_master_readme(self):
        ...
        with open(master_readme, encoding="utf-8") as f:
            content = f.read()
        ...
        for subdir in self.REQUIRED_SUBDIRS:
            if (
                f"[{subdir}/](./{subdir}/README.md)" in content
                or f"source-docs/{subdir}/" in content
            ):
    ...
    def _check_orphaned_files(self):
        ...
        allowed_files = {"README.md", "validate_structure.py", "TREE.md", "__pycache__"}
        ...
        for item in self.ROOT_DIR.iterdir():
            ...
            if item.is_file() and item.name not in allowed_files:
                ...
                item.is_dir()
                and item.name not in allowed_dirs
        ...
        if orphaned:
            self.warnings.append(
                f"⚠ Orphaned files/directories found: {', '.join(orphaned)}"
            )
        else:
    ...
    def _print_results(self):
        ...
        if self.errors:
            ...
            print(
                f"{Color.YELLOW}{Color.BOLD}VALIDATION PASSED WITH WARNINGS{Color.END}"
            )
    ...
    def generate_tree(self, output_file: str = "TREE.md"):
        ...
        tree_lines.append(
            f"- **Total Files:** {len(root_files) + sum(len(list((self.ROOT_DIR / d).iterdir())) for d in self.REQUIRED_SUBDIRS if (self.ROOT_DIR / d).exists())}"
        )
        tree_lines.append("")
...
def main():
    ...
    parser.add_argument(
        "--tree", action="store_true", help="Generate directory tree documentation"
    )
    parser.add_argument(
        "--check-links",
        action="store_true",
