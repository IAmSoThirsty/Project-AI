...
echo "  3. Configure Workload Identity (see SECURITY_VERIFICATION_REPORT.md)"
echo "  4. Update CI/CD pipelines with KMS key reference"
echo "  5. Sign images using: COSIGN_EXPERIMENTAL=1 cosign sign --key 'gcpkms://projects/${PROJECT_ID}/locations/${LOCATION}/keyRings/${KEYRING_NAME}/cryptoKeys/${KEY_NAME}' <IMAGE>"
echo "  6. For production: upgrade key protection level to HSM"
echo ""
...
echo "  - NEVER use JSON key files - use Workload Identity instead"
echo "  - Consider enabling key rotation policy"
echo "  - Review IAM bindings regularly"
echo ""
echo -e "${BLUE}📋 VERIFICATION:${NC}"
echo "  Run the verification commands from SECURITY_VERIFICATION_REPORT.md"
echo ""
echo -e "${GREEN}✅ Setup complete! Proceed with verification and Workload Identity configuration.${NC}"
echo ""
