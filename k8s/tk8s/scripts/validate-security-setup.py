...
class SecurityValidator:
    ...
    def check_kubectl_access(self) -> ValidationResult:
        ...
        if code == 0:
            code2, stdout2, stderr2 = self.run_command(['kubectl', 'cluster-info'], check=False)
            if code2 == 0:
    ...
    def check_kyverno_installed(self) -> ValidationResult:
        ...
        if code == 0:
            return ValidationResult(
                ...
                True,
                "Kyverno is installed and running",
                stdout
            )
        else:
            return ValidationResult(
    ...
    def check_pod_security_admission(self) -> ValidationResult:
        ...
        code, stdout, stderr = self.run_command(
            ['kubectl', 'get', 'namespace', '-l', 'app.kubernetes.io/name=project-ai', '-o', 'json'],
            check=False
        )
        if code != 0:
            return ValidationResult(
    ...
    def run_all_checks(self):
        ...
        if passed == total:
            ...
            print(f"{YELLOW}   {total - passed} checks failed or require attention.{NC}")
            print()
            print("Failed checks:")
