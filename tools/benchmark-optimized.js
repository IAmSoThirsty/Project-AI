...
/**
 * Optimized Thirsty-lang Benchmark Tool
 * Compare performance across different implementations and optimizations
 * with console output suppression for accurate measurements
 */

const fs = require('fs');
const ThirstyInterpreter = require('../src/thirsty_lang/src/index');
...
class ThirstyBenchmark {
  ...
  async runBenchmark(name, code, iterations = 1000) {
    console.log("\nð Running benchmark: "+name);
    console.log("   Iterations: "+iterations);

    const times = [];
    const interpreter = new ThirstyInterpreter();

    // Suppress console output during benchmark for accuracy
    ...
    console.log("   â Average: "+avg.toFixed(3)+" ms");
    console.log("   â Min: "+min.toFixed(3)+" ms");
    console.log("   â Max: "+max.toFixed(3)+" ms");
