import { ApiError, gateway, type DashboardResponse, type ModuleSurface, type WorkOperation, type WorkRequest, type WorkRequestDetail } from "@project-ai/web-shared/api";
...
export function WorkQueueRoute({ mode }: { mode: "inbox" | "requests" }) {
  ...
  const canSubmitRequests = ["owner", "administrator", "operator"].includes(auth.session?.account.role ?? "");
  const canReviewRequests = ["owner", "administrator", "reviewer"].includes(auth.session?.account.role ?? "");
  useEffect(() => {
    let active = true;
    setIsLoading(true);
    setLoadError("");
    ...
    (async () => {
      try {
        const requestResult = await gateway.work.requests();
        let operationResult: { operations: WorkOperation[] } = { operations: [] };
        if (mode === "requests" && canSubmitRequests) {
          try {
            operationResult = await gateway.work.operations();
          } catch (reason) {
            if (!(reason instanceof ApiError && reason.status === 403)) throw reason;
            if (active) setOperationsError(reason.message);
          }
        }
        if (!active) return;
  ...
  const reviewableRequests = requests.filter((item) => item.state === "submitted" && item.created_by !== auth.session?.account.id);
  ...
  async function cancel(requestId: string) {
    ...
    setCancellingId(requestId); setError(""); setNotice("");
    try {
  ...
  async function showDetail(requestId: string) {
    ...
    setDetailLoadingId(requestId); setError("");
  ...
  return <div className="console-page"><PageHeading title={title} description={mode === "inbox" ? "Reviews and escalations assigned to your human account." : "Governed requests remain distinct from ALLOW decisions and executed effects."} />
    ...
    {!isLoading && !loadError && mode === "requests" && (!canSubmitRequests || operationsError) ? <StatePanel title="Request submission restricted" tone="warning">Your interface role cannot record execution requests. {operationsError || "Ask an authorized operator or administrator to submit the human work record."}</StatePanel> : null}
    {!isLoading && !loadError && mode === "requests" && canSubmitRequests && !operationsError ? <section className="records-panel workflow-composer"><div className="panel-heading"><div><h2>Record a request</h2><p>This submits a human work record only. The execution gate is not called.</p></div><ClipboardList /></div><form className="security-form" onSubmit={submit}><label>Title<input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} required /></label><label>Operation<select value={form.operation} onChange={(event) => setForm({ ...form, operation: event.target.value, inputs: {} })} required><option value="">Select an allowlisted operation</option>{operations.map((operation) => <option key={operation.id} value={operation.id}>{operation.label}</option>)}</select></label>{selectedOperation ? <><p className="operation-boundary">{selectedOperation.description} {selectedOperation.consequence}<small>Input contract: {selectedOperation.schema_version}</small></p>{selectedOperation.fields.map((field) => <label key={field.id}>{field.label}<span>{field.description}</span><div className="schema-input"><code>{field.resource_prefix}</code><input value={form.inputs[field.id] ?? ""} onChange={(event) => setForm({ ...form, inputs: { ...form.inputs, [field.id]: event.target.value } })} placeholder={field.placeholder} minLength={field.min_length} maxLength={field.max_length} pattern={field.pattern} required /></div></label>)}</> : null}<label>Rationale<textarea value={form.rationale} onChange={(event) => setForm({ ...form, rationale: event.target.value })} required /></label><button type="submit" disabled={submitting}>{submitting ? "Recording request…" : "Record request"}</button></form></section> : null}
    {!isLoading && !loadError && mode === "inbox" && canReviewRequests && reviewableRequests.length > 0 ? <section className="records-panel workflow-composer"><div className="panel-heading"><div><h2>Record human review</h2><p>Recent MFA step-up is required. Approval forwards for governance; it is not ALLOW.</p></div><Inbox /></div><form className="security-form" onSubmit={decide}><label>Request<select value={review.requestId} onChange={(event) => setReview({ ...review, requestId: event.target.value })} required><option value="">Select request</option>{reviewableRequests.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><label>Decision<select value={review.decision} onChange={(event) => setReview({ ...review, decision: event.target.value as typeof review.decision })}><option value="approve_for_governance">Approve for governance evaluation</option><option value="reject">Reject</option><option value="return_for_information">Return for information</option><option value="abstain">Abstain</option></select></label><label>Rationale<textarea value={review.rationale} onChange={(event) => setReview({ ...review, rationale: event.target.value })} required /></label><button type="submit" disabled={reviewing}>{reviewing ? "Recording review…" : "Record review"}</button></form></section> : null}
    {!isLoading && !loadError ? <section className={`records-panel ${requests.length ? "workflow-list" : "empty-workflow"}`}><Icon />{requests.length ? <div>{requests.map((item) => <article key={item.id}><div><strong>{item.title}</strong><span>{item.operation} · {item.resource}</span><small>Requested {new Date(item.created_at).toLocaleString()}</small></div><div className="workflow-state-actions"><span className="workflow-state">{item.state.replaceAll("_", " ")}</span><button type="button" disabled={Boolean(detailLoadingId)} onClick={() => showDetail(item.id)}>{detailLoadingId === item.id ? "Loading detail…" : "View detail"}</button>{item.created_by === auth.session?.account.id && (item.state === "submitted" || item.state === "needs_information") ? <button type="button" disabled={Boolean(cancellingId)} onClick={() => cancel(item.id)}>{cancellingId === item.id ? "Cancelling…" : "Cancel request"}</button> : null}</div><p>{item.rationale}</p></article>)}</div> : <><h2>No requests are visible</h2><p>The durable workflow store returned zero records for this account.</p></>}</section> : null}
