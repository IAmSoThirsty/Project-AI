...
async def create_forensic_snapshot(campaign_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        metadata = {
            ...
            "included_paths": ["config", "policies", "data/red_team_personas", "data/metrics"],
            "immutable": True
        }
...
async def run_red_team_attack(
    ...
    activity.logger.info(f"Running red team attack: {persona} on {target} (activity: {activity_id})")
    ...
    try:
        ...
        if activity.info().attempt >= 3:
            activity.logger.warning(f"Attack marked as flaky after 3 attempts")
        raise
...
async def evaluate_attack(attack_result: Dict[str, Any]) -> str:
    ...
    """
    activity.logger.info(f"Evaluating attack: {attack_result['persona']} on {attack_result['target']}")
...
@activity.defn
async def generate_sarif(results: List[Dict[str, Any]], campaign_id: str) -> Dict[str, Any]:
    """
...
@activity.defn
async def upload_sarif(sarif: Dict[str, Any], repo: str, commit_sha: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        artifact_file = Path(f"artifacts/sarif/{datetime.now().strftime('%Y%m%d%H%M%S')}.sarif.json")
