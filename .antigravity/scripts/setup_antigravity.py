...
# COMPLIANCE: Sovereign Substrate / setup_antigravity.py
# ============================================================================ #
#
# COMPLIANCE: Sovereign Substrate / setup_antigravity.py
...
class AntigravitySetup:
    ...
    def check_workflow_files(self) -> dict[str, bool]:
        ...
        return {
            "feature_development": (
                workflows_dir / "feature-development.yaml"
            ).exists(),
    ...
    def generate_recommendations(
        ...
        if not config_valid:
            recommendations.append(
                "Fix configuration issues in .antigravity/config.json"
            )

        if not agents.get("project_ai_agent"):
            recommendations.append(
                "Custom agent file missing - reinstall Antigravity integration"
            )

        if not workflows.get("feature_development") or not workflows.get(
            "security_fix"
        ):
            recommendations.append(
                "Workflow definitions missing - reinstall Antigravity integration"
            )
...
def main():
    ...
    all_ready = (
        all(prereqs.values())
        and is_valid
        and all(agents.values())
        and all(workflows.values())
    )
