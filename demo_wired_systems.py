...
def demo_ethics_approval_events():
    ...
    for subsystem_id, priority in subsystems:
        ...
        metadata = {"priority": priority, "description": f"{subsystem_id} initialization"}
    ...
    print(f"   All approvals emitted as events: {len(received_events) == len(subsystems)}")
...
def demo_governance_integration():
    ...
    for subsystem in ["tactical_edge_ai", "supply_logistics", "situational_awareness"]:
        ...
        if must_consult:
            ...
            print(f"      No consultation required")
    ...
    def event_callback(event):
        ...
        print(f"      Must consult: {', '.join(must_consult) if must_consult else 'None'}")
...
def demo_emergency_and_checkpoint_events():
    ...
    def track_all(event):
        ...
        print(f"   Type: {event.payload.get('event_type') or event.payload.get('decision_type')}")
