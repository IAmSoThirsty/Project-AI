...
class LoggingRefactorer(ast.NodeTransformer):
    ...
    def _convert_fstring_to_format(self, fstring: ast.JoinedStr) -> tuple[str, list[ast.expr]]:
...
def main():
    ...
    files = [f for f in all_files if not any(excluded in f.parts for excluded in default_excludes)]
