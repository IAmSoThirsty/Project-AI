...
def run_tier_0():
    ...
    if charter_hash:
        ...
        expected_hash = "b821bfb9676b6b2aefbb10c7e0b7a3aee48757045b675b5d29047de6ff8492eb"
        status = "PASS" if charter_hash == expected_hash else "FAIL"
        with open(charter_path, encoding="utf-8") as f:
            lines = f.readlines()
            version = "2.1" if any("Document Version: 2.1" in line for line in lines) else "UNKNOWN"
    ...
    try:
        ...
        result, msg = FourLaws.validate_action("initiate_genesis", {"entity_class": "appointed"})
...
def run_tier_2():
    ...
    monolith_path = os.path.join(ROOT, "src", "app", "governance", "planetary_defense_monolith.py")
    if os.path.exists(monolith_path):
        ...
        log_check("T2.1.1", "CRITICAL", "FAIL", "planetary_defense_monolith.py not found")
...
def run_tier_3():
    ...
    if os.path.exists(commission_path):
        log_check("T3.1.1", "HIGH", "PASS", f"Legion Commission found at {commission_path}")
    else:
