...
def run_benchmarks():
    ...
    # Health check
    benchmarks.append(benchmark_endpoint("Health Check", "GET", "/health", iterations=200))
    ...
    # Audit log
    benchmarks.append(
        benchmark_endpoint("Audit Log (limit=10)", "GET", "/audit?limit=10", iterations=100)
    )
    ...
    benchmarks.append(
        benchmark_endpoint("Submit Intent", "POST", "/intent", iterations=50, json=intent_data)
    )
    ...
    for b in benchmarks:
        if "error" not in b:
            print(f"{b['name']:<30} {b['mean_ms']:<12.2f} {b['p95_ms']:<12.2f} {b['errors']}")
