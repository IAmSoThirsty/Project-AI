...
def demo_boot_profiles():
    ...
    should_init, _ = boot.should_initialize_subsystem("test_subsystem", {"priority": "MEDIUM"})
    print(f"   NORMAL profile allows test_subsystem: {should_init}")

    # Emergency profile
    boot.set_boot_profile(BootProfile.EMERGENCY)
    should_init, reason = boot.should_initialize_subsystem("test_subsystem", {"priority": "MEDIUM"})
    ...
    should_init, _ = boot.should_initialize_subsystem("ethics_governance", {"priority": "CRITICAL"})
    ...
    should_init, reason = boot.should_initialize_subsystem("other_subsystem", {"priority": "HIGH"})
...
def demo_ethics_first():
    ...
    should_init, _ = boot.should_initialize_subsystem("agi_safeguards", {"priority": "CRITICAL"})
    print(f"   ✓ agi_safeguards: {should_init}")

    # Non-ethics subsystem must wait
    should_init, reason = boot.should_initialize_subsystem("tactical_edge_ai", {"priority": "HIGH"})
    ...
    should_init, _ = boot.should_initialize_subsystem("tactical_edge_ai", {"priority": "HIGH"})
    print(f"   ✓ tactical_edge_ai (after approval): {should_init}")

    should_init, _ = boot.should_initialize_subsystem("command_control", {"priority": "HIGH"})
