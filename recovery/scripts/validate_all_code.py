...
class CodeValidator:
    ...
    def validate_ruff(self) -> ValidationResult:
        ...
        if "[*]" in result.details:
            try:
                fixable_line = [
                    line for line in result.details.split("\n") if "fixable with" in line.lower()
                ][0]
                result.fixable = int(fixable_line.split("fixable")[0].strip().split()[-1])
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Comprehensive code validation for Project-AI")
    parser.add_argument("--fix", action="store_true", help="Auto-fix issues where possible")
    parser.add_argument("--fast", action="store_true", help="Run fast checks only")
    parser.add_argument("--report", action="store_true", help="Generate JSON validation report")
