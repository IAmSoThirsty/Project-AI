...
class ComprehensiveBenchmarkRunner:
    ...
    def benchmark_suite(self, suite_name: str) -> RobustnessAnalysis:
        ...
        # Aggregate analysis
        analysis = self.metrics_engine.aggregate_robustness_analysis(proximity_metrics, suite_name)

        # Export results
        export_path = self.metrics_engine.export_metrics(proximity_metrics, analysis, suite_name)
    ...
    def generate_comparative_report(self) -> str:
        ...
        report_path = (
            self.data_dir / "robustness_metrics" / f"comparative_robustness_report_{timestamp}.md"
        )
        ...
        with open(report_path, "w") as f:
            ...
            total_near_miss = sum(
                r["analysis"].near_miss_count for r in self.results.values() if r["analysis"]
            )
            ...
            all_margins = [
                r["analysis"].min_robustness_margin for r in self.results.values() if r["analysis"]
            ]
            all_confidences = [
            ...
            all_lipschitz = [
                r["analysis"].avg_lipschitz_constant for r in self.results.values() if r["analysis"]
            ]
