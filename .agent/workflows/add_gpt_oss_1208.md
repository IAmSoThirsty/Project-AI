...
2. **Ensure the model files can be downloaded** – the first call to `generate_gpt_oss` will download the model from HuggingFace. Make sure you have internet access and sufficient disk space (~200 GB for the 120 B model).

3. **Instantiate the agent** in your Python code:
   ```python
   from app.agents.codex_deus_maximus import create_codex
   codex = create_codex(data_dir="data")
   ```

4. **Generate a response** using the new method:
   ```python
...
5. **Optional – integrate into existing workflows**:
...
6. **Run a quick sanity check** (auto‑run):
   ```bash
   python - <<PY
   from app.agents.codex_deus_maximus import create_codex
   c = create_codex()
